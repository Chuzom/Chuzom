...
⚡ ROUTE: research/moderate → try llm(task="research")
⚡ ROUTE: code/complex → try llm(task="code", tier="best")
```
...
1. If the task fits the hint, **call the suggested MCP tool** (`llm(task="query")` / `llm(task="analyze")`
   / `llm(task="code")` / `llm(task="research")` / `llm(task="generate")`) and return its result.
...
|---|---|---|
| `research/*` | `llm(task="research")` | Perplexity / web-grounded models |
| `generate/*` | `llm(task="generate")` | Gemini Flash / Haiku for writing |
| `analyze/*` | `llm(task="analyze")` | Sonnet-class for deep analysis |
| `code/*` | `llm(task="code")` | Coder models (Ollama qwen-coder, Codex, etc.) |
| `query/*` | `llm(task="query")` | Haiku / Gemini Flash / Kimi for questions |
...
- Deep analysis → `llm(task="analyze")`
- Code questions → `llm(task="code")`
- Simple questions → `llm(task="query")`
