...
NONDETERMINISM_NOTE = (
    ...
    "corpus row ordering (load_corpus reads deterministically). NOT ruled out "
    "(corrected in the Phase 0.2 re-audit): the TOTAL _call_text invocation "
    "count varies run-to-run (measured ~46/42/47 across three runs of 40 rows), "
    "i.e. content-dependent contract-gate failures DO trigger same-model / "
    "emergency-fallback retries in a run-varying way -- so variable retry "
    "dispatch is a LIKELY (still unconfirmed) contributor to the nondeterminism, "
    "not an excluded cause. (An earlier single-session probe reported exactly-"
    "once-per-row; the per-run dispatch-count measurement refuted it.) Other "
    "unconfirmed suspects: chuzom.quality_feedback's process-global "
    "_quality_store (written on every successful dispatch via "
    "record_quality/score_response in the P2 escalation-scoring path, but not "
    "confirmed as the leak); and _spawn_bg/store_receipt background-task timing. "
    "Conclusion: MITIGATE, not PIN -- rely on "
