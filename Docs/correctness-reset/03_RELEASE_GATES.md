...
| 13 | All critical mutations killed | **FAIL (honest — equivalent mutants remain)** | Scoped mutmut over hermetic modules: `execution_ledger.py` (#173) and `execution_signal.py` — the latter went **32→48 / 54 killed** after strengthening the ENF-FIX-1 signal tests (pinned `fires is True/False` not truthiness; asserted the `reason`/`verb`/`obj` transparency contract on every non-firing branch). The remaining 6 are provably **equivalent mutants** (capturing-group `group(0)==group(1)`; synthetic `XX`-padding of internal reason strings whose meaning is unchanged; empty-prompt fallback that returns `False` either way). Gate stays FAIL because equivalent mutants can't be killed without over-fitting — documented, not hidden |
