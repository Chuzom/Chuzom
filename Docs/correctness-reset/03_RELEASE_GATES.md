...
| 15 | Control-group benchmark shows positive net verified token savings | **FAIL (built + run; clean only when fully local)** | Harness built and **run for real** (`10_CODEX_QUOTA_BENCHMARK.md`). Cash A/B vs metered GPT-4o: **easy corpus is a clean pass** (chuzom $0 all-local vs GPT-4o $0.0024, +net, quality reliable). But **moderate/hard is confounded** — chuzom's apparent $0 is subscription-**offload** (Codex/Claude-Opus at unpriced $0), which Gate 17 flags, so the +$0.03 overstates the real saving. No **clean blanket** cash-savings result across difficulty yet; needs a run with the escalation tier ALSO metered, larger corpus |
| 16 | Quality within non-inferiority margin | **PARTIAL (real evidence, small corpus)** | Now measured with a **reliable GPT-4o judge** (fixing the weak-Ollama-judge confound): easy delta −0.20 (objective), moderate/hard −0.33 — both within the 0.5 margin; every escalated prompt scored q=5. Directionally supported on a small corpus; not yet a release-scale sample |
| 17 | No benchmark event has unclassified spend | **FAIL (working as designed — caught real offload)** | The benchmark's Gate-17 check **fired correctly**: on the moderate/hard run it flagged 11 chuzom escalations to Codex/Claude-Opus subscriptions recorded at $0 as unclassified. That is the check doing its job, not passing — a clean run requires those escalations to route to a metered tier so their cost is captured |
