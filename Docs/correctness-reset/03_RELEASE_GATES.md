...
| 15 | Control-group benchmark shows positive net verified token savings | **FAIL (built + run; clean metered run is NET-NEGATIVE on moderate/hard)** | Harness built and **run for real** (`10_CODEX_QUOTA_BENCHMARK.md`). Easy corpus: **clean pass** (chuzom $0 all-local vs GPT-4o $0.0024). The **clean metered-escalation run** (fifth run — Codex/Claude off, fresh isolated DB so no `semantic_cache` contamination) settles moderate/hard: chuzom **$0.03941 vs GPT-4o $0.02930 → NET −$0.01011** (Chuzom *more* expensive). Once the escalation tier is metered, the prior "+$0.03" is exposed as **subscription-offload priced at $0**; with real prices there is **no blanket cash win across difficulty**. o3 escalations cost $0.012–0.015 (mod-05: o3 q=2 for $0.015) and 5 hard prompts EXHAUSTED at q=1 |
| 16 | Quality within non-inferiority margin | **FAIL (clean metered run: −1.00, beyond margin)** | Easy delta −0.20 (objective, reliable). But the **clean metered moderate/hard run** gives delta **−1.00** — beyond the 0.5 margin — because with subscriptions disabled the premium chain **exhausted** on 5 hard prompts (`hard-04…08`, q=1). The fourth run's within-margin −0.33 was an artifact of subscription-offload answering those prompts at q=5; metered/un-confounded, quality on the hard tier is *not* non-inferior. Passes on easy only |
| 17 | No benchmark event has unclassified spend | **FAIL (working as designed — still catches residual offload)** | The Gate-17 check **fires correctly**. On the clean metered run it flagged **5** residual `codex/gpt-5.5` $0 escalations — because `CHUZOM_DISABLE_SUBPROCESS_BACKENDS` suppresses Codex's availability check but not Codex when it's already a broker provider (router.py:606–611). The check doing its job; a fully-clean run needs the broker-Codex path also metered/removed. Note: this residual only makes Gate 15 *more* negative (those calls would add cost), so it does not rescue the verdict |
