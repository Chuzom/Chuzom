...
# the PR-#155 violation. Analysis code under bench/routerarena/clean/sandbox is
# intentionally NOT scanned (it evaluates; it does not feed the router).
set -euo pipefail
echo "=== RA eval-output spillover guard ==="
PATTERNS=('_sub10_result\.json' 'bench/routerarena/clean/quarantine' 'sub10_labels\.json' 'full_labels\.json' 'chuzom-v3-pred\.json')
SCAN=(src/chuzom bench/routerarena/submission/router)
v=0
for d in "${SCAN[@]}"; do
  [ -d "$d" ] || continue
