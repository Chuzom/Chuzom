...
1. **Quarantine the RA eval artifacts.** The per-dataset result files carry RA-derived accuracies and must be treated as **read-only evidence**, never as input:
   - `bench/routerarena/clean/sandbox/chuzom-cascade_sub10_result.json`
   - `bench/routerarena/clean/sandbox/chuzom-cascade-full_sub10_result.json`
   Keep them **out of PR #130** and out of any branch that feeds the production router. Do not `import`/read them from any router or parameter-building code. Prefer moving them under a clearly-marked `bench/routerarena/clean/quarantine/` dir (git-ignored) or deleting once the numbers are recorded in `STATUS.md`.
...
- [ ] Move `*_sub10_result.json` to `bench/routerarena/clean/quarantine/` (git-ignored) or delete after recording numbers in `STATUS.md`.
