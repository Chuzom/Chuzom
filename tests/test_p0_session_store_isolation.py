...
with an assertion message containing REAL routed content from the developer's own machine:
    ...
    assert 'still works' in '[chuzom-session-context]\\nROUTED(openai/gpt-4o-mini): ...'

That text was not produced by the test. It came from
`~/.chuzom/projects/<id>/session_context_<session>.jsonl` — the live session store of the
...
THE DEFECT: THREE SANDBOX MECHANISMS, AND THIS MODULE HONOURED THE UNUSED ONE
    ...
    CHUZOM_HOME (paths.py)        6 test files, is_isolated()      no
...
returns True under CHUZOM_HOME while this module writes and reads outside that sandbox.
...
Beyond tests: any deployment relocating state with CHUZOM_HOME — a sandbox, a CI runner,
...
import chuzom.session_store as session_store
from chuzom.paths import chuzom_home, is_isolated


class TestChuzomHomeIsHonoured:
    """The canonical mechanism. `is_isolated()` must not be able to lie."""

    def test_state_dir_follows_chuzom_home(self, tmp_path, monkeypatch):
    ...
    def test_isolated_state_dir_agrees_with_the_canonical_helper(self, tmp_path, monkeypatch):
        ...
        """
        monkeypatch.setenv("CHUZOM_HOME", str(tmp_path))
        assert session_store._state_dir() == chuzom_home()
...
class TestPathHomePatchIsHonoured:
    ...
    def test_state_dir_follows_a_patched_path_home(self, tmp_path):
        with patch("pathlib.Path.home", return_value=tmp_path):
            assert session_store._state_dir() == tmp_path / ".chuzom"
    ...
    def test_a_written_event_lands_in_the_sandbox_not_the_real_home(self, tmp_path):
        ...
        wrong: chuzom legitimately records the developer's tool calls, so editing this
        very file wrote the canary string into a real session log, and the test failed
        on chuzom working correctly. A per-run session id can only appear in a file this
        test caused to be created.
        """
        session_id = f"iso-sess-{uuid.uuid4()}"
        real_home_store = pathlib.Path.home() / ".chuzom"

        with patch("pathlib.Path.home", return_value=tmp_path):
            session_store.record_event(session_id, "user_prompt", "sandbox canary")

        written = list((tmp_path / ".chuzom").rglob(f"session_context_{session_id}.jsonl"))
