...
falling through to Claude" to ~/.chuzom/auto-route-debug.log and carried on.
...
request:
  ...
  * config problem  — the model answers, just slower than the timeout. Say so
    with a number: raise CHUZOM_OLLAMA_TIMEOUT to about the observed p90.
...
from chuzom import direct_diagnostics as dd


@pytest.fixture(autouse=True)
def _isolated_state(tmp_path, monkeypatch):
    """CHZ-SR-01: isolate via CHUZOM_HOME, the one mechanism chuzom.paths honours.

    These helpers deliberately take no `home=` override — a test that needs a
    sandbox uses the same lever production does, so the sandbox cannot silently
    diverge from where the code actually writes.
    """
    monkeypatch.setenv("CHUZOM_HOME", str(tmp_path / ".chuzom"))
...
def test_slower_than_the_timeout_but_viable_suggests_a_number():
    ...
    assert "CHUZOM_OLLAMA_TIMEOUT" in advice.message
...
def test_hopeless_latency_says_so_instead_of_offering_a_knob():
    ...
    assert "CHUZOM_OLLAMA_TIMEOUT" not in advice.message, (
...
def test_samples_persist_across_invocations(tmp_path):
    ...
    for s in (9.9, 10.5, 12.6, 19.4, 27.5, 33.6):
        dd.record_sample(s, timed_out=True)
    advice = dd.current_advice(timeout_s=4.0)
    assert advice is not None and advice.kind == "raise_timeout"


def test_the_ring_buffer_is_bounded(tmp_path):
    for i in range(100):
        dd.record_sample(float(i), timed_out=True)
    assert len(dd.load_samples()) <= dd._MAX_SAMPLES


def test_recording_never_raises_on_an_unwritable_state_dir(tmp_path, monkeypatch):
    """It runs inside a hook; an unwritable state dir must not cost a prompt."""
    blocked = tmp_path / "nope"
    blocked.write_text("i am a file, not a directory")
    monkeypatch.setenv("CHUZOM_HOME", str(blocked / ".chuzom"))
    dd.record_sample(1.0, timed_out=True)  # must not raise
    assert dd.load_samples() == []
...
def test_corrupt_sample_lines_are_skipped_not_fatal(tmp_path):
    ...
    body = "\n".join([
        json.dumps({"elapsed_s": 10.0, "timed_out": True}),
        "not json at all",
        json.dumps({"bad": 1}),
    ]) + "\n"
    (d / "direct_samples.jsonl").write_text(body)
    assert dd.load_samples() == [dd.Sample(10.0, True)]
...
def test_doctor_surfaces_the_advice(tmp_path, monkeypatch, capsys):
    ...
    import chuzom.direct_diagnostics as _dd

    for s in (9.9, 10.5, 12.6, 19.4, 27.5, 33.6):
        _dd.record_sample(s, timed_out=True)
    monkeypatch.setattr(Path, "home", classmethod(lambda cls: tmp_path))
    monkeypatch.setenv("CHUZOM_OLLAMA_TIMEOUT", "4")

    advice = _dd.current_advice(timeout_s=4.0)
    assert advice is not None
    assert "CHUZOM_OLLAMA_TIMEOUT=" in advice.message
    # The number must be actionable, not "increase the timeout".
    assert any(ch.isdigit() for ch in advice.message.split("CHUZOM_OLLAMA_TIMEOUT=")[1][:4])
