...
RULES_DIR = REPO / "src" / "chuzom" / "rules"
...
def test_localized_rules_name_only_registered_tools(rules_file):
    ...
    from chuzom.tool_surface import DEPRECATED_TOOLS, localize
...
def test_the_installer_localizes_before_writing(tmp_path, monkeypatch):
    ...
    """
    from chuzom.commands.install import _append_routing_rules
    ...
    from chuzom.tool_surface import localize
...
def test_guarded_is_derived_not_hand_maintained():
    ...
    four chuzom_agent_* — could be emitted unresolved with the lint reporting
