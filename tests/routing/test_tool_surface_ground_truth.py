...
``async def llm_*`` implementations in ``src/chuzom/tools/``. A tier may legally
...
#: tier's ``chuzom_admin``, ``chuzom_status``, the bare ``llm``, and the agent
...
_TOOL_PREFIXES = ("llm", "chuzom_")


def _implemented_tool_names() -> frozenset[str]:
    """Every tool coroutine defined under chuzom/tools/, by AST.
