"""SEC-1: the `chuzom-sse` console script must not exist.
...
Chuzom never regressed this — pyproject carries an explicit "chuzom-sse
intentionally NOT exposed" comment. The PUBLIC port (llm-router) did: its
script table was regenerated during the rebrand with hyphenated names, and
nothing was watching for the hyphenated variant of a ban written against the
underscore spelling. It shipped that way in 13.0.2 and 13.0.3.

This test is here so the same regression cannot happen upstream. It is the
check that would have caught it, and it costs nothing while the code is
already correct. The llm-router version was written with none of the three
conditions satisfied: `main_sse` has no auth middleware
...
The prohibition names the UNDERSCORE spelling (`chuzom-sse`), which is
