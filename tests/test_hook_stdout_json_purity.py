...
routing decision is logged via ``chuzom.model_tracking``. The hook process never
...
def _run_hook(prompt: str, home: Path, ollama_url: str, extra_env=None):
    ...
    env.pop("CHUZOM_DIRECT_EXECUTION", None)
    env["CHUZOM_DISABLE_LLM_CLASSIFIERS"] = "1"  # deterministic heuristic classify
...
def test_stdout_pure_json_even_at_debug_level(tmp_path, stub_ollama):
    ...
    result = _run_hook(
        ...
        extra_env={"CHUZOM_LOG_LEVEL": "DEBUG"},
    )
    out = result.stdout
    assert out.strip(), f"hook produced no stdout (stderr: {result.stderr[:400]})"
