...
the downstream package had this capability and Chuzom did not — one of four
...
``chuzom.audit_routing`` already exists as an unrelated feature (a live
...
from chuzom.misroute_audit import (
    AuditedDecision,
    audit_disabled,
    run_audit,
...
class TestRunAudit:
    ...
    async def test_the_writeback_guard_holds_on_its_own(self, audit_db):
        ...
        """
        from chuzom.misroute_audit import _write_verdict
    ...
    async def test_fails_open_on_a_broken_database(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("CHUZOM_DB_PATH", str(broken))
        monkeypatch.delenv("CHUZOM_AUDIT_DISABLED", raising=False)
...
class TestCli:
    """``chuzom audit misroute`` — a SUBcommand, deliberately.
...
def test_no_collision_with_the_live_audit_log():
    ...
    ``chuzom.audit_routing`` is a live per-turn compliance log; this module is
    ...
    """
    import chuzom.audit_routing as live
    import chuzom.misroute_audit as offline
