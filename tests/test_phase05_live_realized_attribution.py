...
enforce = _load(ENFORCE_HOOK, "chuzom_enforce_route_p05_live")
stop = _load(STOP_HOOK, "chuzom_stop_enforce_p05_live")
...
async def _drive_route_and_call(
    *,
    ...
    """
    monkeypatch.setenv("CHUZOM_ROUTING_LEDGER", str(tmp_path / f"rq-{session_id}.jsonl"))
    monkeypatch.setenv("CHUZOM_BANDIT", "off")
    monkeypatch.setenv("CHUZOM_EXECUTION_LEDGER_DB", str(ledger_db))
...
async def test_advise_mode_end_to_end_realizes_savings_via_hook_subprocess(
    ...
    subprocess hook run under CHUZOM_ENFORCE=advise that HONORS the pending
