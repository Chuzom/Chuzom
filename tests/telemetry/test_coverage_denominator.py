...
def _isolated_home(tmp_path, monkeypatch):
    ...
    CHUZOM_HOME did not isolate cost._get_db() and that gap destroyed real data
...
async def test_router_efficiency_with_no_decisions_is_unknown_not_zero(tmp_path, monkeypatch):
    ...
    """
    monkeypatch.setenv("CHUZOM_DB_PATH", str(tmp_path / "eff.db"))
    from chuzom.cost import get_router_efficiency
...
async def test_router_efficiency_carries_its_denominator(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("CHUZOM_DB_PATH", str(tmp_path / "eff2.db"))
    from chuzom.cost import get_router_efficiency
