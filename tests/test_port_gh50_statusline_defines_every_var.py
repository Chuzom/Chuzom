...
`chuzom-statusline.sh` sets STATE_DIR, USAGE_JSON and USAGE_DB together at
...
_SCRIPT = (
    ...
    / "src" / "chuzom" / "hooks" / "statusline-command.sh"
)
...
def test_savings_log_points_at_the_real_file():
    """hooks/savings_logger.py writes ~/.chuzom/savings_log.jsonl."""
    body = _SCRIPT.read_text()
...
def test_no_variable_is_read_bare_without_being_assigned():
    ...
    silently. NO_COLOR, CHUZOM_ENFORCE, CC_CONTEXT_LIMIT and
    CHUZOM_REFRESH_THROTTLE_SEC are all deliberately environment-supplied
...
def test_health_reports_ok_for_an_ollama_only_setup(tmp_path):
    ...
    (home / ".chuzom" / "savings_log.jsonl").write_text(json.dumps(record) + "\n")
    (home / ".chuzom" / "usage.json").write_text("{}")  # fresh -> not stale
    env = {
...
def test_statusline_and_writer_agree_on_the_log_schema():
    ...
    writer = (
        Path(__file__).resolve().parent.parent
        / "src" / "chuzom" / "hooks" / "savings_logger.py"
    ).read_text()
