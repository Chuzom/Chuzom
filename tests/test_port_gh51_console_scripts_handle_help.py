...
From `pipx list`, only `chuzom` and `chuzom-install-hooks` handled the
flag. The rest ran their default behaviour instead:

  * chuzom-onboard --help   -> launched the interactive API-key wizard and
                                   died on EOFError the moment stdin was not a
                                   TTY (onboard.py's input() call)
  * chuzom-quickstart --help-> same shape
  * chuzom-isolation-test   -> ignored the flag and ran the full health check
...
must be INERT. GH#52 found `chuzom-sse --help` reaching a live model call
