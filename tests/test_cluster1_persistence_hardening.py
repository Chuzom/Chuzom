...
session_store):
  ...
  * B-02/B-03 — the global `CHUZOM_PERSIST_TTL_DAYS` retention window
  ...
  * Safe-failure — if the redactor raises, the raw secret must never be
    ...
    is stored instead).
  * `CHUZOM_PERSIST_RAW=1` is an explicit opt-in escape hatch that disables
...
def _reset_persistence_config(monkeypatch):
    ...
    monkeypatch.delenv("CHUZOM_PERSIST_RAW", raising=False)
    monkeypatch.delenv("CHUZOM_PERSIST_REDACTION", raising=False)
    monkeypatch.delenv("CHUZOM_PERSIST_TTL_DAYS", raising=False)
...
def test_result_cache_redacts_secrets_on_write(tmp_path, monkeypatch):
    ...
    from chuzom import result_cache as rc
...
def test_result_cache_ttl_physically_deletes_expired_rows(tmp_path, monkeypatch):
    monkeypatch.setattr("chuzom.result_cache._ROUTER_DIR", tmp_path)
    monkeypatch.setenv("CHUZOM_PERSIST_TTL_DAYS", "30")
    import chuzom.config as cfg_mod
    cfg_mod._config = None
    from chuzom import result_cache as rc
...
def test_result_cache_persist_raw_opt_in_retains_verbatim(tmp_path, monkeypatch):
    monkeypatch.setattr("chuzom.result_cache._ROUTER_DIR", tmp_path)
    monkeypatch.setenv("CHUZOM_PERSIST_RAW", "1")
    import chuzom.config as cfg_mod
    cfg_mod._config = None
    from chuzom import result_cache as rc
...
def test_result_cache_safe_failure_never_persists_raw_on_redaction_error(tmp_path, monkeypatch):
    ...
    # persistence paths import from chuzom.persist_redaction, so patching the
    # shim attribute leaves the real function in place and the test asserts
    # nothing.
    monkeypatch.setattr("chuzom.persist_redaction.persist_redact", _boom)
