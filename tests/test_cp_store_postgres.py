...
class _FakeCursor:
    ...
    def execute(self, sql: str, params: tuple = ()):  # noqa: D401
        ...
        elif "from chuzom_cp_tenants where tenant_id" in low:
            self._result = [self._conn.tenant_row] if self._conn.tenant_row else []
        elif "from chuzom_cp_policy_versions where tenant_id" in low and "version" in low:
            self._result = [self._conn.version_row] if self._conn.version_row else []
        elif "join chuzom_cp_policy_versions" in low:
            self._result = [self._conn.version_row] if self._conn.active else []
        elif "from chuzom_cp_instances where tenant_id" in low:
            self._result = list(self._conn.instance_rows)
        elif "from chuzom_cp_instances where instance_id" in low:
...
def fake_pg(monkeypatch):
    ...
    monkeypatch.setenv("CHUZOM_CP_POSTGRES_DSN", "postgresql://fake/db")
    return fake_conn


def _store():
    from chuzom.control_plane.store_postgres import PostgresControlPlaneStore
    return PostgresControlPlaneStore()


def test_requires_dsn(monkeypatch, fake_pg):
    monkeypatch.delenv("CHUZOM_CP_POSTGRES_DSN", raising=False)
    from chuzom.control_plane.store_postgres import PostgresControlPlaneStore
...
def test_append_uses_max_version_then_insert(fake_pg):
    ...
    ins_idx = next(i for i, s in enumerate(sqls) if "insert into chuzom_cp_policy_versions" in s)
    assert max_idx < ins_idx
    # Confirm the computed version (3) is what got inserted (params of the insert).
    ins_params = next(p for s, p in fake_pg.calls if "insert into chuzom_cp_policy_versions" in s.lower())
...
def test_record_heartbeat_upserts_instance_and_history(fake_pg):
    ...
    assert "insert into chuzom_cp_instances" in sqls
    assert "insert into chuzom_cp_heartbeats" in sqls  # history appended too
