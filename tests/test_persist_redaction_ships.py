...
``persist_redact`` lived in ``chuzom/enterprise/redaction.py``. ``enterprise/``
...
does not carry those patterns. Measured against the PUBLISHED `chuzom-router`
package (installed from PyPI into a clean venv, `chuzom.enterprise` confirmed
...
The module moved to ``chuzom/persist_redaction.py``, in core, where it ships.
...
Point ``session_store._scrub_secrets`` back at ``chuzom.enterprise.redaction``
...
PERSISTENCE_MODULES = (
    "chuzom.result_cache",
    "chuzom.semantic_cache",
    "chuzom.idempotency",
    "chuzom.context",
    "chuzom.session_store",
)
...
def test_the_redactor_is_importable_without_enterprise():
    ...
    module = importlib.import_module("chuzom.persist_redaction")
    assert not module.__name__.startswith("chuzom.enterprise"), (
...
def test_every_persistence_path_uses_the_core_redactor():
    ...
    for name in PERSISTENCE_MODULES:
        ...
        if "from chuzom.enterprise.redaction import persist_redact" in src:
            offenders.append(name)
        elif "persist_redact" in src and "chuzom.persist_redaction" not in src:
...
def test_the_enterprise_shim_still_resolves():
    ...
    shim = importlib.import_module("chuzom.enterprise.redaction")
