...
WHY THIS EXISTS
---------------
`chuzom-auto-route.py` was measured taking 36.9 seconds, of which 36.26s was 8 sqlite
...
ACCOUNTING write ran first and blocked. Chuzom silently stopped routing on exactly the
...
def _shim_dir(tmp: Path) -> Path:
    ...
    A FIRST ATTEMPT shadowed the `chuzom` package on PYTHONPATH and rewrote its
    ...
    imports at startup before the hook runs. ``from chuzom.execution_ledger import ...``
    ...
    (shim / "sitecustomize.py").write_text(
        textwrap.dedent(
            ...
            _m = types.ModuleType("chuzom.execution_ledger")
            ...
            sys.modules["chuzom.execution_ledger"] = _m
            """
        )
    )
    return shim
...
def test_the_directive_is_emitted_even_when_the_ledger_write_hangs():
    ...
    """
    with tempfile.TemporaryDirectory(prefix="chuzom-emit-order-") as td:
        tmp = Path(td)
        shim = _shim_dir(tmp)
...
def test_the_hook_is_fast_when_the_ledger_is_not_stalled():
    ...
    """
    with tempfile.TemporaryDirectory(prefix="chuzom-emit-order-ok-") as td:
        tmp = Path(td)
