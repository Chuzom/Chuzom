...
def test_llm_delegate_call_clears_the_lock(tmp_path):
    ...
    _write_pending(tmp_path, sid, original_prompt=_OP_PROMPT)
    result = _run_hook(
        {"session_id": sid, "tool_name": "mcp__chuzom__llm_delegate",
...
def test_kill_switch_disables_redirect(tmp_path):
    """CHUZOM_DELEGATE=off falls back to the baseline route, no delegation."""
    sid = "sess-killswitch"
    _write_pending(tmp_path, sid, original_prompt=_OP_PROMPT)
    result = _run_hook(
        {"session_id": sid, "tool_name": "Bash",
         "tool_input": {"command": "python -m pytest -q"}},
        home=tmp_path, extra_env={"CHUZOM_ENFORCE": "hard", "CHUZOM_DELEGATE": "off"},
    )
