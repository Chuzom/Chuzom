...
output — so with ``CHUZOM_CAPABILITY_ROUTING=1`` the detector ran, decided, and
...
from chuzom.capabilities import detect_capabilities, serialize_capability_decision
...
class TestPersistence:
    ...
    def test_flag_controls_whether_anything_is_written(
        ...
        import chuzom.capabilities as caps

        if flag:
            monkeypatch.setenv("CHUZOM_CAPABILITY_ROUTING", flag)
        else:
            monkeypatch.delenv("CHUZOM_CAPABILITY_ROUTING", raising=False)
