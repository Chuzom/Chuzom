...
``chuzom/commands/install.py`` was found (and fixed) to force Codex CLI's
GLOBAL default ``model_provider`` to "chuzom" in ``~/.codex/config.toml``,
silently routing EVERY Codex call (interactive and Chuzom's own dispatch)
...
function in ``chuzom/commands/install.py``:
...
tool's own (non-Chuzom) behavior — or does it only ADD an inert MCP server
registration / hook entry / instructions file that requires the tool (or
user) to explicitly invoke Chuzom before it does anything?
...
are inert until the tool/user explicitly invokes the `chuzom` MCP server
...
def test_opencode_install_only_registers_mcp_server(monkeypatch, tmp_path):
    from chuzom.commands.install import _install_opencode_files
    ...
    assert data["mcpServers"]["chuzom"] == {"command": "chuzom", "args": []}
    assert any("chuzom MCP server" in a for a in actions)
...
def test_gemini_cli_install_settings_json_only_registers_mcp_server(monkeypatch, tmp_path):
    from chuzom.commands.install import _install_gemini_cli_files
...
def test_gemini_cli_install_hooks_json_only_adds_hook_entries(monkeypatch, tmp_path):
    ...
    own lifecycle events regardless of Chuzom, which is expected (that's
    ...
    hooks_json = tmp_path / ".gemini" / "extensions" / "chuzom" / "hooks" / "hooks.json"
...
def test_copilot_cli_install_only_registers_mcp_server(monkeypatch, tmp_path):
    from chuzom.commands.install import _install_copilot_cli_files
...
def test_openclaw_install_only_registers_mcp_server(monkeypatch, tmp_path):
    from chuzom.commands.install import _install_openclaw_files
...
def test_trae_install_only_registers_mcp_server(monkeypatch, tmp_path):
    from chuzom.commands.install import _install_trae_files
...
def test_factory_install_writes_no_config_files_at_all(monkeypatch, tmp_path):
    ...
    from chuzom.commands.install import _install_factory_files
...
def test_vscode_install_only_registers_mcp_server_under_servers_key(monkeypatch, tmp_path):
    ...
    from chuzom.commands.install import _install_vscode_files
    ...
    assert data["servers"]["chuzom"] == {"command": "chuzom", "args": []}
...
def test_cursor_install_only_registers_mcp_server(monkeypatch, tmp_path):
    from chuzom.commands.install import _install_cursor_files
    ...
    rules_md = tmp_path / ".cursor" / "rules" / "chuzom.md"
...
def test_no_other_host_writes_a_forced_default_scalar(
    ...
    import chuzom.commands.install as install_mod
