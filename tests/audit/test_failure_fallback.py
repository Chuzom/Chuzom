...
async def test_single_model_failure_falls_through_and_is_logged(routed_runtime):
    ...
    with (
        patch("chuzom.router._build_and_filter_chain", new_callable=AsyncMock, return_value=[
            "openai/gpt-4o",
            "gemini/gemini-2.5-flash",
        ]),
        patch("chuzom.router.providers.call_llm", new_callable=AsyncMock, side_effect=call_llm),
    ):
...
async def test_all_models_failing_raises_clear_terminal_error_without_hanging(
    ...
    with (
        ...
        patch("chuzom.router.providers.call_llm", new_callable=AsyncMock, side_effect=fail_all),
    ):
