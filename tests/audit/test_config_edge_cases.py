"""SECTION 7 — Configuration edge cases for chuzom.repo_config.

Covers three areas from the task brief:
  1. No ~/.chuzom/routing.yaml at all -> effective_config() must return sane
  ...
  2. Malformed YAML -> `_parse_yaml` swallows ANY exception and returns `{}`
     ...
     tab character, a bad indent) with zero feedback — `chuzom config lint`
...
def test_load_user_config_with_malformed_yaml_falls_back_to_defaults(tmp_path, monkeypatch):
    ...
    (home / ".chuzom" / "routing.yaml").write_text("profile: [unterminated\n")
