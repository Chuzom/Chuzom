...
async def test_concurrent_calls_cannot_exceed_monthly_budget_cap(
    ...
    async def _noop_store_receipt(*a, **k):
        return None
    monkeypatch.setattr("chuzom.receipt_store.store_receipt", _noop_store_receipt)
    ...
    # operator's real ~/.chuzom/session_spend.json.
    import chuzom.session_spend as session_spend_module
    ...
    monkeypatch.setenv("CHUZOM_MONTHLY_BUDGET", str(monthly_budget))
...
# because a subprocess reimporting chuzom.config/chuzom.codex_agent from
