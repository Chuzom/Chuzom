...
2. The dashboard token-axis formatter (``chuzom.ui.session_summary``) must
...
3. Session/lifetime cost totals surfaced to the user (the ``llm_savings``
   MCP tool, backed by ``chuzom.commands.gain.SavingsAnalytics``) must
...
production ``~/.chuzom`` state is touched. Per the known HOME-isolation gap
...
async def test_log_usage_reflects_actually_executed_model_not_failed_candidate(
    ...
    # ~/.chuzom/routing.yaml (user-level model pin) and a real
    # ~/.chuzom/.env (sets OLLAMA_BASE_URL) that mock_env does not
    # isolate from. chuzom.repo_config.effective_config() and
    # chuzom.config's dotenv loader both resolve Path.home() at CALL time
    ...
    monkeypatch.setattr("chuzom.config.RouterConfig.all_ollama_models", lambda self: [])
    monkeypatch.setattr("chuzom.dynamic_routing.get_dynamic_model_chain", lambda *a, **k: None)
    ...
    tracker = get_tracker()
    monkeypatch.setattr("chuzom.router.get_tracker", lambda: tracker)
    ...
    monkeypatch.setattr("chuzom.providers.call_llm", _fake_call_llm)
    monkeypatch.setattr("chuzom.router.cost.log_usage", _spy_log_usage)
...
from chuzom.ui.session_summary import _fmt_tok_axis, _tok_axis_unit  # noqa: E402
...
async def test_savings_analytics_reports_real_decisions_not_zero(temp_db, mock_env):
    ...
    ``chuzom.commands.gain.SavingsAnalytics.get_routing_decisions`` (the
    ...
    schema (``chuzom.cost.CREATE_ROUTING_DECISIONS_TABLE`` defines
    ...
    config = get_config()
    analytics = SavingsAnalytics(db_path=pathlib.Path(config.chuzom_db_path))
    ...
    assert savings["total_decisions"] == 1, (
        ...
        "chuzom/commands/gain.py get_routing_decisions() vs. the actual "
        "schema in chuzom/cost.py CREATE_ROUTING_DECISIONS_TABLE."
    )
