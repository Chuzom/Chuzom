...
from chuzom.repo_config import RepoConfig, TaskRouteOverride
from chuzom.router import AGENTIC_TASK_TYPES, MEDIA_TASK_TYPES, _build_and_filter_chain
from chuzom.types import Complexity, RoutingProfile, TaskType


# ── Fake config double ──────────────────────────────────────────────────────
# Mirrors the shape of chuzom.config.RouterConfig that _build_and_filter_chain
# actually reads, without touching real env vars / ~/.chuzom files so results
...
class _FakeConfig:
    def __init__(
        ...
        *,
        ...
        self.chuzom_gemini_subscription = gemini_subscription
        self.chuzom_claw_code = claw_code
        self.chuzom_routing_policy = routing_policy
        self.chuzom_agentic_model = agentic_model
...
def isolate_chain_build(monkeypatch):
    ...
    monkeypatch.setattr("chuzom.policy.load_org_policy", lambda *a, **k: None)
    ...
    monkeypatch.setattr("chuzom.router.get_active_agent", lambda: None)
    monkeypatch.setattr(
        "chuzom.quota_balance.get_provider_pressures",
...
async def test_explicit_pin_wins_over_ollama_codex_claude_and_every_reorder(monkeypatch):
    ...
    monkeypatch.setattr("chuzom.router.get_active_agent", lambda: "claude_code")
    monkeypatch.setattr(
