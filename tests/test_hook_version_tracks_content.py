"""`chuzom-hook-version` must be bumped whenever the hook's behaviour changes.
...
auto-route.py, then re-record with:
    ...
    python -c "import hashlib,pathlib; \\
      p=pathlib.Path('src/chuzom/hooks/auto-route.py'); \\
...
_HOOK = _ROOT / "src" / "chuzom" / "hooks" / "auto-route.py"
_RECORD = _ROOT / ".chuzom" / "zero-tolerance-audit" / "hook_version_baseline.json"


def _stamps(text: str) -> list[int]:
    return [int(m) for m in re.findall(r"chuzom-hook-version:\s*(\d+)", text)]
...
def test_the_version_was_bumped_with_the_content():
    ...
    stamps = _stamps(text)
    assert stamps, "the hook has no chuzom-hook-version stamp at all"
    current = stamps[0]
    ...
    assert current > rec["version"], (
        f"auto-route.py changed but chuzom-hook-version is still {current}.\n"
