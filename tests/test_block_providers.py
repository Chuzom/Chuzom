"""Gate-17: CHUZOM_BLOCK_PROVIDERS — a hard provider block on EVERY routing path.

Context: `CHUZOM_DISABLE_SUBPROCESS_BACKENDS` disables only a provider's LOCAL
...
`CHUZOM_BLOCK_PROVIDERS` closes that: a blocked provider is removed from the chain
...
async def _chain(monkeypatch, *, block=None, disable=None, broker=frozenset()):
    ...
    if block is not None:
        monkeypatch.setenv("CHUZOM_BLOCK_PROVIDERS", block)
    if disable is not None:
        monkeypatch.setenv("CHUZOM_DISABLE_SUBPROCESS_BACKENDS", disable)
...
def test_blocked_providers_parses_env(monkeypatch):
    monkeypatch.setenv("CHUZOM_BLOCK_PROVIDERS", "codex, Gemini_CLI ,")
    assert _blocked_providers() == frozenset({"codex", "gemini_cli"})
    monkeypatch.delenv("CHUZOM_BLOCK_PROVIDERS", raising=False)
...
async def test_blocked_codex_absent_even_when_broker_offers_it(mock_env, monkeypatch):
    ...
    Fail-before: no code read CHUZOM_BLOCK_PROVIDERS, so the broker re-assert
