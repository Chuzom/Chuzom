"""Tests for the Claude Code CLI offload adapter (chuzom.claude_agent)."""
from types import SimpleNamespace
from unittest.mock import MagicMock, patch

import pytest

from chuzom import claude_agent
from chuzom.claude_agent import (
    ClaudeResult,
    _model_alias,
    is_claude_available,
    offload_available,
...
async def test_run_claude_binary_not_found():
    with patch("chuzom.claude_agent.find_claude_binary", return_value=None):
...
async def test_run_claude_success_and_argv():
    ...
    with (
        patch("chuzom.claude_agent.find_claude_binary", return_value="/usr/bin/claude"),
        patch("chuzom.claude_agent.asyncio.create_subprocess_exec", side_effect=_fake_exec),
...
def _cfg(sub=True, cap=0.80):
    return SimpleNamespace(
        chuzom_claude_subscription=sub, chuzom_claude_offload_max_pressure=cap
    )
...
def test_offload_blocked_when_pressure_over_cap(monkeypatch):
    monkeypatch.setattr(claude_agent, "is_claude_available", lambda: True)
    monkeypatch.setattr("chuzom.claude_usage.get_claude_pressure", lambda: 0.90)
    assert offload_available(_cfg(sub=True, cap=0.80)) is False


def test_offload_allowed_when_headroom(monkeypatch):
    monkeypatch.setattr(claude_agent, "is_claude_available", lambda: True)
    monkeypatch.setattr("chuzom.claude_usage.get_claude_pressure", lambda: 0.40)
