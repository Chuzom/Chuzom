"""#21 — every test that builds context must isolate the live ~/.chuzom.
...
docstring states the hazard:
    ...
    "Without isolating HOME the test read the developer's LIVE session
     accumulator (populated by the active chuzom hooks) and non-deterministically
...
def test_every_context_building_test_isolates_home():
    ...
    assert not missing, (
        f"these tests read the LIVE ~/.chuzom accumulator because they do not "
