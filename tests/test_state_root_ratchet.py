...
Audit #37. `chuzom.paths` is the one module that honours `CHUZOM_HOME`; 182 places in
`src/chuzom/` resolve `~/.chuzom` themselves and never ask it. `usage.db` alone is reached
~23 different ways.
...
_BASELINE = _ROOT / ".chuzom" / "zero-tolerance-audit" / "state_root_baseline.json"
...
class TestTheRatchetCanActuallyFail:
    ...
    def test_it_FAILS_when_a_new_direct_resolution_appears(self, tmp_path):
        ...
        does `Path.home() / ".chuzom"` into src/chuzom moved it 182 -> 183 and exit 1."""
        g = _gate()
        canary = tmp_path / "chuzom"
        canary.mkdir()
        (canary / "_canary.py").write_text(
            'from pathlib import Path\nX = Path.home() / ".chuzom" / "canary.db"\n'
        )
        g.SRC = canary
        g.REPO = tmp_path
        found = g.scan()
    ...
    def test_the_expanduser_spelling_is_caught_too(self, tmp_path):
        ...
        canary.mkdir()
        (canary / "_canary.py").write_text(
            'import os\nX = os.path.expanduser("~/.chuzom/usage.db")\n'
        )
        g.SRC = canary
        g.REPO = tmp_path
    ...
    def test_another_tools_directory_is_NOT_counted(self, tmp_path):
        ...
        those because chuzom was sandboxed would be a NEW defect, not a fix."""
        g = _gate()
        canary = tmp_path / "chuzom"
        canary.mkdir()
        (canary / "_canary.py").write_text(
...
class TestIsIsolatedDoesNotOverClaim:
    def test_it_only_promises_what_paths_resolves(self, monkeypatch, tmp_path):
        ...
        assert paths.chuzom_home() == tmp_path
