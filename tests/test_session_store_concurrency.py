...
def _worker(
    ...
    Runs in a separate OS process. Sets HOME/CHUZOM_PROJECT_ID explicitly
    (rather than relying on inheritance) so the test is correct under both
    fork and spawn start methods.
    """
    os.environ["HOME"] = home
    os.environ["CHUZOM_PROJECT_ID"] = project_id
    os.environ.pop("CHUZOM_SESSION_CONTEXT", None)
...
def test_session_store_concurrency_zero_lost_writes():
    ...
    with tempfile.TemporaryDirectory(prefix="chuzom-c01-") as tmp:
        tmp_path = Path(tmp)
