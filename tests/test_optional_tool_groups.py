...
``server.py`` imported ``chuzom.tools.agoragentic`` at module level, in the same
...
``chuzom.server`` unreachable, because it walks module-level imports and one of
...
_SERVER = Path(__file__).resolve().parents[1] / "src" / "chuzom" / "server.py"
...
def test_optional_group_is_not_in_the_mandatory_import(name: str):
    """It must not ride along in the unguarded `from chuzom.tools import …`."""
    for node in _server_tree().body:
        if isinstance(node, ast.ImportFrom) and node.module == "chuzom.tools":
            imported = {a.name for a in node.names}
            assert name not in imported, (
                f"{name!r} is in the mandatory `from chuzom.tools import …` "
...
def test_optional_group_is_imported_defensively(name: str):
    ...
    for node in ast.walk(_server_tree()):
        ...
        for stmt in ast.walk(node):
            if isinstance(stmt, ast.ImportFrom) and stmt.module == "chuzom.tools":
...
def test_the_server_still_imports_with_the_group_absent(monkeypatch):
    ...
    checked for the name ``chuzom.tools.agoragentic`` — which never appears,
    because ``from chuzom.tools import agoragentic`` calls
    ``__import__("chuzom.tools", …, fromlist=["agoragentic"])``. The blocker
    ...
    level ``mcp`` object. The in-process version re-imported ``chuzom.server``
    ...
    program = textwrap.dedent(
        ...
        class Block:
            def find_spec(self, name, path=None, target=None):
                if name == "chuzom.tools.agoragentic":
                    raise ImportError("simulated: excluded from this build")
                return None

        sys.meta_path.insert(0, Block())
        import chuzom.server  # noqa: F401
        print("OK")
        """
    )
...
def test_mandatory_groups_are_still_mandatory():
    ...
    """
    src = inspect.getsource(__import__("chuzom.server", fromlist=["x"]))
