...
Chuzom-routed calls) can route heavy models through a local pxpipe proxy:

  1. _pxpipe_config() — stdlib-only env var parsing (hooks avoid importing
     the full chuzom.config module).
...
HOOK_PATH = Path(__file__).parent.parent / "src" / "chuzom" / "hooks" / "session-start.py"
...
def _load_hook_module():
    ...
    ~/.chuzom/.env at import time, which mutates os.environ; snapshot and
...
class TestPxpipeConfig:
    def test_disabled_by_default(self, monkeypatch):
        monkeypatch.delenv("CHUZOM_PXPIPE_ENABLED", raising=False)
    ...
    def test_truthy_values_enable(self, monkeypatch, value):
        monkeypatch.setenv("CHUZOM_PXPIPE_ENABLED", value)
        mod = _load_hook_module()
        enabled, _url, _models = mod._pxpipe_config()
        assert enabled is True

    def test_custom_url_and_models_respected(self, monkeypatch):
        monkeypatch.setenv("CHUZOM_PXPIPE_ENABLED", "true")
        monkeypatch.setenv("CHUZOM_PXPIPE_URL", "http://127.0.0.1:9999/")
        monkeypatch.setenv("CHUZOM_PXPIPE_HEAVY_MODELS", "claude-opus-4-8,gpt-5.6")
        mod = _load_hook_module()
        enabled, url, models = mod._pxpipe_config()
        assert enabled is True
...
class TestSyncAnthropicBaseUrl:
    ...
    def _run_sync(self, monkeypatch, tmp_path, *, enabled, reachable, url="http://127.0.0.1:47821"):
        ...
        monkeypatch.setenv("CHUZOM_PXPIPE_ENABLED", "true" if enabled else "false")
        monkeypatch.setenv("CHUZOM_PXPIPE_URL", url)
        mod = _load_hook_module()
