...
re-injection smuggles a blocked provider past the user's `.chuzom.yml` block rule.
...
def _isolate_dynamic(monkeypatch, *, block: list[str]) -> None:
    ...
    - no explicit env/repo agentic pin (caller leaves cfg.chuzom_agentic_model empty
    ...
    monkeypatch.setattr(
        "chuzom.config.RouterConfig.all_ollama_models", lambda self: [DYN]
    )
    monkeypatch.setattr(
        "chuzom.agentic_registry.best_agentic_model", lambda: DYN
    )
...
async def test_explicit_agentic_pin_survives_block(monkeypatch):
    ...
    _isolate_dynamic(monkeypatch, block=["ollama"])
    cfg = RouterConfig()
    cfg.chuzom_agentic_model = DYN  # explicit pin, even though ollama is blocked
    chain = await _chain(cfg)
    assert chain and chain[0] == DYN, (
