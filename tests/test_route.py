...
async def test_route_simple_goes_to_budget(mock_env, mock_ctx):
    ...
    with patch("chuzom.tools.routing.classify_for_routing", new_callable=AsyncMock, return_value=classification), \
...
async def test_route_with_complexity_override(mock_env, mock_ctx):
    ...
    with patch("chuzom.tools.routing.classify_for_routing") as mock_classify, \
         patch("chuzom.tools.routing.route_and_call", new_callable=AsyncMock, return_value=response) as mock_route:
...
async def test_route_classifier_failure_falls_back(mock_env, mock_ctx):
    ...
    with patch("chuzom.tools.routing.classify_for_routing", new_callable=AsyncMock, side_effect=RuntimeError("boom")), \
         patch("chuzom.tools.routing.route_and_call", new_callable=AsyncMock, return_value=response) as mock_route:
