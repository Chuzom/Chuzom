...
writing. During the audit a test that believed `CHUZOM_HOME` sandboxed it wrote
to the operator's real `~/.chuzom/usage.db` and destroyed live data. The
...
def isolated_home(tmp_path, monkeypatch):
    """Redirect chuzom state into tmp_path and PROVE it landed there."""
    from chuzom import paths

    home = tmp_path / "chuzom-home"
    home.mkdir()
    monkeypatch.setenv("CHUZOM_HOME", str(home))

    assert paths.is_isolated(), "CHUZOM_HOME did not take effect"
...
def test_a_failed_write_is_counted_not_silent(isolated_home, monkeypatch):
    ...
    """
    import chuzom.execution_ledger as el
...
def test_no_record_event_call_site_discards_the_boolean():
    ...
    for path in src.rglob("*.py"):
        ...
        if "from chuzom.execution_ledger import" not in text:
            continue
...
def test_lock_timeout_declines_rather_than_running_unlocked(tmp_path, monkeypatch):
    ...
    import chuzom.session_store as ss
