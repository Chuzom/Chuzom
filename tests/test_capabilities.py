...
@pytest.mark.parametrize("prompt, task_type, expect_needs_tools, expect_bits", [
    ...
    ("Show me src/chuzom/routing_quality.py", "query", True, {"read_files"}),
...
def test_needs_claude_tools_flag_on_uses_rich_vector(monkeypatch):
    monkeypatch.setenv("CHUZOM_CAPABILITY_ROUTING", "1")
    from chuzom.hooks.chain_builder import needs_claude_tools
...
def _git_repo(tmp_path: Path) -> Path:
    root = tmp_path / "repo"
    (root / "src" / "chuzom").mkdir(parents=True)
    target = root / "src" / "chuzom" / "widget.py"
...
def test_serialize_relevant_context_is_bounded(tmp_path):
    root = _git_repo(tmp_path)
    rc = collect_relevant_context("look at `record_delegation` in src/chuzom/widget.py", root)
    assert rc is not None
