...
Three independent failures in one line:
  ...
  * `chuzom` is not the PyPI name — that is `chuzom-router`. Not a
  ...
  * Unquoted square brackets are a glob pattern in zsh: the command fails
    outright with "no matches found: chuzom[tui]".
  * `chuzom-router` is normally pipx-installed into its own venv. A bare `pip
    install` — even with the name fixed — installs into whatever Python is
    active in the shell, not the venv `chuzom` actually runs from, so it
    would not fix anything.

The working command, confirmed by the reporter: `pipx inject chuzom-router textual`.
"""
...
_DASHBOARD = _REPO / "src" / "chuzom" / "commands" / "dashboard.py"
...
def test_hint_does_not_name_a_nonexistent_package():
    """The distribution is `chuzom-router`; `chuzom` is the import name."""
    for ln in _hint_lines():
        assert "chuzom[" not in ln, (
...
def test_hint_uses_the_real_distribution_name():
    ...
    assert dist == "chuzom-router", "distribution renamed — update this hint and test"
...
def test_hint_targets_the_venv_the_command_runs_from():
    ...
    assert "pipx inject" in joined, (
        f"hint must use `pipx inject`, which installs into the venv `chuzom` "
