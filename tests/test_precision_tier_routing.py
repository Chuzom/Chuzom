...
from chuzom.router import _needs_precise_answer, route_and_call
...
def _pin_chain(models):
    return patch("chuzom.router._build_and_filter_chain", new=AsyncMock(return_value=list(models)))
...
async def test_precision_prompt_fronts_metered_mini(temp_db, mock_env, mock_acompletion, monkeypatch):
    ...
    with _pin_chain(["ollama/qwen2.5-coder:7b", "ollama/qwen3-coder:30b"]), \
         patch("chuzom.router._call_text", new_callable=AsyncMock) as mock_call:
...
async def test_precision_prompt_stays_local_when_openai_blocked(temp_db, mock_env, mock_acompletion, monkeypatch):
    ...
    CHUZOM_BLOCK_PROVIDERS) — falls back to the local chain."""
    monkeypatch.setenv("OPENAI_API_KEY", "sk-x")
    monkeypatch.setenv("CHUZOM_BLOCK_PROVIDERS", "openai")
