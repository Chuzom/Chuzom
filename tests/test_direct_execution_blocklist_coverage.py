...
`CHUZOM_DIRECT_EXECUTION` defaults to on. With it enabled, a local model is handed
...
_AGENT_LOOP = Path(__file__).resolve().parents[1] / "src" / "chuzom" / "hooks" / "agent_loop.py"
...
def test_documented_gaps_are_still_the_gaps(cmd: str, note: str):
    ...
    assert not _blocked_re().search(cmd), (
        ...
        f"'CHUZOM_DIRECT_EXECUTION — what it actually grants' still lists it as "
