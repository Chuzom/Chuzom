"""P7 — delegation savings telemetry into chuzom's savings_stats ledger."""
from __future__ import annotations

import sqlite3

from chuzom.agentic.telemetry import (
...
async def test_default_recorder_writes_savings_stats_row(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("CHUZOM_DB_PATH", str(db))
    await record_delegation_savings(_RESULT, model="chuzom-agentic-router", session_id="sess")
    rows = sqlite3.connect(str(db)).execute(
        "SELECT estimated_claude_cost_saved, external_cost, model_used, session_id "
        "FROM savings_stats"
    ).fetchall()
    assert rows == [(0.6, 0.0, "chuzom-agentic-router", "sess")]
