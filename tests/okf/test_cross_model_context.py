...
the North Star's central claim: chuzom routes a prompt to a cheap local model,
...
def store(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("CHUZOM_OKF", "1")
...
def test_the_store_lives_outside_the_users_repo(store, monkeypatch, tmp_path):
    ...
    Nothing chuzom infers should land in someone's history by accident.
    """
    from chuzom import okf
