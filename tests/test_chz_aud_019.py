...
Covers the following hooks that previously had zero test coverage:
  - src/chuzom/hooks/response-router.py
  - src/chuzom/hooks/gemini-cli-auto-route.py
  - src/chuzom/hooks/gemini-cli-post-tool.py
  - src/chuzom/hooks/gemini-cli-session-end.py
  - src/chuzom/hooks/bash-compress.py
...
HOOKS_DIR = Path(__file__).resolve().parents[1] / "src" / "chuzom" / "hooks"
...
def _run(script: Path, stdin_data: str = "", extra_env: dict | None = None, tmp_path: Path | None = None) -> subprocess.CompletedProcess:
    ...
    env = os.environ.copy()
    # Isolate from real ~/.chuzom so hooks don't read/write production state
...
class TestResponseRouterHook:
    ...
    through cheaper models. Tests that the hook:
      ...
      - respects the CHUZOM_RESPONSE_ROUTER=off opt-out
    """
    ...
    def test_passes_through_response_unchanged(self, tmp_path):
        ...
        (tmp_path / ".chuzom" / "usage.json").write_text(json.dumps(usage))
    ...
    def test_opt_out_env_var_disables_routing(self, tmp_path):
        """CHUZOM_RESPONSE_ROUTER=off must bypass routing and pass through verbatim."""
        ...
        result = _run(
            RESPONSE_ROUTER,
            stdin_data=sample,
            extra_env={"CHUZOM_RESPONSE_ROUTER": "off"},
            tmp_path=tmp_path,
        )
        ...
        assert sample in result.stdout, (
            "CHUZOM_RESPONSE_ROUTER=off must pass response through verbatim"
        )
...
class TestGeminiCliPostToolHook:
    ...
    def test_flushes_pending_savings_to_log(self, tmp_path):
        ...
        (chuzom_dir / "gemini_session.json").write_text(json.dumps(session_data))
        ...
        savings_log = chuzom_dir / "savings_log.jsonl"
        ...
        session_after = json.loads((chuzom_dir / "gemini_session.json").read_text())
...
class TestGeminiCliSessionEndHook:
    ...
    Tests that the hook:
      ...
      - does not crash when chuzom database is absent
    """
    ...
    def test_does_not_crash_without_database(self, tmp_path):
        """Without a chuzom DB, hook must degrade gracefully (partial summary is OK)."""
        event = {}
        result = _run(GEMINI_CLI_SESSION_END, stdin_data=json.dumps(event), tmp_path=tmp_path)
...
class TestBashCompressHook:
    ...
    Tests that the hook:
      - exits 0 silently when compression is disabled (CHUZOM_BASH_COMPRESS=off)
    ...
    def test_opt_out_env_var_exits_silently(self, tmp_path):
        """CHUZOM_BASH_COMPRESS=off must exit 0 with no output."""
        payload = self._make_bash_payload("ls -la", "file1\nfile2\nfile3\n")
        result = _run(
            BASH_COMPRESS,
            stdin_data=payload,
            extra_env={"CHUZOM_BASH_COMPRESS": "off"},
            tmp_path=tmp_path,
        )
        assert result.returncode == 0, (
            f"bash-compress.py exited {result.returncode} with CHUZOM_BASH_COMPRESS=off; "
            f"stderr: {result.stderr[:200]}"
        )
