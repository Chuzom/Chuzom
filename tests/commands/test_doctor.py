...
class TestRunDoctorHost:
    ...
    def test_run_doctor_host_codex_gateway_reachable(self, capsys, tmp_path, monkeypatch):
        """Codex doctor proves the chuzom provider is registered and reachable
        ...
        monkeypatch.setattr("chuzom.commands.doctor.urllib.request.urlopen", _fake_urlopen)
        _run_doctor_host("codex")
        output = capsys.readouterr().out
        assert "Codex using its own default model provider" in output
        assert "Chuzom model provider registered" in output
    ...
    def test_run_doctor_host_codex_reports_forced_default_as_broken(self, capsys, tmp_path, monkeypatch):
        """A config with model_provider forced to chuzom (left over from an
        older chuzom install) must be flagged as broken, not healthy — it
        ...
        _run_doctor_host("codex")
        output = capsys.readouterr().out
        assert "force-set to 'chuzom'" in output
        assert "chuzom install --host codex --mode gateway" in output
    ...
    def test_run_doctor_host_codex_reports_not_gateway(self, capsys, tmp_path, monkeypatch):
        ...
        output = capsys.readouterr().out
        assert "Codex using its own default model provider" in output
        assert "Chuzom model provider table missing" in output
        assert "chuzom install --host codex --mode gateway" in output
