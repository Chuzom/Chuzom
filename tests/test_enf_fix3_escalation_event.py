...
def _ledger_db(home) -> Path:
    return home / ".chuzom" / "usage.db"


def _run(payload, home):
    e = {k: v for k, v in os.environ.items() if k not in ("CHUZOM_ENFORCE", "CHUZOM_SLIM")}
    e["HOME"] = str(home)
    e["CHUZOM_ENFORCE"] = "hard"
    # Pin the ledger db to this test's home so a session-level
    # CHUZOM_EXECUTION_LEDGER_DB / CHUZOM_DB_PATH can't redirect the write.
    e["CHUZOM_EXECUTION_LEDGER_DB"] = str(_ledger_db(home))
    e.pop("CHUZOM_DB_PATH", None)
...
def test_calling_the_routed_door_does_not_record_an_escalation(tmp_path):
    ...
    sid = "esc2"
    _pending(tmp_path, sid)
    r = _run({"session_id": sid, "tool_name": "mcp__chuzom__llm", "tool_input": {"prompt": "x"}}, tmp_path)
