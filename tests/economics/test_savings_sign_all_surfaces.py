...
structural". Eleven point-fixes would have been the fifth. `chuzom.savings.net_saved`
...
def test_chz_ss_01_actually_detects_a_clamp(tmp_path):
    ...
    (work / "src" / "chuzom" / "tools").mkdir(parents=True)
    shutil.copy(_ROOT / "scripts" / "lint_savings_sign.py", work / "scripts")
    for rel in (
        "src/chuzom/cost.py", "src/chuzom/router.py", "src/chuzom/digest.py",
        "src/chuzom/retrospective.py", "src/chuzom/route_server.py",
        "src/chuzom/tools/dashboard.py",
    ):
        shutil.copy(_ROOT / rel, work / rel)

    assert _run_lint(work) == 0, "the copied subset should start clean"

    target = work / "src" / "chuzom" / "digest.py"
...
def test_the_lint_covers_the_surfaces_this_finding_named():
    ...
    for rel in (
        ...
        "src/chuzom/digest.py",
        "src/chuzom/retrospective.py",
...
def test_the_only_exemption_is_the_documented_gross_metric():
    ...
    assert rel == "src/chuzom/execution_ledger.py"
    low = reason.lower()
...
def test_dashboard_renders_a_loss_with_its_sign_and_not_in_green():
    ...
    """
    src = (_ROOT / "src" / "chuzom" / "tools" / "dashboard.py").read_text()
