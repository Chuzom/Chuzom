...
``chuzom.pricing``. This file asserts the property that module cannot assert
...
from chuzom import benchmarks, calibration, cost, pricing, types
from chuzom.hooks import savings_logger
...
PRICE_CONSUMING_MODULES = [
    "chuzom/cost.py",
    "chuzom/types.py",
    "chuzom/calibration.py",
    "chuzom/benchmarks.py",
    "chuzom/model_registry.py",
    "chuzom/tools/text.py",
    "chuzom/hooks/savings_logger.py",
    "chuzom/hooks/cc-usage-track.py",
    "chuzom/hooks/usage-refresh.py",
]


def _imports_canonical_pricing(path: Path) -> bool:
    """True when ``path`` imports ``chuzom.pricing`` at any scope.
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.ImportFrom):
            if node.module in ("chuzom", "chuzom.pricing") and (
                node.module == "chuzom.pricing"
            ...
            if any(a.name == "chuzom.pricing" for a in node.names):
                return True
    return False
...
def test_price_consuming_module_imports_the_canonical_source(rel: str) -> None:
    ...
    assert _imports_canonical_pricing(path), (
        f"{rel} consumes model prices but does not import chuzom.pricing. "
...
def _every_table_rate_for(model: str) -> dict[str, tuple[float, float]]:
    ...
    cc_usage = _load_script_module("_cc_usage_track", "chuzom/hooks/cc-usage-track.py")
    canonical = pricing.price_for(model)
    found: dict[str, tuple[float, float]] = {
        "chuzom.pricing": (canonical.input, canonical.output),
    }
