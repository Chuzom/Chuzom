...
from chuzom.provenance import Measured, total
...
async def test_a_failed_query_is_distinguishable_from_a_genuine_zero(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("CHUZOM_DB_PATH", str(tmp_path / "usage.db"))
...
def test_the_weekly_quota_value_is_not_a_hardcoded_fifty(monkeypatch):
    ...
    """
    from chuzom.quota_savings import _default_weekly_quota_usd

    monkeypatch.delenv("CHUZOM_WEEKLY_QUOTA_USD", raising=False)
    monkeypatch.delenv("CHUZOM_SUBSCRIPTION_USD_PER_MONTH", raising=False)
...
def test_the_quota_value_follows_the_users_plan(monkeypatch):
    ...
    monkeypatch.delenv("CHUZOM_WEEKLY_QUOTA_USD", raising=False)
    monkeypatch.setenv("CHUZOM_SUBSCRIPTION_USD_PER_MONTH", "100")
    assert _default_weekly_quota_usd() == pytest.approx(100.0 / 4.345, abs=0.01)

    monkeypatch.setenv("CHUZOM_WEEKLY_QUOTA_USD", "12.5")
...
def test_quota_savings_are_labelled_as_quota_not_cash():
    ...
    """
    import chuzom.quota_savings as qs
