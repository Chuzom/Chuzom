...
RED2-02 is "failure reads as zero". The remediation taught `chuzom.provenance`
...
async def test_unreadable_ledger_reports_unknown_not_zero(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("CHUZOM_DB_PATH", str(tmp_path / "broken.db"))
...
def test_broadcast_line_never_shows_a_dollar_figure_when_unknown():
    from chuzom.team import _savings_label
