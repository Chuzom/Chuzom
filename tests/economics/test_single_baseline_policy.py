...
WITHOUT Chuzom is their subscription's top model, not a cheaper Claude they
...
_SRC = Path(__file__).resolve().parent.parent.parent / "src" / "chuzom"
...
def test_env_override_still_resolves_through_the_policy(monkeypatch):
    ...
    monkeypatch.setenv("CHUZOM_SAVINGS_BASELINE", "claude-sonnet-5")
...
def test_unknown_override_falls_back_rather_than_pricing_at_zero(monkeypatch):
    ...
    monkeypatch.setenv("CHUZOM_SAVINGS_BASELINE", "not-a-real-model")
    assert pricing.savings_baseline_model() == "claude-opus-5"
...
def test_dashboard_delegates_to_the_policy():
    from chuzom import dashboard_data
...
_NON_SAVINGS_BASELINES = {
    ...
    # Benchmark comparator for `chuzom test`; a fixed reference model so scores
...
def _baseline_assignments() -> list[tuple[str, str, int, str]]:
    """Every `<NAME containing BASELINE> = "<a claude model>"` under src/chuzom.
