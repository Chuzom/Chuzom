"""SECURITY.md's `CHUZOM_DIRECT_EXECUTION` coverage table must stay true.

WHY THIS EXISTS
===============

Ported from the downstream package, where this was reported as issue #36.

SECURITY.md did not document that `CHUZOM_DIRECT_EXECUTION` is
...
the prompt. Upstream already carries the section; what it did not carry was a test. The section now exists in both, with a table of twelve representative
...
_AGENT_LOOP = _REPO / "src" / "chuzom" / "hooks" / "agent_loop.py"
...
def test_the_section_exists():
    ...
    assert "## CHUZOM_DIRECT_EXECUTION" in text, (
