"""PostToolUse hook — src/chuzom/hooks/context-capture.py.
...
HOOK = Path(__file__).resolve().parents[1] / "src" / "chuzom" / "hooks" / "context-capture.py"
...
def test_sentinel_open_in_result_is_skipped(cc, monkeypatch):
    import chuzom.session_store as real_session_store
    from chuzom.session_store import SENTINEL_OPEN
...
def test_successful_record_event_call_shape(cc, monkeypatch):
    ...
    _run(
        ...
        {
            "tool_name": "Read",
            "tool_input": {"file_path": "/Users/yaliandrona/Projects/Chuzom/README.md"},
