...
gated on CHUZOM_DYNAMIC_LEADERBOARD_ORDERING and DEFAULT OFF so the audited/QUALIFIED
...
from chuzom.dynamic_routing import build_dynamic_routing_table
from chuzom.profiles import ROUTING_TABLE, RoutingProfile
...
def test_default_off_is_a_noop(monkeypatch):
    ...
    monkeypatch.delenv("CHUZOM_DYNAMIC_LEADERBOARD_ORDERING", raising=False)
...
def test_flag_on_reorders_nonbudget_not_budget(monkeypatch):
    ...
    monkeypatch.setenv("CHUZOM_DYNAMIC_LEADERBOARD_ORDERING", "on")
