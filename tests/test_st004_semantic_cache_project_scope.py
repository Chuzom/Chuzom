...
from chuzom.semantic_cache import CREATE_SEMANTIC_CACHE_TABLE
...
def wired(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("chuzom.config.get_config", lambda: _Cfg())

    async def _fresh_db():
        return await aiosqlite.connect(str(db_file))

    monkeypatch.setattr("chuzom.cost._get_db", _fresh_db)
    return db_file


def _set_project(monkeypatch, path: str) -> None:
    monkeypatch.setenv("CHUZOM_PROJECT_DIR", path)
