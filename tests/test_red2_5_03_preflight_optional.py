...
def _load():
    spec = importlib.util.spec_from_file_location("chuzom_session_start_r2503", HOOK_PATH)
...
def _clear_keys(monkeypatch):
    for k in ("OPENAI_API_KEY", "GEMINI_API_KEY", "ANTHROPIC_API_KEY", "CHUZOM_ENFORCE"):
...
def test_enforce_hard_is_a_heads_up_not_a_fix(monkeypatch):
    ...
    out = mod._preflight_check()
    assert FORBIDDEN_IMPERATIVE not in out
    assert "CHUZOM_ENFORCE=hard" in out
