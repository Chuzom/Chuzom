...
The orphaned ~/.claude/rules/llm-router.md (from Chuzom's pre-rebrand identity)
declares routing a HARD CONSTRAINT, contradicting the advise-mode chuzom.md, and
...
def test_migrate_removes_legacy_llm_router(tmp_path, monkeypatch):
    ...
    assert keep.exists(), "migration must not touch the current chuzom.md"
