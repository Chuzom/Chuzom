...
``~/.chuzom``. An unsanitized session_id like ``../../tmp/evil`` escaped the
...
mkdir'd ``~/.chuzom/policies`` — a read-only state dir raised PermissionError
...
def _load():
    spec = importlib.util.spec_from_file_location("chuzom_auto_route_hook_iso", HOOK_PATH)
...
def test_readonly_state_dir_fails_open(tmp_path: Path) -> None:
    ...
    try:
        ...
        os.chmod(home / ".chuzom", 0o700)  # restore so tmp cleanup works
    assert proc.returncode == 0, (
        "CHZ-ST-003 regression: read-only ~/.chuzom crashed the hook "
