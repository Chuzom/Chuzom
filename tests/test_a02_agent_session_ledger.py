...
from chuzom.router import TaskType, RoutingProfile, _LEDGER_SESSION_OVERRIDE
...
def temp_ledger(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("CHUZOM_SESSION_ID", raising=False)
    return db
...
def test_override_resets_and_does_not_leak(temp_ledger, monkeypatch):
    monkeypatch.setenv("CHUZOM_SESSION_ID", "env-session")
    # No override set: emit falls through to CHUZOM_SESSION_ID.
...
def test_precedence_order():
    """override > CHUZOM_SESSION_ID > correlation_id."""
    ...
    try:
        ...
        _LEDGER_SESSION_OVERRIDE.reset(tok)
    os.environ.pop("CHUZOM_SESSION_ID", None)
