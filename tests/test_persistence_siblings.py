...
(~/.chuzom/idempotency.db). Both persisted LLM/session content 100% raw at 0644.
...
async def test_session_summary_redacts_secret_and_is_0600(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("CHUZOM_PERSIST_REDACTION", "on")
    _reset_config()
    from chuzom import context
...
async def test_session_summary_ttl_physically_deletes(tmp_path, monkeypatch):
    db = tmp_path / "usage.db"
    monkeypatch.setenv("CHUZOM_PERSIST_TTL_DAYS", "1")
    monkeypatch.delenv("CHUZOM_PERSIST_RAW", raising=False)
    _reset_config()
    from chuzom import context
    monkeypatch.setattr(context, "_get_db_path", lambda: db)
...
def test_idempotency_redacts_by_default_and_is_0600(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("CHUZOM_PERSIST_REDACTION", "on")
    _reset_config()
    from chuzom import idempotency
...
def test_idempotency_raw_optin_preserves_exact_replay(tmp_path, monkeypatch):
    """The escape hatch CHUZOM_PERSIST_RAW=1 must keep byte-exact dedupe replay."""
...
def test_all_known_content_sinks_route_through_persist_redact():
    ...
    root = Path(__file__).resolve().parents[1] / "src" / "chuzom"
    content_sinks = [
