...
Previously `uninstall()` left `chuzom-statusline.sh` on disk AND its `statusLine`
...
`uninstall_claw_code()` also left the `CHUZOM_CLAW_CODE=true` marker in
`~/.claw-code/.env` so the host still believed chuzom was active.
"""
...
def test_uninstall_removes_statusline_and_sidecars(tmp_path, monkeypatch):
    ...
    statusline = hooks / "chuzom-statusline.sh"
...
def test_uninstall_preserves_a_foreign_statusline(tmp_path, monkeypatch):
    """A statusLine the user set themselves (not chuzom's) must be left alone."""
    hooks = tmp_path / "hooks"
    hooks.mkdir(parents=True)
...
def test_uninstall_claw_code_removes_sidecars_and_flag(tmp_path, monkeypatch):
    ...
    assert "CHUZOM_CLAW_CODE" not in env_after, "RED2-5-02: flag not removed"
