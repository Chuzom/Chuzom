"""Tests for the best-effort operational alert sink (chuzom.alerts).
...
def _clean_env(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv("CHUZOM_ALERTS_DISABLED", raising=False)
    monkeypatch.delenv("CHUZOM_ALERT_WEBHOOK", raising=False)
...
def test_emit_without_webhook_only_logs(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("chuzom.alerts.urllib.request.urlopen", fake_urlopen)
...
def test_disabled_is_a_noop(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("CHUZOM_ALERTS_DISABLED", "1")
    monkeypatch.setenv("CHUZOM_ALERT_WEBHOOK", "http://example.test/hook")
...
def test_webhook_failure_is_swallowed(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("CHUZOM_ALERT_WEBHOOK", "http://example.test/hook")
    monkeypatch.setattr("chuzom.alerts.urllib.request.urlopen", boom)
...
def test_never_raises_on_bad_detail(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("CHUZOM_ALERT_WEBHOOK", "http://example.test/hook")
    monkeypatch.setattr(
        "chuzom.alerts.urllib.request.urlopen",
...
def _diff(diff_pct: float):
    from chuzom.invoice_reconciliation import ReconciliationDiff

    return ReconciliationDiff(
        provider="anthropic", period="2026-06",
        provider_reported_usd=100.0, chuzom_reported_usd=100.0 * (1 - diff_pct),
...
def test_alert_if_discrepant_within_tolerance_no_alert(monkeypatch: pytest.MonkeyPatch) -> None:
    from chuzom import invoice_reconciliation as ir

    fired = []
    monkeypatch.setattr("chuzom.alerts.emit_alert", lambda *a, **k: fired.append((a, k)))
...
def test_alert_if_discrepant_over_tolerance_alerts(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    fired = []
    monkeypatch.setattr("chuzom.alerts.emit_alert", lambda event, **k: fired.append((event, k)))
...
def test_alert_if_discrepant_env_threshold(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    fired = []
    monkeypatch.setenv("CHUZOM_INVOICE_DISCREPANCY_PCT", "0.10")  # 10% tolerance
    monkeypatch.setattr("chuzom.alerts.emit_alert", lambda *a, **k: fired.append(1))
