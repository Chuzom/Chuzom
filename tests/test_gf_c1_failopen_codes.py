...
HERMETICITY
-----------
`failopen` writes under `state_path()`, i.e. `CHUZOM_HOME`. Every test points that at
...
class TestRouterBaselineEstimate:
    def test_records_its_own_code_and_degrades_to_zero(self, store, monkeypatch):
        ...
        monkeypatch.setattr(
            "chuzom.session_spend._estimate_cost",
...
def _break_calibration_import(monkeypatch) -> None:
    """Make `from chuzom import calibration` raise, whatever ran before this test.

    Two steps, and the second one is why this is a helper rather than one line.

    `from chuzom import calibration` first tries `getattr(chuzom, "calibration")`. Once
    ...
    monkeypatch.delattr(chuzom, "calibration", raising=False)
    monkeypatch.setitem(sys.modules, "chuzom.calibration", None)
