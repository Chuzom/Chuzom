...
Plus:
  ...
  * an opt-in live-Ollama test (skipped unless CHUZOM_LIVE_OLLAMA=1 and Ollama is
...
from chuzom.agentic.react import ChatTurn, ReActAgent, ToolCall, default_tool_executor
...
async def test_real_harness_bounded_edit_end_to_end(tmp_path, monkeypatch, temp_db):
    ...
    monkeypatch.setenv("CHUZOM_BOUNDED_OPERATIONAL", "1")
...
@pytest.mark.skipif(
    os.environ.get("CHUZOM_LIVE_OLLAMA", "").strip().lower() not in ("1", "true", "yes", "on")
    or not _ollama_reachable(),
    reason="live Ollama not enabled (set CHUZOM_LIVE_OLLAMA=1 with a reachable Ollama)",
)
