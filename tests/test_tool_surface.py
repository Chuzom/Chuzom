...
completion tool names while ``CHUZOM_SLIM`` defaulted to ``consolidated``, under
...
def test_consolidated_is_the_default_when_env_is_unset(monkeypatch):
    ...
    monkeypatch.delenv("CHUZOM_SLIM", raising=False)
...
def test_tool_surface_has_no_chuzom_imports():
    """It must stay loadable by path from a hook whose interpreter has no chuzom."""
    tree = ast.parse((REPO / "src" / "chuzom" / "tool_surface.py").read_text())
    for node in ast.walk(tree):
        if isinstance(node, ast.ImportFrom) and (node.module or "").startswith("chuzom"):
            pytest.fail(f"tool_surface imports {node.module} — breaks standalone load")
        if isinstance(node, ast.Import):
            for a in node.names:
                assert not a.name.startswith("chuzom"), f"imports {a.name}"


def test_tool_surface_loads_standalone_by_path(tmp_path):
    """Simulates a hook running under an interpreter without chuzom installed."""
    spec = importlib.util.spec_from_file_location(
        "standalone_tool_surface", REPO / "src" / "chuzom" / "tool_surface.py"
    )
...
def test_installer_ships_tool_surface_beside_the_hooks():
    from chuzom.install_hooks import _HOOK_SUPPORT_FILES
    assert ("tool_surface.py", "chuzom_tool_surface.py") in _HOOK_SUPPORT_FILES
...
def test_usage_refresh_counts_the_consolidated_door(tmp_path):
    ...
    hook = REPO / "src" / "chuzom" / "hooks" / "usage-refresh.py"
    env = {**_os.environ, "HOME": str(tmp_path), "CHUZOM_SLIM": "consolidated"}
    for tool in ("mcp__chuzom__llm", "llm_query"):
        subprocess.run([sys.executable, str(hook)], input=_json.dumps({"toolName": tool}),
                       capture_output=True, text=True, env=env)

    log = tmp_path / ".chuzom" / "savings_log.jsonl"
    ...
    assert "mcp__chuzom__llm" in tools, "the consolidated door was not counted"
    assert "llm_query" in tools


def test_usage_refresh_skips_observability_doors(tmp_path):
    """chuzom_status is pure observability. Counting it as a routed call would
    ...
    subprocess.run([sys.executable, str(hook)],
                   input=_json.dumps({"toolName": "mcp__chuzom__chuzom_status"}),
                   capture_output=True, text=True, env=env)
    log = tmp_path / ".chuzom" / "savings_log.jsonl"
    rows = [_json.loads(line) for line in log.read_text().splitlines()] if log.exists() else []
    assert not any("chuzom_status" in r["tool"] for r in rows)
...
def test_installed_rules_never_name_an_unregistered_tool(tier):
    ...
    rules = sorted((REPO / "src" / "chuzom" / "rules").glob("*.md"))
...
def test_localize_never_emits_a_double_call(tier):
    ...
    targets = sorted((REPO / "src" / "chuzom" / "rules").glob("*.md")) + \
...
def test_localize_is_a_true_noop_on_legacy_tiers():
    ...
    for f in sorted((REPO / "src" / "chuzom" / "rules").glob("*.md")):
        src = f.read_text()
...
def test_rules_installer_writes_the_localized_text(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("CHUZOM_SLIM", "consolidated")
    src = ih._RULES_SRC / "chuzom.md"
