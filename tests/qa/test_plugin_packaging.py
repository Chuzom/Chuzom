"""Plugin packaging audit — Tessera must work as a proper plugin for
...
Two integration patterns are covered:
    ...
    A. **Marketplace plugins** (Claude Code, Codex CLI): the host has a
       ...
       launch the Tessera MCP server. Hosts in this group:
    ...
    B. **MCP-config plugins** (Cursor, Gemini CLI): the host has no
       ...
       mechanism. Tessera's Python adapters (hosts/cursor.py,
...
def test_mcp_json_is_valid_and_complete(plugin_dir: str, host_name: str):
    """The .mcp.json must register tessera under a recognizable command."""
    mcp = json.loads((ROOT / plugin_dir / ".mcp.json").read_text())
    assert "mcpServers" in mcp
    assert "tessera" in mcp["mcpServers"]
    entry = mcp["mcpServers"]["tessera"]
    assert "command" in entry
    # Command must reference a tessera launch path (not legacy)
    cmd = entry.get("command", "")
    args = entry.get("args", [])
    full = f"{cmd} {' '.join(args)}"
    assert "tessera" in full, (
        f"{host_name} .mcp.json command doesn't reference tessera: {full!r}"
    )
    assert "claude-code-tessera" not in full, (
        f"{host_name} .mcp.json still references legacy claude-code-tessera"
    )
...
def test_marketplace_json_has_owner_and_plugins(
    ...
    plugin = mp["plugins"][0]
    assert plugin.get("name") == "tessera"
...
MCP_CONFIG_HOSTS = [
    ("cursor", "tessera.hosts.cursor", "CursorAdapter", "Cursor"),
    ("gemini-cli", "tessera.hosts.gemini_cli", "GeminiCliAdapter", "Gemini CLI"),
]
...
def test_mcp_config_adapter_install_uses_real_binary(
    ...
    """The command must be a recognizable Tessera launcher, not legacy."""
    import importlib
    mod = importlib.import_module(module)
    cls = getattr(mod, class_name)
    cfg = tmp_path / "mcp.json"
    cls(config_path=cfg).install(server_command=["tessera"])

    config = json.loads(cfg.read_text())
    entry = config["mcpServers"]["tessera"]
    cmd = entry.get("command", "")
    assert "tessera" in cmd or cmd in ("uvx", "python", "python3"), (
...
def test_mcp_config_adapter_handles_user_override_command(
    ...
    """A power user might run tessera via `uvx tessera-router` — the adapter
    must record whatever command was passed without rewriting it."""
    import importlib
    mod = importlib.import_module(module)
    cls = getattr(mod, class_name)
    cfg = tmp_path / "mcp.json"
    cls(config_path=cfg).install(server_command=["uvx", "tessera-router"])

    config = json.loads(cfg.read_text())
    entry = config["mcpServers"]["tessera"]
    assert entry["command"] == "uvx"
    assert entry["args"] == ["tessera-router"]
...
def test_marketplace_plugin_versions_match_pyproject():
    ...
    causes the host to show 'tessera v10.1.3' in the marketplace while
