...
def _run(payload, home, env):
    e = {k: v for k, v in os.environ.items() if k not in ("CHUZOM_ENFORCE", "CHUZOM_SLIM", "CHUZOM_DELEGATE")}
    e["HOME"] = str(home)
    e.update(env)
...
_ENV = {"CHUZOM_ENFORCE": "hard", "CHUZOM_DELEGATE": "on", "CHUZOM_SLIM": "consolidated"}
...
def test_calling_llm_act_clears_the_execution_lock(tmp_path):
    ...
    r = _run({"session_id": "e3", "tool_name": "mcp__chuzom__llm_act", "tool_input": {"task": "deploy"}},
             tmp_path, _ENV)
