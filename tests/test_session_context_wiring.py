...
calls, no real Ollama/API calls, no real ~/.chuzom writes. Path.home() is
...
def ar(monkeypatch, tmp_path):
    ...
    # real ~/.chuzom, regardless of when each constant was first computed.
    monkeypatch.setenv("HOME", str(tmp_path))
    monkeypatch.setattr(Path, "home", classmethod(lambda cls: tmp_path))
    monkeypatch.setattr(module, "_ROUTER_DIR", tmp_path / ".chuzom", raising=False)
    monkeypatch.setattr(module, "log_routing_decision", lambda **kw: None, raising=False)

    import chuzom.hooks.savings_logger as savings_logger
    ...
    monkeypatch.setenv("CHUZOM_DISABLE_LLM_CLASSIFIERS", "1")
    monkeypatch.setenv("CHUZOM_DIRECT_EXECUTION", "1")
    monkeypatch.setenv("CHUZOM_ENFORCE", "suggest")
    monkeypatch.setenv("CHUZOM_ZERO_CLAUDE", "off")
...
def _patch_direct_execution(ar, monkeypatch, *, execute_chain_fn, needs_tools=False):
    import chuzom.hooks.chain_builder as chain_builder
    import chuzom.hooks.direct_executor as direct_executor
...
def test_records_user_prompt_before_draft(ar, monkeypatch):
    ...
    def fake_execute_chain(prompt, chain, task_type, timeout=4, context=None):
