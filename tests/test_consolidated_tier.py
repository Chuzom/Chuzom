...
CHUZOM_SLIM=consolidated registers ONLY the ~11 front-door tools (old tools hidden,
...
from chuzom.tool_tiers import CONSOLIDATED_TOOLS, make_should_register, tier_summary


def test_consolidated_tier_admits_the_front_doors():
    gate = make_should_register("consolidated")
    for door in ("llm", "llm_act", "chuzom_status", "chuzom_admin", "chuzom_session"):
...
def test_consolidated_tier_hides_the_old_tools():
    ...
    for old in ("llm_query", "llm_analyze", "llm_code", "llm_savings", "llm_usage",
                "llm_set_profile", "chuzom_agent_list"):
