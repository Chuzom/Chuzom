...
at their Okta org:
    ...
    export CHUZOM_OIDC_TEST_ISSUER="https://<your-org>.okta.com/oauth2/default"
    export CHUZOM_OIDC_TEST_TOKEN="<a-real-access-or-id-token-from-that-issuer>"
    export CHUZOM_OIDC_TEST_AUDIENCE="api://default"   # optional; the token's aud
...
_ISSUER = os.environ.get("CHUZOM_OIDC_TEST_ISSUER", "").strip()
_TOKEN = os.environ.get("CHUZOM_OIDC_TEST_TOKEN", "").strip()

pytestmark = pytest.mark.skipif(
    not (_ISSUER and _TOKEN),
    reason="live OIDC test skipped — set CHUZOM_OIDC_TEST_ISSUER + CHUZOM_OIDC_TEST_TOKEN to run against a real tenant",
)


def test_live_oidc_token_validates_against_real_jwks(monkeypatch: pytest.MonkeyPatch) -> None:
    from chuzom.enterprise.oidc import OidcClaims, OidcConfig, OidcValidator
    ...
    monkeypatch.setenv("CHUZOM_OIDC_ISSUER", _ISSUER)
    aud = os.environ.get("CHUZOM_OIDC_TEST_AUDIENCE", "").strip()
    if aud:
        monkeypatch.setenv("CHUZOM_OIDC_AUDIENCE", aud)
    jwks_uri = os.environ.get("CHUZOM_OIDC_TEST_JWKS_URI", "").strip()
    if jwks_uri:
        monkeypatch.setenv("CHUZOM_OIDC_JWKS_URI", jwks_uri)

    config = OidcConfig.from_env()
    assert config is not None, "OidcConfig.from_env() returned None — check CHUZOM_OIDC_* env"
