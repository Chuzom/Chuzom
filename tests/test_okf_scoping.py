...
def test_direct_executor_injects_and_enriches(tmp_path, monkeypatch):
    ...
    spec = importlib.util.spec_from_file_location(
        "direct_executor",
        Path(__file__).resolve().parent.parent / "src" / "chuzom" / "hooks" / "direct_executor.py",
    )
