...
def _run_hook(
    ...
    *,
    ...
    smart-default injection (irrelevant here — every test sets CHUZOM_ENFORCE
    explicitly to advise)."""
    env = {k: v for k, v in os.environ.items() if k != "CHUZOM_ENFORCE"}
    env["HOME"] = str(home)
    env["CHUZOM_ENFORCE"] = "advise"
    if extra_env:
...
def ledger_db(tmp_path):
    """A dedicated ledger DB path, isolated from ~/.chuzom/usage.db, shared
    between the subprocess hook invocation (via CHUZOM_EXECUTION_LEDGER_DB in
...
def test_advise_never_blocks_on_matching_tool(tmp_path, ledger_db):
    from chuzom.execution_ledger import get_route_accounting
    ...
    result = _run_hook(
        ...
        home=tmp_path,
        extra_env={"CHUZOM_EXECUTION_LEDGER_DB": str(ledger_db)},
    )
...
def test_advise_matches_via_expected_tool_exact_name(tmp_path, ledger_db):
    ...
    _write_pending(
        ...
        route_id=route_id,
        turn_id=1,
        expected_tool="mcp__chuzom__some_tool",
    )

    result = _run_hook(
        ENFORCE_ROUTE_HOOK,
        {"session_id": session_id, "tool_name": "mcp__chuzom__some_tool"},
        home=tmp_path,
        extra_env={"CHUZOM_EXECUTION_LEDGER_DB": str(ledger_db)},
    )
