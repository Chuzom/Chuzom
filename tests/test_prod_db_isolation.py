"""Unisolated tests must not write into the user's real ~/.chuzom/usage.db.

WHAT WENT WRONG
---------------
`cost.py` carried a "stub-detection guard" whose docstring claimed that unisolated
tests "can never pollute the real ~/.chuzom/usage.db". It matched an exact fingerprint:
...
from chuzom.cost import _refuse_unisolated_test_write
...
def test_allows_an_isolated_temp_database(tmp_path):
    ...
    CHUZOM_DB_PATH. Blocking those would break the suite and teach everyone to set
    CHUZOM_ALLOW_STUBS=1 permanently, which would put us straight back here."""
