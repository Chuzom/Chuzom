...
`set-enforce` wrote `enforce:` into ~/.chuzom/routing.yaml, and
...
the machine whose process env did not happen to carry CHUZOM_ENFORCE. The
...
from chuzom import enforce_config
...
def home(tmp_path, monkeypatch):
    ...
    # CHZ-SR-01: the session tier resolves through chuzom.paths, so isolation is
    # CHUZOM_HOME rather than a home= override — the same lever production uses.
    monkeypatch.setenv("CHUZOM_HOME", str(tmp_path / ".chuzom"))
    return tmp_path


def _write_global(home, mode):
    (home / ".chuzom" / "routing.yaml").write_text(f"enforce: {mode}\n")


def _write_session(home, sid, mode):
    d = home / ".chuzom" / "sessions" / sid
...
def test_set_enforce_writes_the_session_file_not_the_global_one(home, monkeypatch):
    ...
    from chuzom.commands import set_enforce as se

    monkeypatch.setattr(se.Path, "home", classmethod(lambda cls: home))  # global-file path only
    monkeypatch.setenv("CLAUDE_SESSION_ID", "sess-E2E")
    _write_global(home, "smart")

    se.cmd_set_enforce(["hard"])

    sess = home / ".chuzom" / "sessions" / "sess-E2E" / "enforce"
    assert sess.is_file() and sess.read_text().strip() == "hard"
    assert "enforce: smart" in (home / ".chuzom" / "routing.yaml").read_text(), (
...
def test_global_flag_still_writes_machine_wide(home, monkeypatch):
    ...
    assert "enforce: off" in (home / ".chuzom" / "routing.yaml").read_text()
    assert not (home / ".chuzom" / "sessions" / "sess-E2E" / "enforce").exists()
...
def test_no_session_id_still_writes_globally(home, monkeypatch):
    ...
    assert "enforce: soft" in (home / ".chuzom" / "routing.yaml").read_text()
