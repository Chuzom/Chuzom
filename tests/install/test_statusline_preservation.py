...
def sandbox(tmp_path, monkeypatch):
    ...
    import chuzom.install_manifest as im

    claude = tmp_path / ".claude"
    hooks_dst = claude / "hooks"
    settings_path = claude / "settings.json"
    manifest_path = tmp_path / ".chuzom" / "install-manifest.json"
...
def test_uninstall_restores_the_original_byte_for_byte(sandbox):
    ...
    sandbox.ih.install()
    assert sandbox.read_settings()["statusLine"] != CUSTOM_STATUSLINE  # chuzom's now
...
def test_idempotent_across_install_install_uninstall_uninstall(sandbox):
    """A second install must not re-capture chuzom's own value as "the original".
    ...
    chuzom's replacement, so uninstall restores chuzom's own status line and the
...
def test_chuzoms_own_statusline_is_not_recorded_as_a_users(sandbox):
    """Installing over a previous chuzom install must capture nothing."""
    sandbox.write_settings(
        {"statusLine": {"type": "command", "command": "bash /somewhere/chuzom-statusline.sh"}}
    )

    sandbox.ih.install()

    rec = sandbox.im.find("json_key", sandbox.settings_path, key="statusLine")
    assert rec is None, "chuzom's own statusLine was captured as if it were the user's"
...
def test_a_backup_of_settings_exists_after_a_destructive_install(sandbox):
    ...
    If the manifest is lost (deleted ~/.chuzom, a failed write, a user tidying
