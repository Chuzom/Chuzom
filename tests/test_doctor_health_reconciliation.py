...
Before the fix, `chuzom doctor` never read the router's in-memory HealthTracker, so
...
from chuzom.health import HealthTracker, read_health_snapshot


def test_router_trip_is_visible_cross_process(tmp_path, monkeypatch):
    monkeypatch.setenv("CHUZOM_HEALTH_SNAPSHOT", str(tmp_path / "health.json"))
...
def test_read_snapshot_fail_open_when_absent(tmp_path, monkeypatch):
    monkeypatch.setenv("CHUZOM_HEALTH_SNAPSHOT", str(tmp_path / "nope.json"))
