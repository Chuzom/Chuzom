"""Agno integration for tessera — RouteredModel and RouteredTeam.

Install:
    pip install "claude-code-tessera[agno]"
...
try:
    ...
    raise ImportError(
        ...
        "Install it with: pip install 'claude-code-tessera[agno]'"
    ) from e
...
def _llm_response_to_model_response(resp: LLMResponse) -> ModelResponse:
    """Convert tessera LLMResponse to an Agno ModelResponse."""
...
class RouteredModel(Model):
    """Drop-in Agno Model that routes each call through tessera.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(id="tessera", provider="tessera", name="RouteredModel", **kwargs)
    ...
    def invoke_stream(
        ...
        # tessera doesn't stream natively — return full response as one chunk
        assistant_message.metrics.start_timer()
        llm_resp = _run_coroutine_sync(self._aroute(messages))
        assistant_message.metrics.stop_timer()
...
class RouteredTeam(Team):
    ...
    async def _apply_budget_pressure(self) -> bool:
        ...
        try:
            from tessera.cost import get_quality_report
