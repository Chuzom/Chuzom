"""LiteLLM BudgetManager integration for tessera.

When teams run a LiteLLM Proxy with budget management enabled, tessera
...
This allows enterprises to:
  ...
  - Have tessera automatically route away from expensive providers
...
Configuration::
    ...
    export TESSERA_LITELLM_BUDGET_DB=/path/to/litellm.db

    # Optional: which user/team key to track (defaults to aggregate)
    export TESSERA_LITELLM_USER=my-team
...
_DB_ENV = "TESSERA_LITELLM_BUDGET_DB"
_USER_ENV = "TESSERA_LITELLM_USER"
...
async def get_litellm_budget_cap(provider: str, user: str | None = None) -> float:
    ...
    Args:
        ...
        user:     LiteLLM user/team key. Defaults to ``TESSERA_LITELLM_USER``.
