...
from tessera.budget import get_model_pressure
...
def score_model(
    ...
    *,
    ...
    Returns:
        :class:`~tessera.types.ScoredModel` with all component scores.
    """
    ...
    if capability is None:
        from tessera.types import ProviderTier
...
async def score_all_models(
    ...
    Args:
        models: Sequence of model IDs (strings) or
            :class:`~tessera.types.ModelCapability` objects.
        task_type: Task type string (``"code"``, ``"analyze"``, …).
        complexity: Complexity key.

    Returns:
        List of :class:`~tessera.types.ScoredModel` sorted by score
        descending (best first).  Never raises; returns empty list on error.
    """
    with traced_span(
        "score_all_models",
        tracer_name="tessera.scorer",
        task_type=task_type,
