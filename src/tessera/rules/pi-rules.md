...
> These rules apply when using tessera MCP tools inside Pi (pi.dev coding agent).
...
Pi loads MCP servers from `~/.pi/agent/mcp.json`. Once tessera is
...
Pi's native model is included in your subscription. tessera adds value by:
