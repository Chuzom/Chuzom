...
from tessera.types import QualityMode, RoutingProfile, Tier
...
class RouterConfig(BaseSettings):
    ...
    # `~/.tessera/config.yaml` fallback path used by enterprise installs.
    openrouter_api_key: str = ""

    # ── Claude Pro/Max subscription ──
    # Set to True when using tessera inside Claude Code (Pro/Max subscription).
    ...
    tessera_claude_subscription: bool = False

    # ── Gemini Subscription (Google One AI Pro) (v9.0.1) ──
    # Set to True when using tessera inside Gemini CLI with a subscription.
    ...
    tessera_gemini_subscription: bool = False
    ...
    # Set automatically by `tessera install` when ~/.claw-code/ is detected.
    tessera_claw_code: bool = False
    ...
    # ── Router settings ──
    tessera_profile: RoutingProfile = RoutingProfile.BALANCED
    tessera_tier: Tier = Tier.FREE
    tessera_db_path: Path = Path.home() / ".tessera" / "usage.db"
    tessera_monthly_budget: float = 20.0  # $20/month default cap
    tessera_daily_spend_limit: float = 0.0  # 0 = disabled; >0 fires alert when crossed
    ...
    # tessera_team_endpoint: webhook URL for push notifications.
    ...
    tessera_team_endpoint: str = ""   # e.g. https://hooks.slack.com/...
    tessera_user_id: str = ""         # override auto-detected git email
    tessera_team_chat_id: str = ""    # Telegram chat_id (only for Telegram)

    # ── Digest settings (v3.3) ──
    # Separate from team_endpoint — digest goes to a different channel/webhook.
    # Falls back to tessera_team_endpoint if not set.
    tessera_webhook_url: str = ""     # TESSERA_WEBHOOK_URL

    # ── Tool slim mode (v4.0) ──
    # Reduce the number of registered MCP tools to save context tokens.
    # Values: "off" (all 41 tools), "routing" (12 tools), "core" (4 tools).
    # Set TESSERA_SLIM=routing in the MCP server env to activate.
    tessera_slim: str = "routing"     # TESSERA_SLIM (routing=12 core, off=all)
    ...
    tessera_escalate_above: float = 0.0   # TESSERA_ESCALATE_ABOVE (per-call USD)
    # Hard stop: cancel all calls once session spend exceeds this total.
    # 0.0 = disabled. Example: 1.0 = $1.00/session hard stop.
    tessera_hard_stop_above: float = 0.0  # TESSERA_HARD_STOP_ABOVE (session USD)
    ...
    # Use TESSERA_MAX_AGENT_DEPTH to override. Default: 3.
    tessera_max_agent_depth: int = 3  # TESSERA_MAX_AGENT_DEPTH
    ...
    tessera_discovery_ttl: int = 1800     # TESSERA_DISCOVERY_TTL
    ...
    tessera_benchmark_ttl_days: int = 7   # TESSERA_BENCHMARK_TTL_DAYS
    ...
    tessera_budget_openai: float = 0.0       # TESSERA_BUDGET_OPENAI
    tessera_budget_gemini: float = 0.0       # TESSERA_BUDGET_GEMINI
    tessera_budget_groq: float = 0.0         # TESSERA_BUDGET_GROQ
    tessera_budget_deepseek: float = 0.0     # TESSERA_BUDGET_DEEPSEEK
    tessera_budget_together: float = 0.0     # TESSERA_BUDGET_TOGETHER
    tessera_budget_perplexity: float = 0.0   # TESSERA_BUDGET_PERPLEXITY
    tessera_budget_mistral: float = 0.0      # TESSERA_BUDGET_MISTRAL

    # ── Enterprise integrations (v5.1) ──
    helicone_api_key: str = ""
    tessera_helicone_pull: bool = False  # Pull spend from Helicone API
    tessera_litellm_budget_db: str = ""  # Path to LiteLLM proxy budget DB
    ...
    tessera_spend_aggregation: Literal["max", "sum"] = "max"
    ...
    tessera_community: bool = False   # TESSERA_COMMUNITY
    ...
    def available_providers(self) -> set[str]:
        ...
        if self.tessera_claude_subscription:
            providers.discard("anthropic")
        if self.tessera_gemini_subscription:
    ...
    def model_post_init(self, __context: dict) -> None:
        ...
        try:
            from tessera.safe_config import load_safe_config
    ...
    def apply_keys_to_env(self) -> None:
        ...
        In subscription mode (``tessera_claude_subscription=True``),
        ...
        for field_name, (provider_name, env_var) in self._PROVIDER_MAP.items():
            ...
            if self.tessera_claude_subscription and provider_name == "anthropic":
                continue
...
def get_config() -> RouterConfig:
    ...
    if _config.tessera_claude_subscription:
