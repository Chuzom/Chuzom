...
to Tessera's routing call. It's NOT an agent runtime. The framework owns
the loop; Tessera owns the model pick + cost tracking + lineage.
...
the Tessera core stays framework-agnostic.
"""
...
class FrameworkAdapter(Protocol):
    ...
    Methods:
        wrap_model: takes the framework's model object/config and returns
            a drop-in replacement that routes through Tessera.
