"""Agno framework adapter — concrete, re-exports from tessera.integrations.agno.

The concrete implementation lives in tessera.integrations.agno (ported
...
Install:
    pip install "tessera-router[agno]"

Usage:
    from tessera.frameworks.agno import RouteredModel, RouteredTeam
...
# expose None so callers can `from tessera.frameworks.agno import RouteredModel`
# without crashing.
try:
...
class AgnoAdapter:
    ...
    def wrap_model(self, framework_model: Any, agent_id: str | None = None):
        ...
        if not AGNO_AVAILABLE:
            raise ImportError(
                "Agno not installed. pip install 'tessera-router[agno]'"
            )
