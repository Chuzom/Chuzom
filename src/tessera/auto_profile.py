...
2. Generates ~/.tessera/profile.yaml with token-wise priority
...
from tessera.gemini_cli_agent import is_gemini_cli_available
from tessera.claude_usage import get_claude_pressure
from tessera.logging import get_logger

log = get_logger("tessera.auto_profile")

PROFILE_PATH = Path.home() / ".tessera" / "profile.yaml"
PROFILE_CACHE_PATH = Path.home() / ".tessera" / "profile_cache.json"
...
def detect_services() -> ServiceDetection:
    ...
    # Claude subscription
    if cfg.tessera_claude_subscription:
    ...
    if is_gemini_cli_available():
        ...
        try:
            from tessera.gemini_cli_quota import get_gemini_quota_status_sync
...
def generate_profile_yaml(detected: ServiceDetection) -> str:
    ...
    Returns:
        YAML string for ~/.tessera/profile.yaml
    """
    ...
    yaml += "# Generated by: tessera profile auto\n"
...
def save_profile(yaml_content: str) -> Path:
    """Save profile YAML to ~/.tessera/profile.yaml.
