"""tessera init-claude-memory — Auto-discover setup and generate Claude Code memory files."""

from tessera.config import get_config
from tessera.safe_config import write_safe_config_template, safe_config_path
...
def run_init_claude_memory() -> None:
    ...
    3. Creates ~/.tessera/config.yaml template for enterprise users

    Enterprise use case: When security teams block .env at project level, users can:
    - Fill in ~/.tessera/config.yaml with their keys
    ...
    discovered = {
        ...
        "tessera_profile": config.tessera_profile.value,
        "tessera_claude_subscription": config.tessera_claude_subscription,
    }
    ...
    print(f"  Profile:     {config.tessera_profile.value}")
    print(
        f"  Mode:        {'Claude Subscription (no API key)' if config.tessera_claude_subscription else 'API Key Mode'}"
    )
    print()
    print("=" * 60)
    print()
    ...
    print("  2. If .env is blocked: Edit ~/.tessera/config.yaml and add your API keys")
    print("  3. Run 'tessera status' to verify the setup\n")
