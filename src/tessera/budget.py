...
Pressure sources by provider type:
  ...
  Claude subscription (anthropic)
      Reads ``~/.tessera/usage.json`` (written by the session-start hook).
...
from tessera.types import BudgetState, LOCAL_PROVIDERS
...
_USAGE_JSON = Path.home() / ".tessera" / "usage.json"
...
# Env var override: TESSERA_STALE_PRESSURE_FLOOR (float 0.0–1.0, default 0.5)
...
async def get_budget_state(provider: str) -> BudgetState:
    ...
    Returns:
        :class:`~tessera.types.BudgetState` for the provider.
    """
    # Cache hit
...
async def get_all_budget_states() -> dict[str, BudgetState]:
    ...
    Returns:
        Dict mapping provider name → :class:`~tessera.types.BudgetState`.
    """
    cfg = get_config()
...
async def _claude_subscription_state() -> BudgetState:
    ...
    import os
    stale_floor = float(os.environ.get("TESSERA_STALE_PRESSURE_FLOOR", "0.5"))
    try:
...
async def _api_provider_state(provider: str) -> BudgetState:
    ...
    Spend sources:
      ...
      2. Helicone pull (when TESSERA_HELICONE_PULL=true)
      3. LiteLLM Proxy DB (when TESSERA_LITELLM_BUDGET_DB is set)

    Aggregation strategy is controlled by ``tessera_spend_aggregation``:
    ...
    try:
        from tessera.integrations.litellm_budget import (
    ...
    if cfg.tessera_spend_aggregation == "sum":
...
def _get_cap(provider: str, cfg) -> float:
    ...
    """
    from tessera.budget_store import get_cap as _store_cap
    stored = _store_cap(provider)
    if stored > 0.0:
        return stored

    cap_map = {
        "openai": cfg.tessera_budget_openai,
        "gemini": cfg.tessera_budget_gemini,
        "groq": cfg.tessera_budget_groq,
        "deepseek": cfg.tessera_budget_deepseek,
        "together": cfg.tessera_budget_together,
        "perplexity": cfg.tessera_budget_perplexity,
        "mistral": cfg.tessera_budget_mistral,
    }
...
async def _get_provider_monthly_spend(provider: str) -> float:
    ...
    try:
        import aiosqlite
        db_path = get_config().tessera_db_path
