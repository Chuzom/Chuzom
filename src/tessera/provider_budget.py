...
async def get_provider_spend() -> dict[str, float]:
    ...
    config = get_config()
    db_path = config.tessera_db_path
...
async def get_provider_budgets() -> dict[str, ProviderBudget]:
    ...
    naming convention ``TESSERA_BUDGET_<PROVIDER>=<USD>``, e.g.
    ``TESSERA_BUDGET_OPENAI=10.00``. A value of 0 (or missing env var)
    means unlimited.
    ...
    # Format: TESSERA_BUDGET_OPENAI=10.00 (USD/month)
    import os
    limits: dict[str, float] = {}
    for provider in config.available_providers:
        env_key = f"TESSERA_BUDGET_{provider.upper()}"
