...
log = get_logger("tessera.quota_balance")

_QUOTA_CACHE_FILE = Path.home() / ".tessera" / "codex_quota.json"
...
def get_codex_pressure(daily_limit: int = 1000) -> float:
    ...
    Reads from ~/.tessera/codex_quota.json. Resets counter if date changed.
...
def record_codex_request(daily_limit: int = 1000) -> None:
    """Increment Codex daily counter and persist to ~/.tessera/codex_quota.json.
...
async def get_provider_pressures() -> dict[str, float]:
    ...
    from tessera.gemini_cli_quota import get_gemini_pressure
