"""FastMCP server — MCP entry point for tessera.

All 60 tools are registered by modules in tessera/tools/:
...
Tool slim mode (TESSERA_SLIM=routing|core) reduces registered tools to save
context tokens — see tessera/tool_tiers.py for tier definitions.
...
from tessera.tools.setup import _mask_key, llm_setup  # noqa: F401

configure_logging()
log = get_logger("tessera.server")

mcp = FastMCP("tessera")

# Auto-update routing rules and hooks on startup if a newer version was installed via pip
try:
    from tessera.install_hooks import check_and_update_hooks as _update_hooks
    from tessera.install_hooks import check_and_update_rules as _update_rules
    _msg = _update_rules()
    if _msg:
...
try:
    from tessera.benchmarks import check_and_update_benchmarks as _update_benchmarks
...
try:
    import os as _os
    from tessera.health import get_tracker as _get_tracker
    ...
    try:
        _os.unlink(_os.path.expanduser("~/.tessera/reset_stale.flag"))
    except OSError:
        pass
except Exception:
    pass
...
try:
    from tessera.dynamic_routing import initialize_dynamic_routing
...
from tessera.tool_tiers import make_should_register, tier_summary as _tier_summary  # noqa: E402

_slim = get_config().tessera_slim
...
def router_status() -> str:
    ...
    lines = [
        f"Profile: {config.tessera_profile.value}",
        f"Tier: {config.tessera_tier.value}",
    ...
    if config.tessera_monthly_budget > 0:
        lines.append(f"Budget: ${config.tessera_monthly_budget:.2f}/mo")
