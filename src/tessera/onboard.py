...
Run as a CLI entry point (``tessera-onboard``) to launch the setup wizard.
...
The selected profile is written to ``.env`` as ``TESSERA_PROFILE`` and
...
def main() -> None:
    ...
    current_profile = existing.get("TESSERA_PROFILE", "balanced")
    profile = (
    ...
    lines.append(f"TESSERA_PROFILE={profile}")
