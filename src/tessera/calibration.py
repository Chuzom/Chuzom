...
_PRICING_PER_M: dict[str, dict[str, float]] = {
    # Anthropic — keys match the names in src/tessera/profiles.py chain entries
...
def projection_check(
    ...
    Side-effect-only — logs to ``tessera.calibration``. Telemetry layer
    ...
    log = logging.getLogger("tessera.calibration")
    if predicted <= 0:
        log.warning(
