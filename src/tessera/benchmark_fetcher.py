...
This module requires the ``scripts`` optional dependency group:
  ``pip install claude-code-tessera[scripts]``
"""
...
log = logging.getLogger("tessera.benchmark_fetcher")
...
def generate_benchmarks_md(data: dict[str, Any], output_path: Path) -> None:
    ...
    lines: list[str] = [
        ...
        "<!-- Source: src/tessera/data/benchmarks.json · Generator: scripts/update_benchmarks.py -->",
        "",
        "# Model Benchmarks",
        "",
        ...
        "users on the next `pip install --upgrade claude-code-tessera` + server restart.",
        "",
        "---",
        "",
        "## Tier System",
    ...
    lines += [
        ...
        "        ├── Writes src/tessera/data/benchmarks.json  ← used by router",
        ...
        "with `~/.tessera/benchmarks.json` and overwrites if the bundled copy is newer.",
    ]
