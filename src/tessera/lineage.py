...
def tier_for_model(model_id: str) -> Tier:
    ...
    canonical mapping lives in tessera.model_selector; this is a fallback.
    """
    m = model_id.lower()
...
class LineageStore:
    ...
    Default location: ~/.tessera/lineage.db (gitignored). Override via the
    TESSERA_LINEAGE_PATH env var for tests or custom installs.
    """

    def __init__(self, db_path: Path | None = None) -> None:
        self.db_path = db_path or Path(
            os.environ.get("TESSERA_LINEAGE_PATH")
            or (Path.home() / ".tessera" / "lineage.db")
        )
