...
(``tessera benchmark run <name>``), a policy diff tool
(``tessera policy diff a b``), and a regression detector
(``tessera benchmark regress --since <tag>``).
...
class BenchmarkRunner(Protocol):
    ...
    so a broken runner can't take down ``tessera benchmark`` for everyone.
    """
...
try:  # pragma: no cover — import-time wiring
    from tessera.benchmark.runners import routerarena as _routerarena  # noqa: F401
