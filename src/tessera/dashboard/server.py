...
log = get_logger("tessera.dashboard")

DEFAULT_PORT = 7337
_TOKEN_FILE = Path.home() / ".tessera" / "dashboard.token"
...
async def _get_stats() -> dict:
    ...
    try:
        ...
        _pkg_ver = "?"
    stats["config"] = {
        "profile": config.tessera_profile.value,
        "monthly_budget": config.tessera_monthly_budget,
        "daily_limit": config.tessera_daily_spend_limit,
        "subscription_mode": config.tessera_claude_subscription,
        "version": _pkg_ver,
    }
...
<div id="tab-budget" class="p-8 max-w-[1600px] mx-auto space-y-8 hidden">
  ...
  <p class="text-xs text-outline mt-2">Caps are saved to <code class="font-mono">~/.tessera/budgets.json</code> and take effect immediately on the next routing call.</p>
</div>
...
async def run(port: int = DEFAULT_PORT) -> None:
    ...
    async def handle_budget_get(request: "web.Request") -> "web.Response":
        from tessera.budget import get_all_budget_states
        from tessera.budget_store import list_caps
    ...
    async def handle_budget_set(request: "web.Request") -> "web.Response":
        ...
        from tessera.budget_store import set_cap, remove_cap
        from tessera.budget import invalidate_cache
        if cap <= 0:
            remove_cap(provider)
        else:
    ...
    async def handle_metrics(request: "web.Request") -> "web.Response":
        ...
        lines = [
            "# HELP tessera_spend_usd Monthly spend per provider",
            "# TYPE tessera_spend_usd gauge",
        ]
        for p, s in sorted(states.items()):
            lines.append(f'tessera_spend_usd{{provider="{p}"}} {s.spend_usd:.4f}')

        lines += [
            "# HELP tessera_budget_pressure Budget pressure per provider (0=free 1=exhausted)",
            "# TYPE tessera_budget_pressure gauge",
        ]
        for p, s in sorted(states.items()):
            lines.append(f'tessera_budget_pressure{{provider="{p}"}} {s.pressure:.4f}')

        lines += [
            "# HELP tessera_cap_usd Configured monthly cap per provider (0=unlimited)",
            "# TYPE tessera_cap_usd gauge",
        ]
        for p, s in sorted(states.items()):
            lines.append(f'tessera_cap_usd{{provider="{p}"}} {s.cap_usd:.2f}')

        lines += [
            "# HELP tessera_savings_usd_total Lifetime savings vs Sonnet baseline",
            "# TYPE tessera_savings_usd_total counter",
            f"tessera_savings_usd_total {total_saved:.4f}",
            "# HELP tessera_external_usd_total Lifetime external API spend",
            "# TYPE tessera_external_usd_total counter",
            f"tessera_external_usd_total {total_external:.4f}",
            "",
        ]
