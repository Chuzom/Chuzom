...
Stored to ~/.tessera/model_tracking.jsonl for persistent analysis.
"""
...
log = get_logger("tessera.model_tracking")

TRACKING_PATH = Path.home() / ".tessera" / "model_tracking.jsonl"
...
def log_routing_patterns_to_chronicle(summary: dict) -> bool:
    ...
    try:
        from tessera.chronicle import chronicle_log_decision
    except ImportError:
        return False
    ...
    try:
        chronicle_log_decision(
            ...
            affects=["src/tessera/router.py", "src/tessera/profiles.py"],
            risk="low",
        )
        return True
    except Exception:
