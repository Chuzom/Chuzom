...
from tessera.memory.profiles import get_primary_model_for_tool
from tessera.model_selector import select_model
from tessera.prompt_injection import wrap_prompt_with_boundaries
from tessera.profiles import complexity_to_profile
from tessera.provider_budget import get_provider_budgets, rank_external_models
from tessera.router import route_and_call
from tessera.statusline_hud import record_routing_decision
...
async def llm_classify(
    ...
    # Extra tip when TESSERA_EXPLAIN=1 is not set
    if os.getenv("TESSERA_EXPLAIN") != "1":
        lines.append(row("  Tip: set TESSERA_EXPLAIN=1 to see [→ model · task] on every response"))
...
async def llm_auto(
    ...
    """
    from tessera.cost import import_savings_log, get_lifetime_savings_summary
...
async def llm_stream(
    ...
    if model:
        ...
        chain = get_model_chain(
            ...
            is_subscription_mode=config.tessera_claude_subscription,
        )
...
async def llm_select_agent(
    ...
    from tessera.state import set_active_agent
