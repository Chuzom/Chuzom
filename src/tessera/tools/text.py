...
from tessera.cost import log_compression_stat
...
def _cache_result(
    ...
    """
    try:
        from tessera.result_cache import store_result
        store_result(
            user_prompt=prompt,
...
def _record_quality(resp: LLMResponse, task_type: str, complexity: str | None) -> None:
    ...
    """
    try:
        from tessera.quality_feedback import record_quality, score_response
        qs = score_response(
            response=resp.content or "",
            task_type=task_type,
...
# Controlled by TESSERA_EXPLAIN config: "footer" (default), "header",
# "verbose", "off". Legacy TESSERA_EXPLAIN=1 maps to "header".
# ---------------------------------------------------------------------------
...
def _get_explain_mode() -> str:
    ...
    # Legacy compat: TESSERA_EXPLAIN=1 → "header" (old behavior)
    legacy = os.getenv("TESSERA_EXPLAIN", "")
    if legacy == "1":
        return "header"
    ...
    try:
        from tessera.config import get_config
        return getattr(get_config(), "tessera_explain", "footer")
    except Exception:
        return "footer"
...
def _routing_explanation(resp: LLMResponse, task: str) -> str:
    ...
    ctx_info = ""
    try:
        from tessera.context import get_last_optimization
...
def _apply_response_compression(content: str) -> tuple[str, bool]:
    ...
    if os.getenv("TESSERA_COMPRESS_RESPONSE", "").lower() != "true":
    ...
    try:
        from tessera.compression import ResponseCompressor
