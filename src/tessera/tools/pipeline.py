...
from tessera.orchestrator import PIPELINE_TEMPLATES, auto_orchestrate, run_pipeline
from tessera.types import Tier
...
async def llm_orchestrate(
    ...
    if template and template in PIPELINE_TEMPLATES:
        ...
        if config.tessera_tier == Tier.FREE and len(steps) > 2:
            return (
                f"Template '{template}' has {len(steps)} steps — free tier allows up to 2. "
                "Upgrade to Pro for unlimited pipeline steps: https://tessera.dev/pricing"
            )
