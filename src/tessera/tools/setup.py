...
from tessera.cost import rate_routing_decision
...
```
TESSERA_BUDGET_OPENAI=10.00
TESSERA_BUDGET_GEMINI=5.00
TESSERA_MONTHLY_BUDGET=20.00
```
...
def _setup_add(provider: str, api_key: str) -> str:
    ...
    from tessera.budget_store import get_cap as _get_stored_cap
    budget_nudge = ""
    if _get_stored_cap(provider) <= 0:
        budget_nudge = (
            f"\n\n💰 **Set a monthly budget cap** to protect against runaway costs:\n"
            f"   `tessera budget set {provider} 20`\n"
...
def _setup_install_hooks() -> str:
    ...
    lines.extend([
        ...
        "Work that skips a required route is blocked by default (`TESSERA_ENFORCE=hard`).",
        "The router classifies tasks and injects `⚡ MANDATORY ROUTE` directives that",
        "direct Claude to use the optimal `llm_*` tool.",
        "Set `TESSERA_ENFORCE=soft` or `off` to relax enforcement.",
        "",
...
def _setup_uninstall_hooks() -> str:
    ...
    from tessera.install_hooks import uninstall
    actions = uninstall()
    lines = [
