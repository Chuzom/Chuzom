...
from tessera.commands.gain import show_gain

from tessera.cache import get_cache
from tessera.codex_agent import is_codex_available
...
from tessera.forecast import get_burn_forecast
from tessera.health import get_tracker
from tessera.provider_budget import get_provider_budgets
from tessera.types import RoutingProfile, colorize_provider, MODEL_COST_PER_1K
from tessera import state as _state
...
async def llm_save_session(ctx: Context) -> str:
    ...
    """
    from tessera.context import auto_summarize_session
...
async def llm_usage(period: str = "today") -> str:
    ...
    """
    from tessera.claude_usage import _bar, _row, _time_until
    ...
    if is_codex_available():
        ...
        lines.append(row("  Session spend counts tessera tool calls only"))
    else:
    ...
    config = get_config()
    if config.tessera_monthly_budget > 0:
        monthly_spend = await get_monthly_spend()
        budget = config.tessera_monthly_budget
    ...
    forecast = await get_burn_forecast(budget_limit=config.tessera_monthly_budget)
    if forecast.days_of_data >= 3:
        ...
        if config.tessera_monthly_budget > 0 and forecast.pct_remaining >= 0:
...
async def llm_health() -> str:
    ...
    from tessera.config import probe_ollama
    config = get_config()
    tracker = get_tracker()
    report = tracker.status_report()

    lines = [
        f"## Provider Health (profile: {config.tessera_profile.value})",
    ...
    if not report:
        lines.append("No providers configured. Run `tessera-onboard` to set up API keys.")
    else:
...
async def llm_hook_health() -> str:
    ...
    lines.append(
        "\n💡 Tip: Check ~/.tessera/hook_errors.log for full error history"
    )
    return "\n".join(lines)
...
async def llm_dashboard(port: int = 7337) -> str:
    ...
    subprocess.Popen(
        [sys.executable, "-m", "tessera.dashboard.__main__", "--port", str(port)],
        **kwargs,
    )
    ...
    stop_cmd = (
        "pkill -f 'tessera.dashboard'"
        if sys.platform != "win32"
        else "taskkill /F /FI \"WINDOWTITLE eq tessera.dashboard\""
    )
...
async def llm_savings() -> str:
    ...
    """
    from tessera.cost import get_savings_by_period
    ...
    lines.append(HR)
    lines.append(row("  Tip: run `tessera test \"<prompt>\"` to simulate routing"))
    lines.append(HR)
...
async def llm_team_report(period: str = "week") -> str:
    ...
    """
    from tessera.team import build_team_report, get_project_id, get_user_id, detect_channel

    config = get_config()
    user_id = get_user_id(override=config.tessera_user_id)
    ...
    endpoint = config.tessera_team_endpoint
    if endpoint:
        ...
        lines.append(row("  Set TESSERA_TEAM_ENDPOINT to push to Slack/Discord/Telegram."))
    lines.append(HR)
    return "\n".join(lines)
...
async def llm_team_push(period: str = "week") -> str:
    ...
    Sends a formatted message to the endpoint set by ``TESSERA_TEAM_ENDPOINT``.
    ...
    Args:
        period: ``"today"``, ``"week"``, ``"month"``, or ``"all"``.
    """
    from tessera.team import build_team_report, detect_channel, get_project_id, get_user_id, push_report
    ...
    if not endpoint:
        return (
            ...
            "Set TESSERA_TEAM_ENDPOINT in your .env or routing.yaml:\n\n"
            "  TESSERA_TEAM_ENDPOINT=https://hooks.slack.com/...    # Slack\n"
            "  TESSERA_TEAM_ENDPOINT=https://discord.com/api/webhooks/...  # Discord\n"
            "  TESSERA_TEAM_ENDPOINT=https://api.telegram.org/bot{token}/  # Telegram\n"
            "  TESSERA_TEAM_ENDPOINT=https://your-server.com/webhook  # Generic\n"
        )

    user_id = get_user_id(override=config.tessera_user_id)
    project_id = get_project_id()
    chat_id = config.tessera_team_chat_id
...
async def llm_policy() -> str:
    ...
    Displays the merged policy from all three layers:
      - Org policy   (~/.tessera/org-policy.yaml)
      - User policy  (~/.tessera/routing.yaml)
      - Repo policy  (.tessera.yml)

    Also shows the last 10 policy enforcement events from the audit log.
    """
    from tessera.policy import load_org_policy, policy_summary
    from tessera.repo_config import effective_config as get_repo_config
...
async def llm_digest(period: str = "week", send: bool = False) -> str:
    ...
    Args:
        ...
        send:   If True, POST the digest to TESSERA_WEBHOOK_URL.
    """
    from tessera.digest import format_digest, send_to_webhook
    ...
    config = get_config()
    url = config.tessera_webhook_url or config.tessera_team_endpoint
    if not url:
        return (
            digest + "\n\n"
            "  ℹ️  To send this digest automatically, set:\n"
            "  TESSERA_WEBHOOK_URL=https://hooks.slack.com/...\n"
        )
...
async def llm_benchmark() -> str:
    ...
    Also shows an optional community export status if TESSERA_COMMUNITY=true.
    """
    from tessera.community import (
    ...
    config = get_config()
    if config.tessera_community:
        export_msg = await prepare_community_export()
        report += f"\n\n  📤 Community export: {export_msg}"
    else:
        report += (
            "\n\n  ℹ️  Set TESSERA_COMMUNITY=true to prepare an anonymous "
...
async def llm_session_spend() -> str:
    ...
    lines = [
        ...
        "",
        "Scope: routed tessera calls only; native host turns are not visible.",
    ]
    ...
    config = get_config()
    if config.tessera_escalate_above > 0:
        lines.append("")
        lines.append(
            f"Per-call cap: ${config.tessera_escalate_above:.2f} "
            f"(TESSERA_ESCALATE_ABOVE)"
        )
    if config.tessera_hard_stop_above > 0:
        lines.append(
            f"Session hard stop: ${config.tessera_hard_stop_above:.2f} "
            f"(TESSERA_HARD_STOP_ABOVE)"
        )
...
async def llm_session_savings() -> str:
    ...
    * **Paid API** — OpenAI, Anthropic, OpenRouter, Perplexity, etc.
    ...
    """
    from tessera.session_spend import get_session_spend
    from tessera.tiers import render_tier_table, summarize_tiers
...
async def llm_approve_route(
    ...
    estimated cost exceeded TESSERA_ESCALATE_ABOVE. The pending call is
...
async def llm_quota_status() -> str:
    ...
    """
    try:
        from tessera.quota_balance import get_provider_pressures, get_balanced_provider_order
...
async def llm_budget() -> str:
    ...
    """
    try:
        from tessera.budget import get_all_budget_states, format_budget_summary

        states = await get_all_budget_states()
        summary = format_budget_summary(states)

        from tessera.types import LOCAL_PROVIDERS
        uncapped = [
        ...
        if uncapped:
            ...
            nudge = (
                ...
                f"   Run `tessera budget set <provider> <amount>` to protect against runaway costs."
            )
...
async def llm_share_profile() -> str:
    ...
    Exports ~/.tessera/learned_routes.json and prepares it for upload
    to a shared community repository. Useful for publishing routing patterns
    you've learned that may benefit other tessera users.

    Returns:
        Path to exported profile and upload instructions
    """
    from tessera.memory.profiles import load_learned_profile
    ...
    try:
        ...
        export_path = Path.home() / ".tessera" / "profile_export.json"
        export_path.write_text(json.dumps(export_data, indent=2))

        return (
            f"✅ Profile exported to: {export_path}\n\n"
            f"Share at: https://github.com/ypollak2/tessera-profiles\n"
            f"Your routes:\n"
            + "\n".join(
...
async def llm_import_profile(url: str = "") -> str:
    ...
    """
    try:
        from tessera.memory.profiles import load_learned_profile, save_learned_profile, LearnedRoute

        # Load existing profile
        existing = load_learned_profile()

        # Default to latest community profile if no URL provided
        DEFAULT_COMMUNITY_URL = "https://raw.githubusercontent.com/ypollak2/tessera-profiles/main/community.json"
        if not url:
...
async def llm_model_usage(ctx: Context, hours: int = 24) -> str:
    ...
    """
    from tessera.model_tracking import (
...
async def llm_model_eval(ctx: Context) -> str:
    ...
    """
    from tessera.model_evaluator import (
