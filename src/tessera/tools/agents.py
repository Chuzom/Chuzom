...
Surface area:
    tessera_agent_list             — discovery
    tessera_agent_start_session    — open a session
    tessera_agent_route            — route within session (or refuse if budget breached)
    tessera_agent_check_budget     — non-destructive budget query
    tessera_agent_complete_session — finalize + return summary
    tessera_agent_lineage          — full step-by-step trace
...
def _default_config_path() -> Path:
    ...
    env_path = os.environ.get("TESSERA_AGENTS_CONFIG")
    if env_path:
...
def get_registry() -> AgentRegistry:
    ...
    if _registry is None:
        ...
        if path.exists():
            ...
            # Empty registry — every tessera_agent_* call returns "no agents
...
async def tessera_agent_list() -> dict:
...
async def tessera_agent_start_session(
    agent_id: str,
...
async def tessera_agent_check_budget(session_id: str) -> dict:
...
async def tessera_agent_route(
    ...
    tessera.router.route_and_call requires touching router.py — that
...
async def tessera_agent_complete_session(session_id: str) -> dict:
...
async def tessera_agent_lineage(session_id: str, limit: int = 200) -> dict:
...
def register(mcp) -> None:
    ...
    mcp is the FastMCP instance from tessera.server. Each tool is exposed
    under the canonical name; descriptions are pulled from the docstrings.
    """
    mcp.tool()(tessera_agent_list)
    mcp.tool()(tessera_agent_start_session)
    mcp.tool()(tessera_agent_check_budget)
    mcp.tool()(tessera_agent_route)
    mcp.tool()(tessera_agent_complete_session)
    mcp.tool()(tessera_agent_lineage)
