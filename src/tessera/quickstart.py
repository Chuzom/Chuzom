"""Interactive quickstart wizard for tessera.
...
def main() -> None:
    ...
    print("\n" + "═" * 54)
    print("  tessera quickstart — first routed call in < 5min")
    print("═" * 54)
    ...
    if has_keys:
        ...
        print("  Tip: run `tessera setup` for full provider configuration.")
    elif has_ollama:
        ...
        if not _ask_yes("Continue without API keys?", default=False):
            print("\n  Exiting. Add keys and re-run: tessera quickstart\n")
            return

    # ── Step 3: Install ────────────────────────────────────────────────────────
    _print_step(3, total_steps, "Installing for your hosts")

    from tessera.cli import _install_host, _run_install
    ...
    for host in target_hosts:
        ...
        try:
            ...
            print(f"    Manual: tessera install --host {host}")
    ...
    if _ask_yes("Run test call now?", default=True):
        try:
            ...
            from tessera.types import TaskType, RoutingProfile
            ...
            print(f"\n  ⚠ Test call failed: {e}")
            print("    Check your API keys with: tessera doctor")
    else:
        print("  Skipped.")
    ...
    print("")
    print("  Typical savings with tessera:")
    print("")
    print("  ┌─────────────────┬────────────┬────────────┬──────────┐")
    ...
    print("")
    print("  Commands:")
    print("    tessera status    — view routing status and savings")
    print("    tessera doctor    — diagnose any installation issues")
    print("    tessera setup     — configure additional providers")
    print("═" * 54 + "\n")
