"""IAF-style session retrospective engine for tessera.
...
RETROSPECT_DIR = Path.home() / ".tessera" / "retrospectives"
DIRECTIVES_FILE = Path.home() / ".tessera" / "directives.md"
SESSION_START_FILE = Path.home() / ".tessera" / "session_start.txt"
...
def write_retrospective_file(retro: dict, session_id: str = "") -> Path:
    ...
    File location: ~/.tessera/retrospectives/YYYY-MM-DD-HHMMSS.md
...
def write_directives(actions: list[dict]) -> None:
    ...
    File: ~/.tessera/directives.md (append-only)
