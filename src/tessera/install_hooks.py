"""Install tessera hooks and rules globally into Claude Code.
...
Can be run as:
  - CLI: ``tessera-install-hooks``
  - MCP tool: ``llm_setup(action='install_hooks')``
  - Python: ``from tessera.install_hooks import install; install()``
"""
...
_SUBSCRIPTION_VAR = "TESSERA_CLAUDE_SUBSCRIPTION"

_RULES_VERSION_RE = re.compile(r"<!--\s*tessera-rules-version:\s*(\d+)\s*-->")
_HOOK_VERSION_RE = re.compile(r"#\s*tessera-hook-version:\s*(\d+)")
...
def check_and_update_hooks() -> list[str]:
    ...
    after ``pip install --upgrade claude-code-tessera`` without re-running install.
...
def check_and_update_rules() -> str | None:
    ...
    after ``pip install --upgrade claude-code-tessera`` without re-running install.
    """
    rules_src = _RULES_SRC / "tessera.md"
    rules_dst = _RULES_DST / "tessera.md"
...
_HOOK_DEFS = [
    ("session-start.py", "tessera-session-start.py", "SessionStart", ""),
    ("auto-route.py", "tessera-auto-route.py", "UserPromptSubmit", ""),
    ("status-bar.py", "tessera-status-bar.py", "UserPromptSubmit", ""),
    ("enforce-route.py", "tessera-enforce-route.py", "PreToolUse", ""),
    ("agent-route.py", "tessera-agent-route.py", "PreToolUse", "Agent"),
    ("subagent-start.py", "tessera-subagent-start.py", "SubagentStart", ""),
    ("usage-refresh.py", "tessera-usage-refresh.py", "PostToolUse", "llm_|mcp__tessera__llm"),
    ("cc-usage-track.py", "tessera-cc-usage-track.py", "PostToolUse", "Agent"),
    ("playwright-compress.py", "tessera-playwright-compress.py", "PostToolUse", ""),
    ("bash-compress.py", "tessera-bash-compress.py", "PostToolUse", ""),
    ("session-end.py", "tessera-session-end.py", "Stop", ""),
]
...
_CLAW_CODE_HOOK_DEFS = [
    ("session-start.py",            "tessera-session-start.py",  "SessionStart",     ""),
    ("auto-route.py",               "tessera-auto-route.py",     "UserPromptSubmit", ""),
    ("status-bar-clawcode.py",      "tessera-status-bar.py",     "UserPromptSubmit", ""),
    ("enforce-route.py",            "tessera-enforce-route.py",  "PreToolUse",       ""),
    ("agent-route.py",              "tessera-agent-route.py",    "PreToolUse",       "Agent"),
    ("subagent-start.py",           "tessera-subagent-start.py", "SubagentStart",    ""),
    ("usage-refresh.py",            "tessera-usage-refresh.py",      "PostToolUse",  "llm_|mcp__tessera__llm"),
    ("playwright-compress.py",      "tessera-playwright-compress.py", "PostToolUse",  ""),
    ("bash-compress.py",            "tessera-bash-compress.py",       "PostToolUse",  ""),
    ("session-end-clawcode.py",     "tessera-session-end.py",         "Stop",         ""),
]
...
def _legacy_alias_path(hooks_dir: Path, src_name: str, dst_name: str) -> Path | None:
    ...
    if src_name == dst_name or not dst_name.startswith("tessera-"):
        return None
...
def _sync_legacy_hook_alias(
    ...
    """Keep an existing tessera legacy alias in sync with the canonical hook.

    We only sync unprefixed hook aliases when they are clearly managed by
    tessera already, or when settings explicitly reference the alias path and
...
def _install_claude_code_cli(mcp_entry: dict) -> list[str]:
    """Register tessera in ~/.claude.json so `claude -p` (non-interactive) picks it up.
    ...
    if claude_bin:
        ...
        try:
            result = _sp.run(
                [claude_bin, "mcp", "add", "--scope", "user", "tessera", cmd_str] + args,
                capture_output=True, text=True, timeout=15,
            )
            if result.returncode == 0:
                return ["Registered tessera MCP server in ~/.claude.json (via claude mcp add)"]
        except Exception:
    ...
    try:
        ...
        if "tessera" in servers:
            return ["MCP server already in ~/.claude.json: tessera"]
        servers["tessera"] = mcp_entry
        _CLAUDE_JSON_PATH.write_text(json.dumps(data, indent=2) + "\n", encoding="utf-8")
        return ["Registered tessera MCP server in ~/.claude.json (direct merge)"]
    except OSError as e:
        return [f"WARNING: could not register MCP in ~/.claude.json: {e}"]


def _uninstall_claude_code_cli() -> list[str]:
    """Remove tessera from ~/.claude.json."""
    import subprocess as _sp
    ...
    if claude_bin:
        try:
            result = _sp.run(
                [claude_bin, "mcp", "remove", "--scope", "user", "tessera"],
                capture_output=True, text=True, timeout=10,
            )
            if result.returncode == 0:
                return ["Removed tessera from ~/.claude.json (via claude mcp remove)"]
        except Exception:
            pass
    ...
    try:
        if not _CLAUDE_JSON_PATH.exists():
            return []
        data = json.loads(_CLAUDE_JSON_PATH.read_text(encoding="utf-8"))
        if "tessera" not in data.get("mcpServers", {}):
            return []
        del data["mcpServers"]["tessera"]
        _CLAUDE_JSON_PATH.write_text(json.dumps(data, indent=2) + "\n", encoding="utf-8")
        return ["Removed tessera from ~/.claude.json"]
    except OSError:
        return []


def _install_claude_desktop() -> list[str]:
    """Add tessera to Claude Desktop's claude_desktop_config.json.
    ...
    entry = {"command": tessera_bin, "args": []}

    config = _load_desktop_config(config_path)
    servers = config.setdefault("mcpServers", {})

    if "tessera" in servers:
        return ["Claude Desktop: tessera already registered"]

    servers["tessera"] = entry
    config_path.parent.mkdir(parents=True, exist_ok=True)
    config_path.write_text(json.dumps(config, indent=2) + "\n", encoding="utf-8")
    return [f"Registered tessera in Claude Desktop → {config_path}"]


def _uninstall_claude_desktop() -> list[str]:
    """Remove tessera from Claude Desktop config. Returns actions taken."""
    config_path = claude_desktop_config_path()
    if config_path is None or not config_path.exists():
        return []

    config = _load_desktop_config(config_path)
    if "tessera" not in config.get("mcpServers", {}):
        return []

    del config["mcpServers"]["tessera"]
    config_path.write_text(json.dumps(config, indent=2) + "\n", encoding="utf-8")
    return [f"Removed tessera from Claude Desktop → {config_path}"]
...
def check_api_keys() -> list[str]:
    ...
    gemini_subscription_on = os.environ.get("TESSERA_GEMINI_SUBSCRIPTION", "").lower() in ("1", "true", "yes")
    ...
    if gemini_subscription_on:
        lines.append("  ✓  Gemini subscription mode active (TESSERA_GEMINI_SUBSCRIPTION=true)")
...
def install(force: bool = False) -> list[str]:
    ...
    # Build the entry using the installed tessera binary when available
    # (pip install), falling back to uv run for development installs.
    tessera_bin = shutil.which("tessera")
    if tessera_bin:
        mcp_entry: dict = {"command": tessera_bin, "args": []}
    else:
        uv_path = shutil.which("uv") or "uv"
        project_dir = str(_PACKAGE_DIR.parent.parent)
        mcp_entry = {"command": uv_path, "args": ["run", "--directory", project_dir, "tessera"]}
    ...
    if "tessera" not in mcp_servers:
        mcp_servers["tessera"] = mcp_entry
        _save_settings(settings2)
        actions.append("Registered tessera MCP server in ~/.claude/settings.json")
    else:
        actions.append("MCP server already in ~/.claude/settings.json: tessera")
    ...
    statusline_dst = _HOOKS_DST / "tessera-statusline.sh"
...
def uninstall() -> list[str]:
    ...
    if "tessera" in mcp_servers:
        del mcp_servers["tessera"]
        _save_settings(settings2)
        actions.append("Removed tessera MCP server from ~/.claude/settings.json")
...
def install_claw_code() -> list[str]:
    ...
    # ── Set TESSERA_CLAW_CODE=true in ~/.claw-code/.env ───────────────
    ...
    env_path = cc_dir / ".env"
    claw_flag = "TESSERA_CLAW_CODE=true"
    try:
        existing = env_path.read_text(encoding="utf-8") if env_path.exists() else ""
        if "TESSERA_CLAW_CODE" not in existing:
            ...
            actions.append(f"TESSERA_CLAW_CODE already set in {env_path}")
    except OSError as e:
        actions.append(f"WARN could not write {env_path}: {e}")

    # ── Register MCP server in claw-code settings ────────────────────────
    tessera_bin = shutil.which("tessera") or "tessera"
    mcp_entry = {"command": tessera_bin, "args": []}
    mcp_servers = settings.setdefault("mcpServers", {})
    if "tessera" not in mcp_servers:
        mcp_servers["tessera"] = mcp_entry
        settings_path.write_text(json.dumps(settings, indent=2) + "\n")
        actions.append(f"Registered tessera MCP server in {settings_path}")
    else:
...
def uninstall_claw_code() -> list[str]:
    """Remove tessera hooks and MCP registration from claw-code. Returns actions taken."""
    actions: list[str] = []
    ...
    if "tessera" in mcp_servers:
        del mcp_servers["tessera"]
        actions.append("Removed tessera MCP server from claw-code")
...
def main() -> None:
    """CLI entry point for tessera-install-hooks."""
    ...
    print("  To uninstall: tessera-install-hooks uninstall\n")
