...
from tessera.profiles import provider_from_model

log = get_logger("tessera.discover")
...
_DISCOVERY_CACHE = os.path.expanduser("~/.tessera/discovery.json")
...
def _update_discovery_cache(ollama_models: list[dict]) -> None:
    """Write discovered Ollama models to ~/.tessera/discovery.json.
...
def get_available_providers() -> set[str]:
    ...
    if config.anthropic_api_key and not config.tessera_claude_subscription:
...
def _load_cache(ttl: int = 3600) -> dict | None:
    ...
    import json
    import time
    from tessera.types import ProviderTier, TaskType
...
def get_cached_ollama_models() -> list[str]:
    ...
    Returns cached Ollama models from ~/.tessera/discovery.json only.
