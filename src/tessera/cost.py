...
SAVINGS_LOG_PATH = Path.home() / ".tessera" / "savings_log.jsonl"
...
is_simulated:       1 for dry-run test calls (tessera test), 0 for real calls
"""
...
async def _get_db() -> aiosqlite.Connection:
    ...
    """
    config = get_config()
    config.tessera_db_path.parent.mkdir(parents=True, exist_ok=True)
    # Secure file before creation (stores sensitive cost/token data)
    if not config.tessera_db_path.exists():
        config.tessera_db_path.touch(mode=0o600)
    db = await aiosqlite.connect(str(config.tessera_db_path))
...
def _get_team_identity() -> tuple[str, str]:
    ...
    """
    try:
        from tessera.team import get_project_id, get_user_id
        from tessera.config import get_config
        cfg = get_config()
        uid = get_user_id(override=cfg.tessera_user_id)
        pid = get_project_id()
        return uid, pid
    except Exception:
        return "", ""
...
async def log_usage(
    ...
    # ~/.tessera/usage.db. Tests that legitimately need to write stub rows
    # must set TESSERA_ALLOW_STUBS=1 or use the temp_db fixture (which
    # repoints TESSERA_DB_PATH and is unaffected by this guard).
    if (
...
def _validate_routing_insert(
    ...
    if final_provider not in VALID_PROVIDERS:
        raise ValueError(
            ...
            f"If this is a test, use TESSERA_DB_PATH to isolate test databases."
        )
    ...
    *,
    ...
    try:
        ...
        if success and response:
            try:
                from tessera.judge import evaluate_response_async
...
def _get_codex_baseline_for_task(task_type: str | None, complexity: str | None) -> str:
    ...
    Env override: TESSERA_CODEX_BASELINE (default: gpt-5.4). v9.3.0.
    """
    import os
    override = os.environ.get("TESSERA_CODEX_BASELINE", "").strip().lower()
...
def _get_gemini_baseline_for_task(task_type: str | None, complexity: str | None) -> str:
    ...
    Env override: TESSERA_GEMINI_BASELINE (default: gemini-2.5-pro). v9.3.1.
    """
    import os
    override = os.environ.get("TESSERA_GEMINI_BASELINE", "").strip().lower()
...
async def log_claude_usage(
    ...
    *,
    ...
    if "pytest" in sys.modules:
        config = get_config()
        prod_path = Path.home() / ".tessera" / "usage.db"
        if str(config.tessera_db_path) == str(prod_path):
            raise RuntimeError(
                ...
                f"Config path: {config.tessera_db_path}\n"
...
# Users can override via TESSERA_SAVINGS_BASELINE env var (default: "sonnet")
BASELINE_PRICING = {
...
def _get_baseline_model() -> str:
    ...
    """
    import os
    return os.environ.get("TESSERA_SAVINGS_BASELINE", "sonnet")
...
def _get_baseline_for_task(task_type: str | None, complexity: str | None) -> str:
    ...
    """
    import os
    # Env override still wins (back-compat with TESSERA_SAVINGS_BASELINE).
    override = os.environ.get("TESSERA_SAVINGS_BASELINE", "").strip().lower()
...
async def import_savings_log() -> int:
    ...
    ``~/.tessera/savings_log.jsonl``.  This function reads all lines,
