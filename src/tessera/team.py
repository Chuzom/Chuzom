...
def get_user_id(override: str = "") -> str:
    ...
    Priority:
      1. ``override`` arg (from ``TESSERA_USER_ID`` env var)
...
def _slack_payload(report: dict[str, Any]) -> dict:
    ...
    return {
        "blocks": [
            ...
            {
                "type": "context",
                "elements": [{"type": "mrkdwn", "text": "Powered by <https://github.com/ypollak2/tessera|tessera>"}],
            },
        ]
    }
...
def _discord_payload(report: dict[str, Any]) -> dict:
    ...
    return {
        "embeds": [{
            ...
            "footer": {"text": "tessera · github.com/ypollak2/tessera"},
        }]
    }
...
def _generic_payload(report: dict[str, Any]) -> dict:
    return {"source": "tessera", "version": "3.0", "report": report}
...
async def push_report(
    ...
    if channel == "slack":
        ...
        if not telegram_chat_id:
            return False, "Telegram requires TESSERA_TEAM_CHAT_ID to be set."
        post_url = url.rstrip("/")
...
async def build_team_report(
    ...
    """
    from tessera.cost import get_team_savings
