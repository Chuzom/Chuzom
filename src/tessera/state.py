...
from tessera.types import PRO_FEATURES, Tier
...
def get_active_profile():
    ...
    return get_config().tessera_profile
...
def _check_tier(feature: str) -> str | None:
    ...
    """
    config = get_config()
    if config.tessera_tier == Tier.FREE and feature in PRO_FEATURES:
        return (
            f"'{feature}' requires Pro tier ($12/mo). "
            f"Current tier: free. Upgrade at https://tessera.dev/pricing"
        )
    return None
