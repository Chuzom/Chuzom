...
copied to ``~/.tessera/benchmarks.json`` on startup) and uses it to
...
_INSTALLED = Path.home() / ".tessera" / "benchmarks.json"
...
def check_and_update_benchmarks() -> str | None:
    """Copy the bundled benchmarks file to ``~/.tessera/`` if it is newer.
...
def get_benchmark_data() -> dict[str, Any] | None:
    ...
    Load order: ``~/.tessera/benchmarks.json`` (user-local, may be newer
...
def get_model_failure_penalty(
    ...
    try:
        if failure_rates is not None:
            ...
            import asyncio
            from tessera.cost import get_model_failure_rates
...
def get_quality_score(model_id: str, task_type: str) -> float:
    ...
    Lookup order:
      1. **Benchmark data** (``~/.tessera/benchmarks.json``) — API models
...
def maybe_refresh_benchmarks_background(ttl_days: int = 7) -> bool:
    ...
    Checks whether ``~/.tessera/benchmarks.json`` was last fetched more
    ...
    Args:
        ttl_days: Days before the benchmark file is considered stale.
            Reads ``tessera_benchmark_ttl_days`` from config when available.
    ...
    try:
        ...
        ttl_days = get_config().tessera_benchmark_ttl_days
    except Exception:
        pass
