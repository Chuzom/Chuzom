...
The bandit consumes :class:`tessera.telemetry.ModelStats` and reorders the
...
``tessera.judge.reorder_by_quality`` with a proper exploit/explore split:
...
:data:`~tessera.telemetry.MIN_SAMPLES_FOR_SIGNAL` samples, the bandit
...
from tessera.telemetry import MIN_SAMPLES_FOR_SIGNAL, ModelStats, aggregate_stats
...
log = logging.getLogger("tessera.bandit")
