...
_QUOTA_CACHE_FILE = Path.home() / ".tessera" / "gemini_quota.json"
...
async def _run_gemini_stats() -> dict | None:
    ...
    """
    try:
        from tessera.gemini_cli_agent import find_gemini_binary
...
def _load_quota_cache() -> dict | None:
    """Load cached quota from ~/.tessera/gemini_quota.json if fresh.
...
def _save_quota_cache(data: dict) -> None:
    """Save quota data to ~/.tessera/gemini_quota.json."""
    try:
