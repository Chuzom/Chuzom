...
Usage:
    tessera                  — start the MCP server (stdio transport)
    tessera install              — install hooks, rules, and MCP server config globally
    tessera install --check      — show what would be installed without doing it
    tessera install --force      — reinstall even if already present
    tessera install --claw-code  — also install into claw-code (auto-detects ~/.claw-code/)
    tessera install --headless   — install for Docker/agent/CI environments (API-key mode, no OAuth)
    tessera install --host codex       — write Codex CLI config files
    tessera install --host opencode    — write OpenCode config files
    tessera install --host gemini-cli  — write Gemini CLI config files
    tessera install --host copilot-cli — write GitHub Copilot CLI config files
    tessera install --host openclaw    — write OpenClaw config files
    tessera install --host trae        — write Trae IDE config files
    tessera install --host pi          — write Pi coding agent (pi.dev) config files
    tessera install --host factory     — confirm Factory Droid plugin manifest
    tessera install --host desktop     — print Claude Desktop config snippet
    tessera install --host copilot     — print VS Code / Copilot config snippet
    tessera install --host all         — install / print all host configs
    tessera uninstall        — remove hooks and MCP registration
    tessera uninstall --purge — also delete ~/.tessera/ (usage DB, .env, logs)
    tessera setup            — interactive wizard: configure providers and API keys
    tessera init-policy      — interactive wizard: choose or create a routing policy (v7.5.0)
    tessera status           — show routing status, today's savings, subscription pressure
    tessera savings-report   — detailed token/cost breakdown (all-time, by model/provider)
    tessera savings-report --period week  — weekly savings report
    tessera doctor           — check that everything is wired up correctly
    tessera demo             — show routing decisions for sample prompts
    tessera dashboard        — start the web dashboard at localhost:7337
    tessera dashboard --port 7338  — use a custom port
    tessera set-enforce <mode>  — switch enforcement mode (smart|soft|hard|off)
    tessera team report [period]  — show team savings report (default: week)
    tessera team push [period]    — push report to Slack/Discord/Telegram/webhook
    tessera team setup            — interactively configure team endpoint
    tessera budget                — show all providers with spend, cap, pressure
    tessera budget set <p> <amt>  — set monthly cap in USD for provider p
    tessera budget remove <p>     — clear the cap for provider p
    tessera last [--count N]      — show your last N routing decisions (default: 5)
    tessera replay [--limit N]    — full transcript of routing decisions this session
    tessera snapshot [--date DATE] — mid-session monitoring: accuracy trends and gap detection
    tessera retrospect [--weekly] — IAF-style session debrief with routing directives
    tessera stats [--period recent] — show combined download stats (llm-routing + claude-code-tessera)
    tessera verify                — end-to-end health check (30 seconds)
"""
...
def _merge_json_mcp_block(
    ...
    Args:
        ...
        server_name: Name of MCP server (e.g., "tessera")
...
def _append_routing_rules(
    ...
    Args:
        ...
        rules_filename: Name of rules file in src/tessera/rules/ (e.g., "vscode-rules.md")
    ...
    if dest_path.exists():
        ...
        if "tessera" in existing:
            actions.append(f"skipped: {dest_path.name} already contains tessera rules")
            return actions
...
def _install_vscode_files() -> list[str]:
    """Install tessera MCP config for VS Code."""
    actions = []
    home = Path.home()
    ...
    actions.extend(
        _merge_json_mcp_block(
            mcp_json,
            "tessera",
            {"command": "uvx", "args": ["claude-code-tessera"]},
            root_key="servers",
        )
    )
...
def _install_cursor_files() -> list[str]:
    """Install tessera MCP config for Cursor IDE."""
    actions = []
    home = Path.home()
    ...
    # Add cursor rules
    cursor_rules = home / ".cursor" / "rules" / "tessera.md"
...
def _install_opencode_files() -> list[str]:
    """Install tessera MCP config for OpenCode."""
    actions = []
    home = Path.home()
...
def _install_gemini_cli_files() -> list[str]:
    """Install tessera MCP config for Gemini CLI."""
    actions = []
    home = Path.home()
    ...
    ext_dir = home / ".gemini" / "extensions" / "tessera"
...
def _install_copilot_cli_files() -> list[str]:
    """Install tessera MCP config for GitHub Copilot CLI."""
    actions = []
    home = Path.home()
...
def _install_openclaw_files() -> list[str]:
    """Install tessera MCP config for OpenClaw."""
    actions = []
    home = Path.home()
...
def _install_trae_files() -> list[str]:
    """Install tessera MCP config for Trae IDE."""
    actions = []
    home = Path.home()
...
def _install_pi_files() -> list[str]:
    """Install tessera MCP config for Pi coding agent (pi.dev)."""
    actions = []
    home = Path.home()
...
def _install_codex_cli_files() -> list[str]:
    """Install tessera MCP config for Codex CLI."""
    actions = []
    home = Path.home()
    ...
    # Add Codex rules
    rules_file = home / ".codex" / "rules" / "tessera.md"
...
def isolation_test_command() -> None:
    ...
    try:
        ...
        print("  cd /Users/yali.pollak/Projects/tessera")
...
def main() -> None:
    ...
    if args and args[0] == "install":
        ...
        from tessera.commands.uninstall import cmd_uninstall
        cmd_uninstall(args[1:])
    elif args and args[0] == "update":
        from tessera.commands.update import cmd_update
        cmd_update(args[1:])
    elif args and args[0] == "setup":
        from tessera.commands.setup import cmd_setup
        cmd_setup(args[1:])
    elif args and args[0] == "status":
        from tessera.commands.status import cmd_status
        cmd_status(args[1:])
    elif args and args[0] == "routing":
        from tessera.commands.routing import cmd_routing
        cmd_routing(args[1:])
    elif args and args[0] == "profile":
        from tessera.commands.profile import cmd_profile
        cmd_profile(args[1:])
    elif args and args[0] == "init-claude-memory":
        from tessera.cli_init_memory import run_init_claude_memory
        run_init_claude_memory()
    elif args and args[0] == "doctor":
        from tessera.commands.doctor import cmd_doctor
        cmd_doctor(args[1:])
    elif args and args[0] == "quickstart":
        from tessera.quickstart import main as _qs_main
        _qs_main()
    elif args and args[0] == "demo":
        from tessera.commands.demo import cmd_demo
        cmd_demo(args[1:])
    elif args and args[0] == "dashboard":
        from tessera.commands.dashboard import cmd_dashboard
        cmd_dashboard(args[1:])
    elif args and args[0] == "tui":
        from tessera.dashboard.tui import run as _tui_run
        _tui_run()
    elif args and args[0] == "share":
        from tessera.commands.share import cmd_share
        cmd_share(args[1:])
    elif args and args[0] == "test":
        from tessera.commands.test import cmd_test
        cmd_test(args[1:])
    elif args and args[0] == "onboard":
        from tessera.commands.onboard import cmd_onboard
        cmd_onboard(args[1:])
    elif args and args[0] == "config":
        from tessera.commands.config import cmd_config
        cmd_config(args[1:])
    elif args and args[0] == "init-policy":
        from tessera.cli_init_policy import run_init_policy_wizard
        run_init_policy_wizard()
    elif args and args[0] == "set-enforce":
        from tessera.commands.set_enforce import cmd_set_enforce
        cmd_set_enforce(args[1:])
    elif args and args[0] == "team":
        from tessera.commands.team import cmd_team
        cmd_team(args[1:])
    elif args and args[0] == "budget":
        from tessera.commands.budget import cmd_budget
        cmd_budget(args[1:])
    elif args and args[0] == "replay":
        from tessera.commands.replay import main as _replay_main
        _replay_main(args[1:])
    elif args and args[0] == "verify":
        from tessera.commands.verify import main as _verify_main
        _verify_main(args[1:])
    elif args and args[0] == "last":
        from tessera.commands.last import main as _last_main
        _last_main(args[1:])
    elif args and args[0] == "retrospect":
        from tessera.commands.retrospect import main as _retrospect_main
        _retrospect_main(args[1:])
    elif args and args[0] == "snapshot":
        from tessera.commands.snapshot import main as _snapshot_main
        _snapshot_main(args[1:])
    elif args and args[0] == "stats":
        from tessera.commands.stats import cmd_stats
        sys.exit(cmd_stats(args[1:]))
    elif args and args[0] == "savings-report":
        from tessera.commands.savings_report import main as _savings_report_main
        sys.exit(_savings_report_main(args[1:]))
    elif args and args[0] == "benchmark":
        from tessera.commands.benchmark import cmd_benchmark
        sys.exit(cmd_benchmark(args[1:]))
    elif args and args[0] == "policy":
        from tessera.commands.policy import cmd_policy
        ...
        from tessera.server import main as _mcp_main
        _mcp_main()
