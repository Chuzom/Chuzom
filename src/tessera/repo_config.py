...
Loads two optional YAML files and merges them:
  1. ~/.tessera/routing.yaml   — user-level overrides (always applied)
  2. .tessera.yml              — repo-level overrides (searched up from cwd)
...
class RepoConfig:
    ...
    def effective_enforce(self) -> str:
        ...
        env = os.environ.get("TESSERA_ENFORCE", "").lower()
    ...
    def effective_profile(self) -> str | None:
        ...
        env = os.environ.get("TESSERA_PROFILE", "").lower()
...
def find_repo_config_path(start: Path | None = None) -> Path | None:
    """Search start (default cwd) and ancestors for .tessera.yml."""
    here = start or Path.cwd()
    for candidate in [here, *here.parents]:
        p = candidate / ".tessera.yml"
        if p.exists():
            return p
...
def load_user_config() -> RepoConfig:
    """Load ~/.tessera/routing.yaml (user-level config)."""
    path = Path.home() / ".tessera" / "routing.yaml"
    if not path.exists():
        return RepoConfig()
...
def load_repo_config(start: Path | None = None) -> RepoConfig:
    """Load .tessera.yml from cwd or nearest ancestor (repo-level config)."""
