"""Async sidecar routing service for tessera.
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(
            Path.home() / ".tessera" / "service.log", mode="a"
        ),
    ],
)
logger = logging.getLogger("tessera.service")

app = FastAPI(title="tessera-service", version="5.3.0")
...
async def score_endpoint(req: ScoreRequest) -> ScoreResponse:
    ...
    """
    try:
        from tessera.types import TaskType, Complexity
...
def start_service(host: str = "127.0.0.1", port: int = 7337, log_level: str = "warning"):
    ...
    logger.info(f"Starting tessera service on {host}:{port}")
...
if __name__ == "__main__":
    port = int(os.environ.get("TESSERA_SERVICE_PORT", "7337"))
