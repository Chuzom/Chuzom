...
def record_hook_error(hook_name: str, error: str, context: dict | None = None) -> None:
    ...
    try:
        ...
        import sys
        print(f"[tessera] Hook {hook_name} error (couldn't log): {error}", file=sys.stderr)
...
def check_hook_permissions() -> dict[str, str]:
    ...
    Checks ~/.claude/hooks/tessera-*.py files, which are the actual hooks
    ...
    # Check for tessera-*.py hooks
    if hooks_dir.exists():
        for hook_file in hooks_dir.glob("tessera-*.py"):
            # Extract hook name from tessera-NAME.py → NAME
            hook_name = hook_file.stem.replace("tessera-", "")
...
def get_deadlock_report() -> dict | None:
    ...
    """
    try:
        from tessera.hook_deadlock_detector import HookDeadlockDetector
