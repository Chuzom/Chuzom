"""``tessera benchmark …`` — Plan 07 Cat G operational CLI.
...
underlying module (``tessera.benchmark`` / ``benchmark.regression``),
...
from tessera.benchmark import Prediction, get_runner, list_runners
...
def _cmd_run(argv: list[str]) -> int:
    parser = argparse.ArgumentParser(prog="tessera benchmark run")
    parser.add_argument("name", help="Runner name (see `tessera benchmark list`)")
    ...
    parser.add_argument(
        ...
        help="Version tag to store under (defaults to current tessera version).",
    )
...
def _cmd_regress(argv: list[str]) -> int:
    parser = argparse.ArgumentParser(prog="tessera benchmark regress")
...
def _print_help(exit_code: int = 0) -> int:
    print(
        "Usage: tessera benchmark <subcommand> [options]\n"
        "\n"
        "Subcommands:\n"
