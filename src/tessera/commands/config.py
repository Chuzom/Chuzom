...
def _run_config_init() -> None:
    """Create a starter .tessera.yml in the current directory."""
    from tessera.repo_config import fingerprint_repo
    ...
    template = f"""\
# .tessera.yml — repo-level routing config
# Docs: https://github.com/ypollak2/tessera
version: 1
...
"""
    ...
    print(f"  Edit it, then run {_bold('tessera config lint')} to validate.\n")
...
def _run_config(flags: list[str]) -> None:
    ...
    - init: Create a new .tessera.yml template
    """
    sub = flags[0] if flags else "show"

    from tessera.repo_config import (
        effective_config,
    ...
    print(f"\n{_bold('tessera config')}\n")
    print(HR)

    # Sources
    user_path = Path.home() / ".tessera" / "routing.yaml"
    repo_path = find_repo_config_path()
    print(f"  {_bold('User config:')}  {user_path}  {'✓' if user_path.exists() else _dim('(not found)')}")
    print(f"  {_bold('Repo config:')}  {repo_path or _dim('(none — no .tessera.yml in tree)')}")
    ...
    if not user_path.exists() and repo_path is None:
        warnings.append(
            "No config files found — using defaults. Run `tessera config init` to create one."
        )
...
def cmd_config(args: list[str]) -> int:
    """Execute: tessera config [show|lint|init]
