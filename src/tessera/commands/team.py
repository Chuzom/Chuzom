...
def _run_team(subcmd: str, flags: list[str]) -> None:
    """tessera team report|push|setup [period]."""
    import asyncio
    from tessera.team import (
    ...
    if calls == 0:
        ...
        print(f"  Try: {_bold('tessera team report all')}\n")
        return
    ...
    if subcmd == "push":
        if not endpoint:
            ...
            print(f"  Run {_bold('tessera team setup')} to configure Slack/Discord/Telegram.\n")
            return
        channel = detect_channel(endpoint)
        print(f"\n  Pushing to {_bold(channel)}...")
        success, msg = asyncio.run(push_report(
            report, endpoint, telegram_chat_id=config.tessera_team_chat_id,
        ))
        if success:
            print(_green(f"  {msg}"))
        else:
        ...
        channel = detect_channel(endpoint)
        print(f"\n  {_dim(f'Endpoint: {channel} configured — run tessera team push to send.')}")
    else:
        print(f"\n  {_dim('Tip: run tessera team setup to configure Slack/Discord/Telegram push.')}")
    print()


def _run_team_setup(config) -> None:
    """Interactive wizard to configure the team notification endpoint."""
    print(f"\n{_bold('[tessera] Team Notification Setup')}\n")
    ...
    for key, val in [("TESSERA_TEAM_ENDPOINT", url), ("TESSERA_TEAM_CHAT_ID", chat_id)]:
        if not val:
            continue
    ...
    from tessera.team import detect_channel
    channel = detect_channel(url)
    print(f"\n{_green('✓')} Team endpoint configured: {_bold(channel)}")
    print(f"  Run {_bold('tessera team push')} to send your first report.\n")


# ── Entry point ─────────────────────────────────────────────────────────────

def cmd_team(args: list[str]) -> int:
    """Execute: tessera team [report|push|setup] [period]
