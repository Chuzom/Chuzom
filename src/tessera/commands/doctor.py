...
from tessera.terminal_style import Color
...
def _hook_version_num(path: Path) -> int:
    ...
    _re = re.compile(r"#\s*tessera-hook-version:\s*(\d+)")
    try:
...
def _run_doctor_host(host: str) -> None:
    ...
    for h in hosts_to_check:
        ...
        if h == "claude":
            # Check hooks
            from tessera.install_hooks import _HOOKS_DST, _HOOK_DEFS
            ...
            if mcp_json.exists():
                try:
                    ...
                    if "tessera" in data.get("servers", {}):
                        print(_ok(f"tessera registered in {mcp_json}"))
                    else:
                        print(
                            _fail(
                                f"tessera not in servers ({mcp_json})",
                                fix="tessera install --host vscode",
                            )
                        )
                        issues.append("tessera not registered in VS Code mcp.json")
            ...
            cursor_rules = Path.home() / ".cursor" / "rules" / "tessera.md"

            if mcp_json.exists():
                try:
                    data = json.loads(mcp_json.read_text())
                    if "tessera" in data.get("mcpServers", {}):
                        print(_ok(f"tessera registered in {mcp_json}"))
                    else:
                        print(
                            _fail(
                                f"tessera not in mcpServers ({mcp_json})",
                                fix="tessera install --host cursor",
                            )
                        )
                        issues.append("tessera not registered in Cursor mcp.json")
                except Exception as e:
                    print(_fail(f"could not parse {mcp_json}: {e}"))
            else:
                print(
...
def _run_doctor(host: Optional[str] = None) -> tuple[int, list[str]]:
    ...
    for src_name, dst_name, event, _ in _HOOK_DEFS:
        ...
        if dst.exists():
            ...
            from tessera.install_hooks import _HOOKS_SRC
            ...
            if src.exists():
                ...
                if src_v > dst_v:
                    ...
                    issues.append(
                        f"Hook {dst_name} is outdated — run `tessera install --force`"
                    )
                else:
                    print(_ok(f"{dst_name}  ({event})"))
            else:
                            ...
                            if os.path.exists(_interp):
                                ...
                                print(
                                    _fail(
                                        ...
                                        fix="tessera install --force",
                                    )
                                )
                                issues.append(
                                    f"Hook interpreter missing: {_interp} — "
                                    f"run `tessera install --force` to fix"
                                )
    ...
    if rules_dst.exists():
        print(_ok("tessera.md"))
    else:
        print(_fail("tessera.md — not installed", fix="tessera install"))
    ...
    if _SETTINGS_PATH.exists():
        try:
            ...
            pass
    registered_cc = "tessera" in settings.get("mcpServers", {})
    if registered_cc:
    ...
    if desktop_path is None:
        ...
        try:
            ...
            if "tessera" in cfg.get("mcpServers", {}):
            ...
            for _, dst_name, event, _ in _CLAW_CODE_HOOK_DEFS:
                ...
                if dst.exists():
                    ...
                    print(
                        _fail(
                            f"{dst_name}  — not installed",
                            fix="tessera install --claw-code",
                        )
                    )
                    issues.append(f"claw-code hook {dst_name} not installed")
            if "tessera" in cc_settings.get("mcpServers", {}):
                print(
    ...
    print(f"\n{_bold('  Version')}")
    try:
        from tessera import __version__ as project_version
        print(_ok(f"tessera {project_version}"))
    except Exception:
        try:
            from importlib.metadata import version
            v = version("tessera")
            print(_ok(f"tessera {v}"))
        except Exception:
...
def cmd_doctor(args: list[str]) -> int:
    """Execute: tessera doctor [--host claude|vscode|cursor|all]
