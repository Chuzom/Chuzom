...
def _run_install(flags: list[str]) -> None:
    ...
    if check_only:
        print(f"\n{_bold('[tessera] Install check')}  (no changes made)\n")
        ...
        if rules_dst.exists():
            print(_ok("tessera.md  (routing rules)"))
        else:
            print(_fail("tessera.md  (routing rules)  — not installed",
                        fix="tessera install"))
            all_ok = False
        ...
        if desktop_path is None:
            ...
            if desktop_path.exists():
                try:
                    ...
                    registered = "tessera" in cfg.get("mcpServers", {})
                except Exception:
                    pass
            if registered:
        ...
        if all_ok:
            ...
            print(_yellow("  Run `tessera install` to fix the issues above."))
        print()
        return
    ...
    if force:
        from tessera.install_hooks import _load_settings, _save_settings
        settings = _load_settings()
        settings.get("mcpServers", {}).pop("tessera", None)
    ...
    from tessera.install_hooks import install_claw_code, claw_code_settings_path
    ...
    print("    tessera doctor               — verify everything is wired up")
    print("    tessera status               — today's cost & savings")
    print("    tessera dashboard            — web dashboard (localhost:7337)")
    print("    tessera install --check      — preview install state")
    print("    tessera install --force      — reinstall / update paths")
    print("    tessera install --claw-code  — also install into claw-code")
    print("    tessera uninstall            — remove\n")
...
def _run_install_headless() -> None:
    ...
    """
    from tessera.install_hooks import install
  ...
  # Install tessera and wire in hooks
  RUN pip install claude-code-tessera && tessera install

  # Route to API providers — no Anthropic subscription in CI
  ENV TESSERA_CLAUDE_SUBSCRIPTION=false
...
"""
    ...
    print(_dim("  (tessera install does this automatically; shown for reference)"))
    import json as _json
    example = {
        "mcpServers": {"tessera": {"command": "tessera", "args": []}},
        "hooks": {
            "UserPromptSubmit": [{"matcher": "", "hooks": [{"type": "command", "command": "python3 ~/.claude/hooks/tessera-auto-route.py"}]}],
            "Stop":             [{"matcher": "", "hooks": [{"type": "command", "command": "python3 ~/.claude/hooks/tessera-session-end.py"}]}],
        }
    }
    ...
    print("    cat ~/.tessera/usage.json        # routing stats")
    print("    sqlite3 ~/.tessera/usage.db \\")
...
2. Copy routing rules so Codex knows when to call llm_auto:
   ...
   cp "$(python3 -c "import tessera; import pathlib; print(pathlib.Path(tessera.__file__).parent / 'rules' / 'codex-rules.md')")" \\
...
3. Enable MCP in VS Code settings (Copilot > MCP: Enable).
   Restart VS Code. Run @tessera llm_savings to verify.
...
def _install_codex_files() -> list[str]:
    ...
    if "tessera" not in existing:
        with config_yaml.open("a") as f:
            f.write(mcp_block)
        actions.append(f"✓ Added tessera MCP server to {config_yaml}")
    else:
        actions.append(f"  tessera already in {config_yaml} (skipped)")

    # 2. PostToolUse hook in ~/.codex/hooks.json
    hooks_json = codex_dir / "hooks.json"
    hook_script = home / ".tessera" / "hooks" / "codex-post-tool.py"

    # Copy the hook script to ~/.tessera/hooks/
    ...
    if rules_src.exists():
        ...
        if instructions.exists():
            ...
            if "tessera" not in existing_inst:
...
def _merge_json_mcp_block(
    ...
    if server_name not in servers:
        ...
        actions.append(f"✓ Added tessera MCP server to {config_path}")
    else:
        actions.append(f"  tessera already in {config_path} (skipped)")
    return actions
...
def _install_opencode_files() -> list[str]:
    ...
    # 1. MCP server entry
    server_entry = {"command": "uvx", "args": ["claude-code-tessera"]}
    actions += _merge_json_mcp_block(opencode_dir / "config.json", "tessera", server_entry)

    # 2. Hook script
    hook_dest, hook_actions = _copy_hook_script(
        "opencode-post-tool.py", home / ".tessera" / "hooks"
    )
...
def _install_gemini_cli_files() -> list[str]:
    ...
    server_entry = {"command": "uvx", "args": ["claude-code-tessera"]}
    actions += _merge_json_mcp_block(gemini_dir / "settings.json", "tessera", server_entry)

    # 2. Extension manifest + hooks directory
    ext_dir = gemini_dir / "extensions" / "tessera"
    ...
    if not ext_manifest.exists():
        manifest = {
            ...
            "mcpServers": {"tessera": server_entry},
        }
    ...
    post_hook_dest, hook_actions = _copy_hook_script(
        "gemini-cli-post-tool.py", home / ".tessera" / "hooks"
    )
    actions += hook_actions

    auto_hook_dest, hook_actions = _copy_hook_script(
        "gemini-cli-auto-route.py", home / ".tessera" / "hooks"
    )
    actions += hook_actions

    status_hook_dest, hook_actions = _copy_hook_script(
        "status-bar.py", home / ".tessera" / "hooks"
    )
    actions += hook_actions

    end_hook_dest, hook_actions = _copy_hook_script(
        "gemini-cli-session-end.py", home / ".tessera" / "hooks"
    )
    actions += hook_actions

    start_hook_dest, hook_actions = _copy_hook_script(
        "session-start.py", home / ".tessera" / "hooks"
    )
    actions += hook_actions
...
def _install_copilot_cli_files() -> list[str]:
    ...
    server_entry = {"command": "uvx", "args": ["claude-code-tessera"]}
    actions += _merge_json_mcp_block(copilot_dir / "mcp.json", "tessera", server_entry)
...
def _install_openclaw_files() -> list[str]:
    ...
    server_entry = {"command": "uvx", "args": ["claude-code-tessera"]}
    actions += _merge_json_mcp_block(openclaw_dir / "mcp.json", "tessera", server_entry)
...
def _install_trae_files() -> list[str]:
    ...
    server_entry = {"command": "uvx", "args": ["claude-code-tessera"]}
    actions += _merge_json_mcp_block(trae_dir / "mcp.json", "tessera", server_entry)
...
def _install_factory_files() -> list[str]:
    ...
    if plugin_dir.exists():
        ...
        actions.append("  Install via: factory plugin install ypollak2/tessera")
    else:
        actions.append("  .factory-plugin/ not found in repo root — run from the tessera repo dir")
    actions.append("  Or install .claude-plugin/ directly: factory plugin install ypollak2/tessera")
    return actions
...
def _install_vscode_files() -> list[str]:
    ...
    server_entry = {"command": "uvx", "args": ["claude-code-tessera"]}
    actions += _merge_json_mcp_block(mcp_json, "tessera", server_entry, root_key="servers")
...
def _install_cursor_files() -> list[str]:
    ...
    server_entry = {"command": "uvx", "args": ["claude-code-tessera"]}
    actions += _merge_json_mcp_block(mcp_json, "tessera", server_entry, root_key="mcpServers")

    # Append routing rules to ~/.cursor/rules/tessera.md
...
def _install_host(host: str) -> None:
    ...
    print(f"\n{bold}tessera install --host {host}{reset}\n")
    print("─" * min(w, 70))
    ...
    _FILE_WRITERS = {
        ...
        "factory":    (_install_factory_files,      "Run: factory plugin install ypollak2/tessera"),
    ...
    print(
        f"\nFor Claude Code (hooks + full cost-routing): {bold}tessera install{reset}\n"
