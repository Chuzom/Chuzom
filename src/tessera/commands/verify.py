...
Example output:
  ✅ Configuration loaded from ~/.tessera/config.yaml
  ✅ SQLite database: ~/.tessera/usage.db (45 MB, last write 5 mins ago)
...
def check_configuration() -> tuple[bool, str]:
    ...
    Returns:
        (success, message)
    """
    config_path = Path.home() / ".tessera" / "config.yaml"
...
def check_hooks() -> tuple[bool, list[str]]:
    ...
    hook_names = [
        "tessera-auto-route.py",
        "tessera-session-end.py",
        "tessera-enforce-route.py",
    ]
...
def main(args: Optional[list[str]] = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Health check for tessera"
    )
    parser.parse_args(args or [])

    print()
    print(Color.ORCHESTRATE_BLUE("=" * 70))
    print(Color.ORCHESTRATE_BLUE("              tessera health check"))
    print(Color.ORCHESTRATE_BLUE("=" * 70))
    print()
