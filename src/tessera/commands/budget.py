...
def _run_budget(subcmd: str, flags: list[str]) -> None:
    """tessera budget [list|set|remove] ...
    ...
    """
    import asyncio
    from tessera.budget_store import list_caps, remove_cap, set_cap
    from tessera.budget import get_all_budget_states, invalidate_cache
    from tessera.types import LOCAL_PROVIDERS

    if subcmd == "set":
        if len(flags) < 2:
            print(_red("Usage: tessera budget set <provider> <amount>"))
            sys.exit(1)
        ...
        print(f"  {_dim('Saved to ~/.tessera/budgets.json')}")
        return

    if subcmd == "remove":
        if not flags:
            print(_red("Usage: tessera budget remove <provider>"))
            sys.exit(1)
        provider = flags[0]
    ...
    if uncapped:
        ...
        print(_dim("     Set one: tessera budget set <provider> <amount>"))

    stored = caps
    if stored:
        print()
        print(_dim("  Caps stored in ~/.tessera/budgets.json"))
    print()
...
def cmd_budget(args: list[str]) -> int:
    """Execute: tessera budget [list|set|remove]
