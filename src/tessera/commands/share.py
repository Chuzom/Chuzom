...
def _run_share() -> None:
    ...
    card_lines = [
        f"┌{border}┐",
        _box_line(""),
        _box_line(f"  🤖 tessera saved me ${total_saved:.2f} (lifetime)"),
        ...
        _box_line(""),
        _box_line("  ⭐ github.com/ypollak2/tessera"),
        _box_line(""),
        f"└{border}┘",
    ]
    ...
    plain = (
        f"🤖 tessera saved me ${total_saved:.2f} (lifetime)\n"
        ...
        f"⭐ github.com/ypollak2/tessera"
    )
    _copy_to_clipboard(plain)

    # ── Twitter/X intent URL ──────────────────────────────────────────
    tweet = (
        f"🤖 tessera saved me ${total_saved:.2f} so far "
        f"({savings_pct}% cheaper than always-Sonnet)\n\n"
