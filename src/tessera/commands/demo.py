...
def _run_demo() -> None:
    ...
    cc_mode = os.getenv("TESSERA_CLAUDE_SUBSCRIPTION", "").lower() in ("true", "1", "yes")
    gemini_mode = os.getenv("TESSERA_GEMINI_SUBSCRIPTION", "").lower() in ("true", "1", "yes")
    ...
    if not using_real:
        ...
        print(f"  {_dim('Source: ~/.tessera/usage.db  (your actual routing decisions)')}\n")
    ...
    if not using_real:
        print(f"\n  {_yellow('Next steps:')}")
        print(f"    {_yellow('→')} Run {_bold('tessera install')} to enable automatic routing")
        if not cc_mode:
            print(f"    {_yellow('→')} Set {_bold('TESSERA_CLAUDE_SUBSCRIPTION=true')} to use subscription models")
    else:
        print(f"\n  {_yellow('Your routing history:')}")
    print(f"  {_yellow('→')} Check savings: {_bold('tessera gain')}")
    print(f"  {_yellow('→')} View dashboard: {_bold('tessera dashboard')}\n")
