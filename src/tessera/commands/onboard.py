...
def _run_onboard() -> None:
    ...
    # ── 6. Write config to ~/.tessera/.env ────────────────────────────────
    env_path = os.path.join(STATE_DIR, ".env")
    os.makedirs(STATE_DIR, exist_ok=True)
    env_lines = [
        f"TESSERA_ENFORCE={enforce}",
        f"TESSERA_PROFILE={profile}",
    ]
    try:
    ...
    print("    • Run `tessera status` to see savings accumulate")
    if enforce == "shadow":
        print("    • Upgrade to suggest/enforce when ready: tessera onboard")
    print()
