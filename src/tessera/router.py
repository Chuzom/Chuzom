...
This module is the central dispatch layer of tessera. It receives a
...
from tessera import cost, media, providers
from tessera.budget import get_budget_state, reserve_tokens, release_tokens
from tessera.state import get_active_agent
from tessera.codex_agent import CODEX_MODELS, is_codex_available, run_codex
from tessera.contract import build_contract
from tessera.gates import run_gates
from tessera.gemini_cli_agent import GEMINI_MODELS, is_gemini_cli_available, run_gemini_cli
from tessera.logging import get_logger
from tessera.compaction import compact_structural
from tessera.config import get_config
from tessera.repo_config import effective_config as get_repo_config
from tessera.context import build_context_messages, get_session_buffer
from tessera.health import get_tracker
from tessera.profiles import get_model_chain, provider_from_model
from tessera.receipt_store import compute_receipt, store_receipt
from tessera.tracing import set_span_attributes, traced_span
from tessera.types import BudgetExceededError, Complexity, LLMResponse, RoutingProfile, TaskType
...
async def _build_and_filter_chain(
    ...
    if model_override:
        ...
        if (
            (config.tessera_claude_subscription and model_override.startswith("anthropic/"))
            or (config.tessera_gemini_subscription and model_override.startswith("gemini/"))
        ):
            log.warning(
    ...
    try:
        from tessera.dynamic_routing import get_dynamic_model_chain
    ...
    if models_to_try is None:
        models_to_try = get_model_chain(
            ...
            is_subscription_mode=config.tessera_claude_subscription or config.tessera_gemini_subscription,
        )

    if task_type not in MEDIA_TASK_TYPES:
        from tessera.claude_usage import get_claude_pressure
        ...
        # ── Policy engine ─────────────────────────────────────────────────────
        from tessera.policy import OrgPolicy, apply_policy, load_org_policy
        ...
        if active_agent is None:
            if config.tessera_claude_subscription:
                active_agent = "claude_code"
            elif config.tessera_gemini_subscription:
        ...
        if profile == RoutingProfile.QUOTA_BALANCED:
            try:
                from tessera.quota_balance import (
...
def _resolve_profile(
    ...
    Args:
        ...
        config: Application config (provides tessera_profile default).
    ...
    if profile is None and not model_override:
        ...
        if c == Complexity.DEEP_REASONING:
            use_thinking = True
        resolved = _COMPLEXITY_TO_PROFILE.get(c, config.tessera_profile)
    else:
        resolved = profile or config.tessera_profile
...
def _auth_error_hint(provider: str) -> str:
    ...
    if env_var:
        return (
            ...
            f"    Fix: run `tessera setup` to configure it, or set {env_var} in "
            f"~/.tessera/.env\n"
    ...
    return (
        ...
        f"    Fix: run `tessera setup` to configure your providers.\n"
        f"    Note: Claude Code subscription covers Haiku/Sonnet/Opus — no API key needed "
...
async def _dispatch_model_loop(
    ...
    for attempt, model in enumerate(models_to_try, start=1):
        ...
        try:
            from tessera.quality_feedback import should_skip_model
        ...
        try:
            ...
            if provider == "codex":
                try:
                    from tessera.quota_balance import record_codex_request
                    record_codex_request(config.codex_daily_limit)
                except Exception as _quota_err:
                    log.debug("Failed to record Codex request: %s", _quota_err)
            elif provider == "gemini_cli":
                try:
                    from tessera.gemini_cli_quota import record_gemini_request
            ...
            _raw_limit = getattr(config, "tessera_daily_spend_limit", 0.0)
            ...
            if task_type not in MEDIA_TASK_TYPES:
                try:
                    from tessera import semantic_cache
    ...
    setup_hint = (
        " Run `tessera setup` to configure provider API keys, or "
        "`tessera doctor` to diagnose all issues."
...
async def route_and_call(
    ...
    *,
    ...
    with traced_span(
        ...
        async with _budget_lock:
            # Guard: tessera_daily_spend_limit may be MagicMock in tests
            _raw_daily = getattr(config, "tessera_daily_spend_limit", 0.0)
            ...
            if _daily_limit > 0:
                ...
                if daily_spend + _pending_spend >= _daily_limit:
                    raise BudgetExceededError(
                        ...
                        "To raise the limit: set TESSERA_DAILY_SPEND_LIMIT env var."
                    )

            # Per-task daily cap enforcement (from org policy)
            from tessera.policy import get_task_cap, load_org_policy
            ...
            if task_cap and task_cap > 0:
                ...
                if task_daily_spend + _pending_spend >= task_cap:
                    raise BudgetExceededError(
                        ...
                        f"To raise the limit: update ~/.tessera/org-policy.yaml task_caps."
                    )
            ...
            if config.tessera_monthly_budget > 0:
                ...
                if monthly_spend + _pending_spend >= budget:
                    raise BudgetExceededError(
                        ...
                        "To raise the limit: set TESSERA_MONTHLY_BUDGET env var."
                    )
            ...
            try:
                from tessera.session_spend import _estimate_cost as _est_fn
        ...
        #
        # v10.0.0 migration knob: ``TESSERA_BANDIT=off`` skips the reorder
        ...
        _bandit_off = os.environ.get("TESSERA_BANDIT", "on").lower() in {"off", "0", "false", "no"}
        if models_to_try and not model_override and not _bandit_off:
            try:
                from tessera.bandit import EpsilonGreedyBandit
        ...
        _escalate_above = getattr(config_for_escalation, "tessera_escalate_above", 0.0)
        if isinstance(_escalate_above, (int, float)) and _escalate_above > 0 and task_type not in MEDIA_TASK_TYPES:
            try:
                ...
                if _estimated > config_for_escalation.tessera_escalate_above:
                    from tessera.tools.admin import _set_pending_approval
                    _set_pending_approval({"model": _top_model, "estimated_cost": _estimated})
                    raise BudgetExceededError(
                        f"Call to {_top_model} (estimated ${_estimated:.4f}) exceeds "
                        f"TESSERA_ESCALATE_ABOVE=${config_for_escalation.tessera_escalate_above:.2f}. "
                ...
                if (config_for_escalation.tessera_hard_stop_above > 0
                        and _session_total >= config_for_escalation.tessera_hard_stop_above):
                    raise BudgetExceededError(
                        f"Session spend ${_session_total:.4f} has reached the hard stop limit "
                        f"(TESSERA_HARD_STOP_ABOVE=${config_for_escalation.tessera_hard_stop_above:.2f}). "
                        f"No further calls will be made this session. "
                        f"Unset TESSERA_HARD_STOP_ABOVE to continue."
                    )
        ...
        if system_prompt is None and task_type not in MEDIA_TASK_TYPES and models_to_try:
            try:
                from tessera.context_prep import prepare_prompt as _prepare
                _prepared = _prepare(
                    user_prompt=prompt,
                    task_type=task_type,
                    complexity=c,
