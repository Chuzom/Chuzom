...
level. This module provides an alternative: ~/.tessera/config.yaml in the
...
Priority order for configuration:
  ...
  2. ~/.tessera/config.yaml (user-level fallback)
...
def safe_config_path() -> Path:
    ...
    Located in ~/.tessera/ to avoid project-level security restrictions.
    """
    return Path.home() / ".tessera" / "config.yaml"


def load_safe_config() -> dict[str, Any]:
    """Load configuration from ~/.tessera/config.yaml if it exists.
...
def write_safe_config_template(discovered: dict[str, Any]) -> None:
    ...
    Used by `tessera init-claude-memory` to generate a starting point.
...
# Place this at: ~/.tessera/config.yaml
# Permissions: chmod 600 ~/.tessera/config.yaml (readable by you only)
#
...
# Router settings
tessera_profile: "{discovered.get('tessera_profile', 'balanced')}"
tessera_claude_subscription: {str(discovered.get('tessera_claude_subscription', False)).lower()}

# (More options available — see tessera/src/tessera/config.py for full list)
"""
