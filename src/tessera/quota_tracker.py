...
class QuotaTracker:
    ...
    - USAGE_JSON: Path to cached usage.json (default ~/.tessera/usage.json)
    ...
    TTL_SECONDS: int = int(os.environ.get("TESSERA_QUOTA_TTL", "300"))
    MAX_RETRY: int = int(os.environ.get("TESSERA_QUOTA_RETRY", "3"))
    RETRY_DELAY_SEC: float = float(os.environ.get("TESSERA_QUOTA_DELAY", "2.0"))
    USAGE_JSON: Path = Path.home() / ".tessera" / "usage.json"
    ...
    async def _fetch_claude_oauth_data(self) -> tuple[float, float, float]:
        ...
        from tessera.claude_usage import refresh_claude_usage
