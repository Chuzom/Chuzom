...
from tessera.types import ClassificationResult
...
class PolicyManager:
    ...
    DEFAULT_POLICY_DIR = Path.home() / ".tessera" / "policies"
    ...
    def load_policy(self, name: str) -> RoutingPolicy:
        ...
        Searches in order:
        1. User custom policies (~/.tessera/policies/)
        2. Preset policies (bundled with tessera)
    ...
    def get_active_policy(self) -> RoutingPolicy:
        ...
        Loads from TESSERA_POLICY env var, or returns default (balanced).
        ...
        policy_name = os.environ.get("TESSERA_POLICY", "balanced")
        try:
...
def load_org_policy(path: Path | None = None) -> OrgPolicy | None:
    ...
    Args:
        path: Path to org policy YAML file. Defaults to ~/.tessera/org-policy.yaml

    Returns:
        OrgPolicy if file exists, else default permissive policy
    """
    if path is None:
        path = Path.home() / ".tessera" / "org-policy.yaml"
