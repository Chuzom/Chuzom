...
from tessera.policy import RoutingPolicy, PolicyManager, get_policy_manager
...
def _select_preset_policy(pm: PolicyManager) -> None:
    ...
    os.environ["TESSERA_POLICY"] = choice
    
    print(f"\n✅ Selected policy: {choice}")
    print(f"   Set TESSERA_POLICY={choice} in your environment")
...
def _create_custom_policy(pm: PolicyManager) -> None:
    ...
    os.environ["TESSERA_POLICY"] = name
    ...
    print("\n💡 To use this policy:")
    print(f"   export TESSERA_POLICY={name}")
    print("   Or set in ~/.env or ~/.tessera/config.yaml")
