...
Env vars:
  TESSERA_BASH_COMPRESS       Set to "off" to disable this hook
  TESSERA_DB_PATH             SQLite path for compression stats (default: ~/.tessera/usage.db)
"""
...
try:
    from tessera.compression.rtk_adapter import RTKAdapter
except ImportError:
    RTKAdapter = None
...
def main() -> None:
    ...
    if os.environ.get("TESSERA_BASH_COMPRESS", "").lower() == "off":
        sys.exit(0)
