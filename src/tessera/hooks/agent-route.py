...
def _get_max_depth() -> int:
    """Read TESSERA_MAX_AGENT_DEPTH from environment, default 3."""
    try:
        return int(os.environ.get("TESSERA_MAX_AGENT_DEPTH", "3"))
    except (ValueError, TypeError):
        return 3


def _get_session_id() -> str:
    """Read current session ID from ~/.tessera/session_id.txt."""
    session_file = Path.home() / ".tessera" / "session_id.txt"
    try:
...
def _read_agent_depth(session_id: str) -> int:
    ...
    """
    depth_file = Path.home() / ".tessera" / "agent_depth.json"
    try:
...
def _log_agent_call(subagent_type: str, prompt: str, decision: str) -> None:
    ...
    Persists to ~/.tessera/agent_calls.json with a rolling history of last 50 calls.
    Used by PostToolUse[Agent] hook to suggest fallbacks when agents fail.
    """
    calls_file = Path.home() / ".tessera" / "agent_calls.json"
...
def _initialize_session_budget() -> float:
    ...
    Creates ~/.tessera/session_budget.json with initial budget based on
    quota pressure. Called once per session to set up provisional tracking.

    Returns the initial budget in USD.
    """
    budget_file = Path.home() / ".tessera" / "session_budget.json"
...
def _get_remaining_budget() -> float:
    ...
    Priority:
      1. ~/.tessera/session_budget.json (provisional tracking)
...
def _get_claude_pressure() -> float:
    ...
    Priority:
      1. ~/.tessera/usage.json  — written by llm_update_usage, fastest
      2. ~/.tessera/usage.db    — SQLite claude_usage table, authoritative
...
def main() -> None:
    ...
    if current_depth >= max_depth:
        result = {
            "decision": "block",
            "reason": (
                f"[tessera] Agent loop circuit breaker: depth {current_depth}/{max_depth}. "
    ...
    if estimated_cost > remaining_budget:
        result = {
            "decision": "block",
            "reason": (
                f"[tessera] Agent would exceed session budget.\n\n"
    ...
    if estimated_cost > AGENT_MAX_COST_USD:
        result = {
            "decision": "block",
            "reason": (
                f"[tessera] Agent estimated cost exceeds per-agent limit.\n\n"
    ...
    _usage_path = Path.home() / ".tessera" / "usage.json"
    try:
