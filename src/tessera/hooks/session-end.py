...
def _query_cumulative_savings() -> list[tuple[str, int, int, int, float]]:
    ...
    v10.1.6: delegates to ``tessera.dashboard_data`` so the UNION logic
...
def _query_daily_14d() -> list[tuple[str, int, int, float]]:
    ...
    v10.1.6: delegates to ``tessera.dashboard_data.query_daily``. The
...
def _format(tools: dict[str, dict], cc_rows: list[dict], free_rows: list[dict],
    ...
    try:
        from tessera.hooks.dashboard_enhanced import (
...
def _build_and_save_learned_profile() -> None:
    ...
    This is called at session-end to update ~/.tessera/learned_routes.json
...
def main() -> None:
    ...
    summary = None
    try:
        from tessera.hooks.cyber_grid import render_cyber_grid
    ...
    if not summary:
        if not tools and not cc_rows and not current and not free_rows and not has_cumulative:
            ...
            try:
                from tessera.dashboard.server import _get_or_create_token, DEFAULT_PORT
    ...
    try:
        from tessera.monitoring.periodic import load_session_snapshots, analyze_session_trends, format_trend_summary
    ...
    try:
        from tessera.auto_profile import should_rescan, rescan_and_update
        if should_rescan():
    ...
    try:
        import asyncio
        from tessera.model_evaluator import EVAL_CACHE_PATH, EVAL_TTL_SECONDS
        ...
        if should_eval:
            from tessera.model_evaluator import evaluate_available_models
            try:
