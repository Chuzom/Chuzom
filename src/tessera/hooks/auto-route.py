...
try:
    from tessera.statusline_hud import initialize_hud
except ImportError:
    def initialize_hud():
...
try:
    from tessera.model_tracking import log_routing_decision
except ImportError:
...
# ── .env loader (reads tessera's .env for API keys) ──────────────────────

_ENV_PATHS = [
    Path.cwd() / ".env",  # CWD .env (hook runs from project root)
    Path(__file__).resolve().parent.parent.parent.parent / ".env",  # dev: src/tessera/hooks → project root
    Path.home() / ".tessera" / ".env",  # user-level config
    Path.home() / ".env",
]
...
OLLAMA_MODEL = os.environ.get("TESSERA_OLLAMA_MODEL", "gemma4:latest")
OLLAMA_URL = os.environ.get("TESSERA_OLLAMA_URL", "http://localhost:11434")
OLLAMA_TIMEOUT = int(os.environ.get("TESSERA_OLLAMA_TIMEOUT", "15"))
CONFIDENCE_THRESHOLD = int(os.environ.get("TESSERA_CONFIDENCE_THRESHOLD", "2"))  # v7.5.0: Aggressive routing — route more with lower threshold
# Privacy-first: classify locally only (heuristic + Ollama) by default.
# Set TESSERA_CLASSIFY_LOCAL_ONLY=false to enable external classifiers.
_local_only_raw = os.environ.get(
    "TESSERA_CLASSIFY_LOCAL_ONLY",
    os.environ.get("TESSERA_DISABLE_LLM_CLASSIFIERS", "true"),
).lower()
...
def _get_active_policy():
    ...
    try:
        ...
        from tessera.policy import get_active_policy as get_policy
...
_CC_MODE = os.environ.get("TESSERA_CLAUDE_SUBSCRIPTION", "").lower() in ("true", "1", "yes")
...
def _get_pressure() -> dict[str, float]:
    ...
    """
    usage_path = Path.home() / ".tessera" / "usage.json"
    ttl_seconds = int(os.environ.get("TESSERA_QUOTA_TTL", "300"))
    ...
    db_path = Path.home() / ".tessera" / "usage.db"
    try:
        import sqlite3
...
_USAGE_JSON = str(Path.home() / ".tessera" / "usage.json")
...
def _fetch_usage_inline() -> dict | None:
    ...
    try:
        ...
        state_dir = str(Path.home() / ".tessera")
...
# Written to ~/.tessera/session_{id}.json when Claude's first tool call in
...
# ── MCP Capability Map ───────────────────────────────────────────────────────
#
# Known non-tessera MCP servers and the intent patterns that match them.
...
def _build_mcp_capability_map(tools: list[dict]) -> dict[str, list[str]]:
    ...
    Only non-tessera MCP servers are included — llm_* tools are handled
...
def _zero_claude_enabled() -> bool:
    ...
    env_value = os.environ.get("TESSERA_ZERO_CLAUDE", "").strip().lower()
    if env_value:
...
def _load_learned_routes() -> dict[str, dict]:
    """Load learned routing overrides from ~/.tessera/learned_routes.json.
    ...
    """
    try:
        learned_path = Path.home() / ".tessera" / "learned_routes.json"
...
_DEBUG_LOG = Path.home() / ".tessera" / "auto-route-debug.log"
...
def main() -> None:
    ...
    # Kill-switch: TESSERA_DISABLE_CONTINUATION_BYPASS=1 forces every turn
    # through the classifier (useful when the heuristic regresses).
    if not zero_claude and session_id and _is_continuation(prompt):
        if os.environ.get("TESSERA_DISABLE_CONTINUATION_BYPASS", "").lower() in ("1", "true", "yes", "on"):
    ...
    # If the prompt clearly targets an available non-tessera MCP server
    ...
    # Priority: env var > .tessera.yml repo config > ~/.tessera/.env > "hard"
    ...
    _enforce_mode = os.environ.get("TESSERA_ENFORCE", "").lower()
    if not _enforce_mode:
        # Try reading from .tessera.yml in cwd or ancestor
        try:
            import yaml as _yaml
            _repo_yml = Path.cwd() / ".tessera.yml"
    ...
    try:
        ...
        _old_level = logging.getLogger("tessera.model_tracking").level
        logging.getLogger("tessera.model_tracking").setLevel(logging.CRITICAL)
        ...
        try:
            ...
            logging.getLogger("tessera.model_tracking").setLevel(_old_level)
    except Exception:
    ...
    if _CC_MODE and session_id:
        try:
            ...
            session_spend_path = os.path.expanduser("~/.tessera/session_spend.json")
            prompt_sequence = 0
            ...
            db_path = os.path.expanduser("~/.tessera/usage.db")
    ...
    _direct_enabled = os.environ.get("TESSERA_DIRECT_EXECUTION", "true").lower() in ("1", "true", "yes", "on")
    ...
    if _direct_enabled and _enforce_mode not in ("shadow", "off"):
        try:
            ...
            if _needs_claude_tools(prompt, task_type):
                ...
                from tessera.hooks.direct_executor import execute_agent as _execute_agent
            ...
            if _direct_result:
                ...
                from tessera.hooks.response_formatter import (
    ...
    if _enforce_mode == "shadow":
        ...
        directive = (
            ...
            f"\n"
            f"   Violations are logged and escalated. See ~/.tessera/enforcement.log"
        )
        indicator = f"✨ {task_type}/{complexity} ✨ {tool} → 🧠 {selected_model}"
        write_pending = True
    ...
    trend_indicator = ""
    try:
        from tessera.monitoring.live_tracker import get_live_trend_indicator
        trend_indicator = get_live_trend_indicator()
        
        # Attempt to capture hourly snapshot (async, fire-and-forget)
        try:
            from tessera.monitoring.live_tracker import check_and_capture_hourly_snapshot
            import threading
