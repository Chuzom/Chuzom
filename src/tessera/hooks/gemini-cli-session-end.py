...
Usage: Installed at ~/.tessera/hooks/gemini-cli-session-end.py by `tessera install`.
...
async def get_session_summary() -> dict:
    ...
    try:
        import os
        start_file = os.path.expanduser("~/.tessera/session_start.txt")
    ...
    try:
        from tessera.gemini_cli_quota import get_gemini_quota_status
    ...
    try:
        ...
        session_spend = 0.0
        if session_start > 0:
            from tessera.cost import get_total_spend_since
        ...
        from tessera.cost import get_provider_spend_breakdown
