...
This hook fires before every tool call and:
  ...
  7. If the tool IS in the task-specific blocklist → enforce based on TESSERA_ENFORCE:
...
Enforcement modes:
  ...
  hard            — Bash/Edit/Write are blocked for ALL task types until an
                    ...
                    Set: export TESSERA_ENFORCE=hard
  off             — Enforcement completely disabled. No pending state is checked.

Compliance log: ~/.tessera/enforcement.log
Pending state:  ~/.tessera/pending_route_{session_id}.json

Environment variables:
  TESSERA_ENFORCE  smart | soft | hard | off   (default: smart)
"""
...
# Written to ~/.tessera/session_{id}.json when Claude's first file edit in
...
def _read_pressure() -> dict[str, float]:
    """Read subscription pressure from ~/.tessera/usage.json.

    Returns: Dict with 'sonnet' and 'weekly' keys as fractions 0.0–1.0.
    """
    try:
        data = json.loads((Path.home() / ".tessera" / "usage.json").read_text())
...
def main() -> None:
    ...
    enforce = os.environ.get("TESSERA_ENFORCE", "").lower()
    yaml_enforce = ""
    try:
        ...
        pass
    if not enforce:
        # Fall back to ~/.tessera/routing.yaml so users who set
        ...
        try:
            _log_msg = (
                f"[{time.strftime('%H:%M:%S')}] WARNING: TESSERA_ENFORCE={enforce} "
                f"(env var) overrides routing.yaml enforce={yaml_enforce}. "
                f"Remove from ~/.zshrc or unset TESSERA_ENFORCE to use routing.yaml value.\n"
            )
    ...
    session_budget_limit = float(os.environ.get("TESSERA_SESSION_BUDGET", "5.00"))
    ...
    try:
        ...
        if session_budget_limit > 0 and session_spend > session_budget_limit:
            ...
            if tool_name not in {"Read", "Edit", "Write", "MultiEdit", "Glob", "Grep", "LS", "Bash"}:
                block_reason = (
                    f"[tessera] SESSION BUDGET EXCEEDED\n\n"
                    ...
                    f"  2. Or unset TESSERA_SESSION_BUDGET to disable the limit"
                )
    ...
    # ("mcp__tessera__llm_query") — accept both forms.
    ...
    block_reason = (
        f"[tessera] Routing directive BLOCKED.{escalation}{loop_warning}{window_warning}\n\n"
        ...
        f"  • Soft-fail for testing: export TESSERA_ENFORCE=soft\n"
        f"  • Disable entirely: export TESSERA_ENFORCE=off"
    )
    ...
    if violation_count >= 3:
        nudge = (
            f"\n[tessera] ⚠️  ESCALATION: {violation_count} routing violations this session.\n"
            f"  Next prompt expecting {expected_tool}:\n"
            f"  → Call the MCP tool FIRST before any Bash/Read/Edit/Write.\n"
            f"  → See ~/.tessera/enforcement.log for full history.\n"
            f"  → Set TESSERA_ENFORCE=hard to block violations automatically.\n"
        )
