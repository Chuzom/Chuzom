...
which tessera MCP tool to call.

Usage: Installed at ~/.tessera/hooks/gemini-cli-auto-route.py by `tessera install`.
...
async def classify_complexity(prompt: str) -> tuple[str, float]:
    ...
    try:
        from tessera.classifier import classify_prompt_heuristic
