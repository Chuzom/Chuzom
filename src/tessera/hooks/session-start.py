...
Mode detection (auto):
  TESSERA_CLAUDE_SUBSCRIPTION=true → subscription mode (OAuth pressure cascade)
...
# Import timeout config from tessera package if available
try:
    from tessera.timeout_config import subprocess_timeout, http_timeout
except ImportError:
    # Fallback to hardcoded defaults if tessera not installed
    def subprocess_timeout() -> int:
        return int(os.environ.get("TESSERA_SUBPROCESS_TIMEOUT", "15"))
    def http_timeout() -> int:
        return int(os.environ.get("TESSERA_HTTP_TIMEOUT", "10"))

STATE_DIR              = os.path.expanduser("~/.tessera")
...
# Load .env so TESSERA_CLAUDE_SUBSCRIPTION and other settings are available.
_ENV_PATHS = [
...
BANNER_ZERO_CLAUDE = """
╔════════════════════════════════════════════════════════════════╗
║  ⚡ tessera ACTIVE — strict zero-Claude routing             ║
╠════════════════════════════════════════════════════════════════╣
...
BANNER_SUBSCRIPTION = """
╔════════════════════════════════════════════════════════════════╗
║  ⚡ tessera ACTIVE — subscription mode (MCP-tool routing)  ║
╠════════════════════════════════════════════════════════════════╣
...
BANNER_API_KEYS = """
╔════════════════════════════════════════════════════════════════╗
║  ⚡ tessera ACTIVE — API-key routing in effect             ║
╠════════════════════════════════════════════════════════════════╣
...
def _refresh_claude_usage() -> str:
    ...
    On success: writes to ~/.tessera/usage.json and session_start_cc_pct.json
    ...
    sys.stderr.write(
        "[tessera] ⚠ Quota refresh failed (3 attempts)\n"
        "[tessera]   Using conservative 50% pressure defaults\n"
    )
...
def _preflight_check() -> str:
    ...
    # Enforce-route mode
    enforce = os.environ.get("TESSERA_ENFORCE", "smart")
    if enforce == "hard":
        issues.append("TESSERA_ENFORCE=hard (may block tools — use smart or off to debug)")
    elif enforce == "off":
...
def _format_learned_memory() -> str:
    ...
    Loads ~/.tessera/learned_routes.json and formats as:
    【ROUTING MEMORY】
...
def _maybe_refresh_benchmarks_bg() -> None:
    ...
    Only fires when ``~/.tessera/benchmarks.json`` is missing or older than
    ``TESSERA_BENCHMARK_TTL_DAYS`` (default 7 days).
    """
    benchmarks_path = os.path.join(STATE_DIR, "benchmarks.json")
    ttl_days = int(os.environ.get("TESSERA_BENCHMARK_TTL_DAYS", "7"))
    ...
    script = (
        "from tessera.benchmark_fetcher import generate_benchmarks_json; "
...
def main() -> None:
    ...
    # subscription mode regardless of TESSERA_CLAUDE_SUBSCRIPTION env var.
