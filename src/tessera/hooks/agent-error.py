...
def _get_fallback_suggestion(last_call: dict | None, failure_type: str) -> str | None:
    ...
    if _is_retrieval_intent(prompt):
        return (
            "[tessera] Agent failed to retrieve file/code data. "
    ...
    if failure_type == "resource_limit":
        ...
        return (
            "[tessera] Agent hit a resource limit (timeout, quota, etc.). "
            "Try one of these:\n\n"
    ...
    if failure_type == "unparseable_output":
        return (
            "[tessera] Agent output couldn't be parsed (likely agent bug). "
    ...
    if failure_type == "resource_crash":
        return (
            "[tessera] Agent crashed (memory/process killed). "
    ...
    if task_type == "code":
        return (
            "[tessera] Agent failed. For code tasks, try llm_code MCP tool:\n\n"
            "  llm_code(prompt=\"...\")\n\n"
            "This routes to the best code model and handles scope better."
        )
    elif task_type == "analysis":
        return (
            "[tessera] Agent failed. For analysis tasks, try llm_analyze MCP tool:\n\n"
            "  llm_analyze(prompt=\"...\")\n\n"
            "This provides stronger reasoning for complex tasks."
        )
    elif task_type == "generate":
        return (
            "[tessera] Agent failed. For writing/generation, try llm_generate MCP tool:\n\n"
            "  llm_generate(prompt=\"...\")\n\n"
        ...
        return (
            "[tessera] Agent failed. Try one of these MCP tools:\n\n"
            "  • llm_query: For simple questions\n"
            "  • llm_analyze: For analysis\n"
            "  • llm_code: For code work\n"
