...
from tessera.hooks.direct_executor import ModelSpec
...
def _ollama_models() -> list[ModelSpec]:
    ...
    models_str = os.environ.get("OLLAMA_BUDGET_MODELS",
                                os.environ.get("TESSERA_OLLAMA_MODEL", "qwen3.5:latest"))
...
def needs_claude_tools(prompt: str, task_type: str) -> bool:
    ...
    if re.search(
        r'\b(src/|tests/|hooks/|in the codebase|this file|this repo|this project|current project|current version|what version|package\.json|pyproject\.toml|tessera|blocked by hook|error message)\b',
        prompt,
        re.IGNORECASE,
    ):
        return True
