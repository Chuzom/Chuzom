...
log = get_logger("chuzom.quota_balance")

_QUOTA_CACHE_FILE = Path.home() / ".chuzom" / "codex_quota.json"
...
def get_codex_pressure(daily_limit: int = 1000) -> float:
    ...
    Reads from ~/.chuzom/codex_quota.json. Resets counter if date changed.
...
def record_codex_request(daily_limit: int = 1000) -> None:
    """Increment Codex daily counter and persist to ~/.chuzom/codex_quota.json.
...
async def get_provider_pressures() -> dict[str, float]:
    ...
    from chuzom.gemini_cli_quota import get_gemini_pressure
