...
async def get_provider_budgets() -> dict[str, ProviderBudget]:
    ...
    naming convention ``CHUZOM_BUDGET_<PROVIDER>=<USD>``, e.g.
    ``CHUZOM_BUDGET_OPENAI=10.00``. A value of 0 (or missing env var)
    means unlimited.
    ...
    # Format: CHUZOM_BUDGET_OPENAI=10.00 (USD/month)
    import os
    limits: dict[str, float] = {}
    for provider in config.available_providers:
        env_key = f"CHUZOM_BUDGET_{provider.upper()}"
