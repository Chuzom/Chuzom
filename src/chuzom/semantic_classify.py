...
* **No hardcoded ``provider/model`` literals** (house rule
  ``chuzom-no-hardcoding-opensource``). This module decides *category*, never a
...
Embedding backend
-----------------
Reuses Chuzom's existing embedding path — ``nomic-embed-text`` via the local
Ollama ``/api/embeddings`` endpoint (the same model ``semantic_cache.py`` uses,
768-dim, free, no torch in the hot path). Optionally, set
``CHUZOM_SEMANTIC_CLASSIFIER_BACKEND=st`` to use a local
...
from chuzom.types import Complexity, Subject, TaskType

log = get_logger("chuzom.semantic_classify")
...
def _embed(text: str, expected_model: str) -> list[float] | None:
    ...
    ``CHUZOM_SEMANTIC_CLASSIFIER_BACKEND`` selects the backend:
      * unset / ``ollama`` → Ollama ``nomic-embed-text`` (default).
      * ``st`` / ``sentence-transformers`` → local ST model named
        ``CHUZOM_SEMANTIC_ST_MODEL`` (default = ``expected_model``).
    ...
    """
    backend = os.getenv("CHUZOM_SEMANTIC_CLASSIFIER_BACKEND", "ollama").strip().lower()

    if backend in ("st", "sentence-transformers"):
        st_model = os.getenv("CHUZOM_SEMANTIC_ST_MODEL", expected_model)
...
def _centroids_path() -> str:
    return os.getenv("CHUZOM_SEMANTIC_CENTROIDS", str(_DEFAULT_CENTROIDS_PATH))
