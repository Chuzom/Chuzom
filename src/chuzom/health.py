...
def _snapshot_path() -> Path:
    ...
    override = os.environ.get("CHUZOM_HEALTH_SNAPSHOT")
    if override:
        return Path(override)
    return Path.home() / ".chuzom" / "provider_health.json"
...
def read_health_snapshot(path: Path | None = None) -> dict:
    ...
    Lets a separate process (``chuzom doctor``) report the SAME breaker state the
...
class ProviderHealth:
    ...
    # (e.g. `chuzom doctor` runs as a separate CLI process); the epoch mirrors let the
...
class HealthTracker:
    ...
    def snapshot(self) -> dict[str, dict]:
        ...
        INV-HEALTH-001: this is the bridge that lets a separate process (``chuzom
