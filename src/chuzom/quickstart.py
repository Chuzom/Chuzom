"""Interactive quickstart wizard for chuzom.
...
def main() -> None:
    ...
    if has_keys:
        ...
        print("  Tip: run `chuzom setup` for full provider configuration.")
    elif has_ollama:
        ...
        if not _ask_yes("Continue without API keys?", default=False):
            print("\n  Exiting. Add keys and re-run: chuzom quickstart\n")
            return

    # ── Step 3: Install ────────────────────────────────────────────────────────
    _print_step(3, total_steps, "Installing for your hosts")

    from chuzom.cli import _install_host, _run_install
    ...
    for host in target_hosts:
        ...
        try:
            ...
            print(f"    Manual: chuzom install --host {host}")
    ...
    if _ask_yes("Run test call now?", default=True):
        try:
            ...
            from chuzom.types import TaskType, RoutingProfile
            ...
            print(f"\n  ⚠ Test call failed: {e}")
            print("    Check your API keys with: chuzom doctor")
    else:
        print("  Skipped.")
    ...
    print("")
    print("  Typical savings with chuzom:")
    print("")
    print("  ┌─────────────────┬────────────┬────────────┬──────────┐")
    ...
    print("")
    print("  Commands:")
    print("    chuzom status    — view routing status and savings")
    print("    chuzom doctor    — diagnose any installation issues")
    print("    chuzom setup     — configure additional providers")
    print("═" * 54 + "\n")
