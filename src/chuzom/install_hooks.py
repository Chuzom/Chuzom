"""Install chuzom hooks and rules globally into Claude Code.
...
Can be run as:
  - CLI: ``chuzom-install-hooks``
  - MCP tool: ``llm_setup(action='install_hooks')``
  - Python: ``from chuzom.install_hooks import install; install()``
"""
...
_SUBSCRIPTION_VAR = "CHUZOM_CLAUDE_SUBSCRIPTION"

_RULES_VERSION_RE = re.compile(r"<!--\s*chuzom-rules-version:\s*(\d+)\s*-->")
_HOOK_VERSION_RE = re.compile(r"#\s*chuzom-hook-version:\s*(\d+)")
...
_HOOK_DEFS = [
    ("session-start.py", "chuzom-session-start.py", "SessionStart", ""),
    ("auto-route.py", "chuzom-auto-route.py", "UserPromptSubmit", ""),
    ("status-bar.py", "chuzom-status-bar.py", "UserPromptSubmit", ""),
    ("enforce-route.py", "chuzom-enforce-route.py", "PreToolUse", ""),
...
_CLAW_CODE_HOOK_DEFS = [
    ("session-start.py",            "chuzom-session-start.py",  "SessionStart",     ""),
    ("auto-route.py",               "chuzom-auto-route.py",     "UserPromptSubmit", ""),
    ("status-bar-clawcode.py",      "chuzom-status-bar.py",     "UserPromptSubmit", ""),
    ("enforce-route.py",            "chuzom-enforce-route.py",  "PreToolUse",       ""),
    ("agent-route.py",              "chuzom-agent-route.py",    "PreToolUse",       "Agent"),
    ("subagent-start.py",           "chuzom-subagent-start.py", "SubagentStart",    ""),
    ("usage-refresh.py",            "chuzom-usage-refresh.py",      "PostToolUse",  "llm_|mcp__chuzom__llm"),
...
def _legacy_alias_path(hooks_dir: Path, src_name: str, dst_name: str) -> Path | None:
    ...
    if src_name == dst_name or not dst_name.startswith("chuzom-"):
        return None
...
def _sync_legacy_hook_alias(
    ...
    """Keep an existing chuzom legacy alias in sync with the canonical hook.

    We only sync unprefixed hook aliases when they are clearly managed by
    chuzom already, or when settings explicitly reference the alias path and
...
def _install_claude_code_cli(mcp_entry: dict) -> list[str]:
    """Register chuzom in ~/.claude.json so `claude -p` (non-interactive) picks it up.
    ...
    if claude_bin:
        ...
        try:
            result = _sp.run(
                [claude_bin, "mcp", "add", "--scope", "user", "chuzom", cmd_str] + args,
                capture_output=True, text=True, timeout=15,
            )
            if result.returncode == 0:
                return ["Registered chuzom MCP server in ~/.claude.json (via claude mcp add)"]
        except Exception:
    ...
    try:
        ...
        if "chuzom" in servers:
            return ["MCP server already in ~/.claude.json: chuzom"]
        servers["chuzom"] = mcp_entry
        _CLAUDE_JSON_PATH.write_text(json.dumps(data, indent=2) + "\n", encoding="utf-8")
        return ["Registered chuzom MCP server in ~/.claude.json (direct merge)"]
    except OSError as e:
        return [f"WARNING: could not register MCP in ~/.claude.json: {e}"]


def _uninstall_claude_code_cli() -> list[str]:
    """Remove chuzom from ~/.claude.json."""
    import subprocess as _sp
    ...
    if claude_bin:
        try:
            result = _sp.run(
                [claude_bin, "mcp", "remove", "--scope", "user", "chuzom"],
                capture_output=True, text=True, timeout=10,
            )
            if result.returncode == 0:
                return ["Removed chuzom from ~/.claude.json (via claude mcp remove)"]
        except Exception:
            pass
    ...
    try:
        if not _CLAUDE_JSON_PATH.exists():
            return []
        data = json.loads(_CLAUDE_JSON_PATH.read_text(encoding="utf-8"))
        if "chuzom" not in data.get("mcpServers", {}):
            return []
        del data["mcpServers"]["chuzom"]
        _CLAUDE_JSON_PATH.write_text(json.dumps(data, indent=2) + "\n", encoding="utf-8")
        return ["Removed chuzom from ~/.claude.json"]
    except OSError:
        return []


def _install_claude_desktop() -> list[str]:
    """Add chuzom to Claude Desktop's claude_desktop_config.json.
    ...
    chuzom_bin = shutil.which("chuzom") or "chuzom"
    entry = {"command": chuzom_bin, "args": []}

    config = _load_desktop_config(config_path)
    servers = config.setdefault("mcpServers", {})

    if "chuzom" in servers:
        return ["Claude Desktop: chuzom already registered"]

    servers["chuzom"] = entry
    config_path.parent.mkdir(parents=True, exist_ok=True)
    config_path.write_text(json.dumps(config, indent=2) + "\n", encoding="utf-8")
    return [f"Registered chuzom in Claude Desktop → {config_path}"]


def _uninstall_claude_desktop() -> list[str]:
    """Remove chuzom from Claude Desktop config. Returns actions taken."""
    config_path = claude_desktop_config_path()
    if config_path is None or not config_path.exists():
        return []

    config = _load_desktop_config(config_path)
    if "chuzom" not in config.get("mcpServers", {}):
        return []

    del config["mcpServers"]["chuzom"]
    config_path.write_text(json.dumps(config, indent=2) + "\n", encoding="utf-8")
    return [f"Removed chuzom from Claude Desktop → {config_path}"]
...
def check_api_keys() -> list[str]:
    ...
    gemini_subscription_on = os.environ.get("CHUZOM_GEMINI_SUBSCRIPTION", "").lower() in ("1", "true", "yes")
    ...
    if gemini_subscription_on:
        lines.append("  ✓  Gemini subscription mode active (CHUZOM_GEMINI_SUBSCRIPTION=true)")
...
def install(force: bool = False) -> list[str]:
    ...
    # Build the entry using the installed chuzom binary when available
    # (pip install), falling back to uv run for development installs.
    chuzom_bin = shutil.which("chuzom")
    if chuzom_bin:
        mcp_entry: dict = {"command": chuzom_bin, "args": []}
    else:
        uv_path = shutil.which("uv") or "uv"
        project_dir = str(_PACKAGE_DIR.parent.parent)
        mcp_entry = {"command": uv_path, "args": ["run", "--directory", project_dir, "chuzom"]}
    ...
    if "chuzom" not in mcp_servers:
        ...
        actions.append("MCP server already in ~/.claude/settings.json: chuzom")
...
def uninstall() -> list[str]:
    ...
    if "chuzom" in mcp_servers:
        ...
        _save_settings(settings2)
        actions.append("Removed chuzom MCP server from ~/.claude/settings.json")
...
def install_claw_code() -> list[str]:
    ...
    # ── Set CHUZOM_CLAW_CODE=true in ~/.claw-code/.env ───────────────
    ...
    env_path = cc_dir / ".env"
    claw_flag = "CHUZOM_CLAW_CODE=true"
    try:
        existing = env_path.read_text(encoding="utf-8") if env_path.exists() else ""
        if "CHUZOM_CLAW_CODE" not in existing:
            ...
            actions.append(f"CHUZOM_CLAW_CODE already set in {env_path}")
    except OSError as e:
        actions.append(f"WARN could not write {env_path}: {e}")

    # ── Register MCP server in claw-code settings ────────────────────────
    chuzom_bin = shutil.which("chuzom") or "chuzom"
    mcp_entry = {"command": chuzom_bin, "args": []}
    mcp_servers = settings.setdefault("mcpServers", {})
    if "chuzom" not in mcp_servers:
        mcp_servers["chuzom"] = mcp_entry
        settings_path.write_text(json.dumps(settings, indent=2) + "\n")
        actions.append(f"Registered chuzom MCP server in {settings_path}")
    else:
...
def uninstall_claw_code() -> list[str]:
    """Remove chuzom hooks and MCP registration from claw-code. Returns actions taken."""
    actions: list[str] = []
