...
chuzom's existing ``savings_stats`` ledger (the table ``llm_savings`` reads).
...
def _db_path() -> Path:
    return Path(os.environ.get("CHUZOM_DB_PATH") or (Path(os.path.expanduser("~")) / ".chuzom" / "usage.db"))


def savings_payload(
    result: dict[str, Any], *, model: str = "chuzom-agentic-router", session_id: str = ""
) -> dict[str, Any]:
        ...
        try:
            ...
            conn.execute(
                ...
                (
                    ...
                    payload.get("model", "chuzom-agentic-router"),
                    "claude_code",
                ),
            )
            conn.commit()
...
async def record_delegation_savings(
    ...
    *,
    ...
    model: str = "chuzom-agentic-router",
    session_id: str = "",
) -> dict[str, Any]:
