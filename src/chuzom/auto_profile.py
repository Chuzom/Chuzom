...
2. Generates ~/.chuzom/profile.yaml with token-wise priority
...
from chuzom.gemini_cli_agent import is_gemini_cli_available
from chuzom.claude_usage import get_claude_pressure
from chuzom.logging import get_logger

log = get_logger("chuzom.auto_profile")

PROFILE_PATH = Path.home() / ".chuzom" / "profile.yaml"
PROFILE_CACHE_PATH = Path.home() / ".chuzom" / "profile_cache.json"
...
def detect_services() -> ServiceDetection:
    ...
    # Claude subscription
    if cfg.chuzom_claude_subscription:
    ...
    if is_gemini_cli_available():
        ...
        try:
            from chuzom.gemini_cli_quota import get_gemini_quota_status_sync
...
def generate_profile_yaml(detected: ServiceDetection) -> str:
    ...
    Returns:
        YAML string for ~/.chuzom/profile.yaml
    """
    ...
    # Generate YAML
    yaml = "# Auto-detected Chuzom Profile\n"
    yaml += "# Generated by: chuzom profile auto\n"
...
def save_profile(yaml_content: str) -> Path:
    """Save profile YAML to ~/.chuzom/profile.yaml.
