...
Loads two optional YAML files and merges them:
  1. ~/.chuzom/routing.yaml   — user-level overrides (always applied)
  2. .chuzom.yml              — repo-level overrides (searched up from cwd)
...
class RepoConfig:
    ...
    def effective_profile(self) -> str | None:
        ...
        env = os.environ.get("CHUZOM_PROFILE", "").lower()
...
def find_repo_config_path(start: Path | None = None) -> Path | None:
    """Search start (default cwd) and ancestors for .chuzom.yml."""
    here = start or Path.cwd()
    for candidate in [here, *here.parents]:
        p = candidate / ".chuzom.yml"
        if p.exists():
            return p
...
def load_user_config() -> RepoConfig:
    """Load ~/.chuzom/routing.yaml (user-level config)."""
    path = Path.home() / ".chuzom" / "routing.yaml"
    if not path.exists():
        return RepoConfig()
...
def load_repo_config(start: Path | None = None) -> RepoConfig:
    """Load .chuzom.yml from cwd or nearest ancestor (repo-level config)."""
