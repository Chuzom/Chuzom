"""chuzom init-claude-memory — Auto-discover setup and generate Claude Code memory files."""

from chuzom.config import get_config
from chuzom.safe_config import write_safe_config_template, safe_config_path
...
def run_init_claude_memory() -> None:
    ...
    3. Creates ~/.chuzom/config.yaml template for enterprise users

    Enterprise use case: When security teams block .env at project level, users can:
    - Fill in ~/.chuzom/config.yaml with their keys
    ...
    discovered = {
        ...
        "chuzom_profile": config.chuzom_profile.value,
        "chuzom_claude_subscription": config.chuzom_claude_subscription,
    }
    ...
    print(f"  Profile:     {config.chuzom_profile.value}")
    print(
        f"  Mode:        {'Claude Subscription (no API key)' if config.chuzom_claude_subscription else 'API Key Mode'}"
    )
    print()
    print("=" * 60)
    print()
    ...
    print("  2. If .env is blocked: Edit ~/.chuzom/config.yaml and add your API keys")
    print("  3. Run 'chuzom status' to verify the setup\n")
