...
from chuzom.commands.gain import show_gain
...
from chuzom.forecast import get_burn_forecast
...
async def llm_save_session(ctx: Context) -> str:
    ...
    """
    from chuzom.context import auto_summarize_session
...
async def llm_usage(period: str = "today") -> str:
    ...
    if is_codex_available():
        ...
        lines.append(row("  Session spend counts chuzom tool calls only"))
    else:
    ...
    forecast = await get_burn_forecast(budget_limit=config.chuzom_monthly_budget)
    if forecast.days_of_data >= 3:
        ...
        if config.chuzom_monthly_budget > 0 and forecast.pct_remaining >= 0:
...
async def llm_health() -> str:
    ...
    from chuzom.config import probe_ollama
    config = get_config()
    tracker = get_tracker()
...
async def llm_hook_health() -> str:
    ...
    lines.append(
        "\n💡 Tip: Check ~/.chuzom/hook_errors.log for full error history"
    )
    return "\n".join(lines)
...
async def llm_dashboard(port: int = 7337) -> str:
    ...
    subprocess.Popen(
        [sys.executable, "-m", "chuzom.dashboard.__main__", "--port", str(port)],
        **kwargs,
    )
    ...
    stop_cmd = (
        "pkill -f 'chuzom.dashboard'"
        if sys.platform != "win32"
        else "taskkill /F /FI \"WINDOWTITLE eq chuzom.dashboard\""
    )
...
async def llm_savings() -> str:
    ...
    lines.append(HR)
    lines.append(row("  Tip: run `chuzom test \"<prompt>\"` to simulate routing"))
    lines.append(HR)
...
async def llm_team_report(period: str = "week") -> str:
    ...
    """
    from chuzom.team import build_team_report, get_project_id, get_user_id, detect_channel

    config = get_config()
    user_id = get_user_id(override=config.chuzom_user_id)
    ...
    endpoint = config.chuzom_team_endpoint
    if endpoint:
        ...
        lines.append(row("  Set CHUZOM_TEAM_ENDPOINT to push to Slack/Discord/Telegram."))
    lines.append(HR)
    return "\n".join(lines)
...
async def llm_team_push(period: str = "week") -> str:
    ...
    Sends a formatted message to the endpoint set by ``CHUZOM_TEAM_ENDPOINT``.
    ...
    Args:
        period: ``"today"``, ``"week"``, ``"month"``, or ``"all"``.
    """
    from chuzom.team import build_team_report, detect_channel, get_project_id, get_user_id, push_report
    ...
    if not endpoint:
        return (
            ...
            "Set CHUZOM_TEAM_ENDPOINT in your .env or routing.yaml:\n\n"
            "  CHUZOM_TEAM_ENDPOINT=https://hooks.slack.com/...    # Slack\n"
            "  CHUZOM_TEAM_ENDPOINT=https://discord.com/api/webhooks/...  # Discord\n"
            "  CHUZOM_TEAM_ENDPOINT=https://api.telegram.org/bot{token}/  # Telegram\n"
            "  CHUZOM_TEAM_ENDPOINT=https://your-server.com/webhook  # Generic\n"
        )

    user_id = get_user_id(override=config.chuzom_user_id)
    project_id = get_project_id()
    chat_id = config.chuzom_team_chat_id
...
async def llm_policy() -> str:
    ...
    Displays the merged policy from all three layers:
      - Org policy   (~/.chuzom/org-policy.yaml)
      - User policy  (~/.chuzom/routing.yaml)
      - Repo policy  (.chuzom.yml)

    Also shows the last 10 policy enforcement events from the audit log.
    """
    from chuzom.policy import load_org_policy, policy_summary
...
async def llm_digest(period: str = "week", send: bool = False) -> str:
    ...
    Args:
        ...
        send:   If True, POST the digest to CHUZOM_WEBHOOK_URL.
    """
    from chuzom.digest import format_digest, send_to_webhook
    ...
    config = get_config()
    url = config.chuzom_webhook_url or config.chuzom_team_endpoint
    if not url:
        return (
            digest + "\n\n"
            "  ℹ️  To send this digest automatically, set:\n"
            "  CHUZOM_WEBHOOK_URL=https://hooks.slack.com/...\n"
        )
...
async def llm_benchmark() -> str:
    ...
    Also shows an optional community export status if CHUZOM_COMMUNITY=true.
    """
    from chuzom.community import (
    ...
    config = get_config()
    if config.chuzom_community:
        export_msg = await prepare_community_export()
        report += f"\n\n  📤 Community export: {export_msg}"
    else:
        report += (
            "\n\n  ℹ️  Set CHUZOM_COMMUNITY=true to prepare an anonymous "
...
async def llm_session_spend() -> str:
    ...
    config = get_config()
    if config.chuzom_escalate_above > 0:
        lines.append("")
        lines.append(
            f"Per-call cap: ${config.chuzom_escalate_above:.2f} "
            f"(CHUZOM_ESCALATE_ABOVE)"
        )
    if config.chuzom_hard_stop_above > 0:
        lines.append(
            f"Session hard stop: ${config.chuzom_hard_stop_above:.2f} "
            f"(CHUZOM_HARD_STOP_ABOVE)"
        )
...
async def llm_approve_route(
    ...
    estimated cost exceeded CHUZOM_ESCALATE_ABOVE. The pending call is
...
async def llm_quota_status() -> str:
    ...
    """
    try:
        from chuzom.quota_balance import get_provider_pressures, get_balanced_provider_order
...
async def llm_budget() -> str:
    ...
    """
    try:
        from chuzom.budget import get_all_budget_states, format_budget_summary

        states = await get_all_budget_states()
        summary = format_budget_summary(states)

        from chuzom.types import LOCAL_PROVIDERS
        uncapped = [
        ...
        if uncapped:
            ...
            nudge = (
                ...
                f"   Run `chuzom budget set <provider> <amount>` to protect against runaway costs."
            )
...
async def llm_share_profile() -> str:
    ...
    Exports ~/.chuzom/learned_routes.json and prepares it for upload
    to a shared community repository. Useful for publishing routing patterns
    you've learned that may benefit other chuzom users.

    Returns:
        Path to exported profile and upload instructions
    """
    from chuzom.memory.profiles import load_learned_profile
    ...
    try:
        ...
        export_path = Path.home() / ".chuzom" / "profile_export.json"
        export_path.write_text(json.dumps(export_data, indent=2))

        return (
            f"✅ Profile exported to: {export_path}\n\n"
            f"Share at: https://github.com/ypollak2/chuzom-profiles\n"
            f"Your routes:\n"
            + "\n".join(
...
async def llm_import_profile(url: str = "") -> str:
    ...
    """
    try:
        from chuzom.memory.profiles import load_learned_profile, save_learned_profile, LearnedRoute

        # Load existing profile
        existing = load_learned_profile()

        # Default to latest community profile if no URL provided
        DEFAULT_COMMUNITY_URL = "https://raw.githubusercontent.com/ypollak2/chuzom-profiles/main/community.json"
        if not url:
...
async def llm_model_usage(ctx: Context, hours: int = 24) -> str:
    ...
    """
    from chuzom.model_tracking import (
...
async def llm_model_eval(ctx: Context) -> str:
    ...
    """
    from chuzom.model_evaluator import (
