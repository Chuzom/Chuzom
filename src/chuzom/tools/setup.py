...
```
CHUZOM_BUDGET_OPENAI=10.00
CHUZOM_BUDGET_GEMINI=5.00
CHUZOM_MONTHLY_BUDGET=20.00
```
...
def _setup_add(provider: str, api_key: str) -> str:
    ...
    from chuzom.budget_store import get_cap as _get_stored_cap
    budget_nudge = ""
    if _get_stored_cap(provider) <= 0:
        budget_nudge = (
            f"\n\n💰 **Set a monthly budget cap** to protect against runaway costs:\n"
            f"   `chuzom budget set {provider} 20`\n"
...
def _setup_install_hooks() -> str:
    ...
    lines.extend([
        ...
        "Work that skips a required route is blocked by default (`CHUZOM_ENFORCE=hard`).",
        "The router classifies tasks and injects `⚡ MANDATORY ROUTE` directives that",
        "direct Claude to use the optimal `llm_*` tool.",
        "Set `CHUZOM_ENFORCE=soft` or `off` to relax enforcement.",
        "",
