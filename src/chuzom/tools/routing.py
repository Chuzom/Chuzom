...
from chuzom.memory.profiles import get_primary_model_for_tool
from chuzom.model_selector import select_model
from chuzom.prompt_injection import wrap_prompt_with_boundaries
from chuzom.profiles import complexity_to_profile
...
async def llm_classify(
    ...
    # Extra tip when CHUZOM_EXPLAIN=1 is not set
    if os.getenv("CHUZOM_EXPLAIN") != "1":
        lines.append(row("  Tip: set CHUZOM_EXPLAIN=1 to see [→ model · task] on every response"))
...
async def llm_auto(
    ...
    """
    from chuzom.cost import import_savings_log, get_lifetime_savings_summary
...
async def llm_stream(
    ...
    if model:
        ...
        chain = get_model_chain(
            ...
            is_subscription_mode=config.chuzom_claude_subscription,
        )
...
async def llm_select_agent(
    ...
    from chuzom.state import set_active_agent
