...
Example output:
  ✅ Configuration loaded from ~/.chuzom/config.yaml
  ✅ SQLite database: ~/.chuzom/usage.db (45 MB, last write 5 mins ago)
...
def check_configuration() -> tuple[bool, str]:
    ...
    Returns:
        (success, message)
    """
    config_path = Path.home() / ".chuzom" / "config.yaml"
...
def main(args: Optional[list[str]] = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Health check for chuzom"
    )
    parser.parse_args(args or [])

    print()
    print(Color.ORCHESTRATE_BLUE("=" * 70))
    print(Color.ORCHESTRATE_BLUE("              chuzom health check"))
    print(Color.ORCHESTRATE_BLUE("=" * 70))
    print()
