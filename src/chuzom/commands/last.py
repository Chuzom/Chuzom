#!/usr/bin/env python3
"""chuzom last — Show the most recent routing decisions (real-time feedback).

Command: uv run chuzom last [--count N]
...
from chuzom.terminal_style import Color, Symbol
...
def print_recent_decisions(decisions: list[dict]) -> None:
    ...
    """
    if not decisions:
        print("No routing decisions yet. Use chuzom to route your first request.")
        return
