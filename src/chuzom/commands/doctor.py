...
class _RoutingDecisionState(NamedTuple):
    ...
    : doctor collapsed "table unreadable", "table empty but the machine is
...
def _routing_decision_state(db_path: Path) -> _RoutingDecisionState:
    ...
    try:
        ...
        try:
            ...
            # one, and reporting them alike is what made  unreproducible.
            return _RoutingDecisionState(
                False, 0, 0,
...
def _check_savings_posture() -> list[str]:
    ...
    if db.is_file():
        ...
        if total_n == 0:
            # Say WHICH of the three states this is: unreadable, empty-but-busy,
            # or genuinely idle. They printed the same sentence.
...
def _run_doctor(host: Optional[str] = None) -> tuple[int, list[str]]:
    ...
    try:
        ...
        # DIRECT-execution timeouts never reached the breaker, so this section
        # stayed empty while the local path failed on every prompt and said so
        # only in a debug log.
        try:
            from chuzom.direct_diagnostics import current_advice

            _timeout = float(os.environ.get("CHUZOM_OLLAMA_TIMEOUT", "4"))
