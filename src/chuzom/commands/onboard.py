...
def _run_onboard() -> None:
    ...
    # ── 6. Write config to ~/.chuzom/.env ────────────────────────────────
    env_path = os.path.join(STATE_DIR, ".env")
    os.makedirs(STATE_DIR, exist_ok=True)
    env_lines = [
        f"CHUZOM_ENFORCE={enforce}",
        f"CHUZOM_PROFILE={profile}",
    ]
    try:
    ...
    print("    • Run `chuzom status` to see savings accumulate")
    if enforce == "shadow":
        print("    • Upgrade to suggest/enforce when ready: chuzom onboard")
    print()
