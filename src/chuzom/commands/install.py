...
def _run_install(flags: list[str]) -> None:
    ...
    if check_only:
        print(f"\n{_bold('[chuzom] Install check')}  (no changes made)\n")
        ...
        if rules_dst.exists():
            print(_ok("chuzom.md  (routing rules)"))
        else:
            print(_fail("chuzom.md  (routing rules)  — not installed",
                        fix="chuzom install"))
            all_ok = False
        ...
        if desktop_path is None:
            ...
            if desktop_path.exists():
                try:
                    ...
                    registered = "chuzom" in cfg.get("mcpServers", {})
                except Exception:
                    pass
            if registered:
        ...
        if all_ok:
            ...
            print(_yellow("  Run `chuzom install` to fix the issues above."))
        print()
        return
    ...
    if force:
        from chuzom.install_hooks import _load_settings, _save_settings
        settings = _load_settings()
        settings.get("mcpServers", {}).pop("chuzom", None)
    ...
    from chuzom.install_hooks import install_claw_code, claw_code_settings_path
    ...
    print("    chuzom doctor               — verify everything is wired up")
    print("    chuzom status               — today's cost & savings")
    print("    chuzom dashboard            — web dashboard (localhost:7337)")
    print("    chuzom install --check      — preview install state")
    print("    chuzom install --force      — reinstall / update paths")
    print("    chuzom install --claw-code  — also install into claw-code")
    print("    chuzom uninstall            — remove\n")
...
"""
    ...
    print(_dim("  (chuzom install does this automatically; shown for reference)"))
    import json as _json
    example = {
        "mcpServers": {"chuzom": {"command": "chuzom", "args": []}},
        "hooks": {
            "UserPromptSubmit": [{"matcher": "", "hooks": [{"type": "command", "command": "python3 ~/.claude/hooks/chuzom-auto-route.py"}]}],
            "Stop":             [{"matcher": "", "hooks": [{"type": "command", "command": "python3 ~/.claude/hooks/chuzom-session-end.py"}]}],
        }
    }
    ...
    print("    cat ~/.chuzom/usage.json        # routing stats")
    print("    sqlite3 ~/.chuzom/usage.db \\")
...
def _install_codex_files() -> list[str]:
    ...
    hook_script = home / ".chuzom" / "hooks" / "codex-post-tool.py"

    # Copy the hook script to ~/.chuzom/hooks/
...
def _install_opencode_files() -> list[str]:
    ...
    hook_dest, hook_actions = _copy_hook_script(
        "opencode-post-tool.py", home / ".chuzom" / "hooks"
    )
...
def _install_gemini_cli_files() -> list[str]:
    ...
    if not ext_manifest.exists():
        manifest = {
            ...
            "mcpServers": {"chuzom": server_entry},
        }
    ...
    post_hook_dest, hook_actions = _copy_hook_script(
        "gemini-cli-post-tool.py", home / ".chuzom" / "hooks"
    )
    actions += hook_actions

    auto_hook_dest, hook_actions = _copy_hook_script(
        "gemini-cli-auto-route.py", home / ".chuzom" / "hooks"
    )
    actions += hook_actions

    status_hook_dest, hook_actions = _copy_hook_script(
        "status-bar.py", home / ".chuzom" / "hooks"
    )
    actions += hook_actions

    end_hook_dest, hook_actions = _copy_hook_script(
        "gemini-cli-session-end.py", home / ".chuzom" / "hooks"
    )
    actions += hook_actions

    start_hook_dest, hook_actions = _copy_hook_script(
        "session-start.py", home / ".chuzom" / "hooks"
    )
    actions += hook_actions
...
def _install_factory_files() -> list[str]:
    ...
    if plugin_dir.exists():
        ...
        actions.append("  Install via: factory plugin install ypollak2/chuzom")
    else:
        actions.append("  .factory-plugin/ not found in repo root — run from the chuzom repo dir")
    actions.append("  Or install .claude-plugin/ directly: factory plugin install ypollak2/chuzom")
    return actions
...
def _install_host(host: str) -> None:
    ...
    print(f"\n{bold}chuzom install --host {host}{reset}\n")
    print("─" * min(w, 70))
    ...
    print(
        f"\nFor Claude Code (hooks + full cost-routing): {bold}chuzom install{reset}\n"
