...
def _run_config_init() -> None:
    """Create a starter .chuzom.yml in the current directory."""
    from chuzom.repo_config import fingerprint_repo
    ...
    template = f"""\
# .chuzom.yml — repo-level routing config
# Docs: https://github.com/ypollak2/chuzom
version: 1
...
"""
    ...
    print(f"  Edit it, then run {_bold('chuzom config lint')} to validate.\n")
...
def _run_config(flags: list[str]) -> None:
    ...
    - init: Create a new .chuzom.yml template
    """
    ...
    # Sources
    user_path = Path.home() / ".chuzom" / "routing.yaml"
    repo_path = find_repo_config_path()
    print(f"  {_bold('User config:')}  {user_path}  {'✓' if user_path.exists() else _dim('(not found)')}")
    print(f"  {_bold('Repo config:')}  {repo_path or _dim('(none — no .chuzom.yml in tree)')}")
    ...
    if not user_path.exists() and repo_path is None:
        warnings.append(
            "No config files found — using defaults. Run `chuzom config init` to create one."
        )
...
def cmd_config(args: list[str]) -> int:
    """Execute: chuzom config [show|lint|init]
