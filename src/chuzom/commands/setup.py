...
def _run_setup() -> None:
    """Interactive wizard: configure providers and write API keys to ~/.chuzom/.env."""
    from pathlib import Path

    env_path = Path.home() / ".chuzom" / ".env"
    env_path.parent.mkdir(parents=True, exist_ok=True)

    print(f"\n{_bold('Chuzom — Setup Wizard')}\n")
    print("This wizard configures your provider API keys.")
    print("Keys are saved to ~/.chuzom/.env and loaded automatically by the router.\n")

    # ── Step 1: Claude Code subscription ──────────────────────────────────────
    print(_bold("Step 1: Subscription mode"))
    cc_mode = os.getenv("CHUZOM_CLAUDE_SUBSCRIPTION", "")
    gemini_mode = os.getenv("CHUZOM_GEMINI_SUBSCRIPTION", "")
    
    enable_cc = False
    if cc_mode.lower() in ("true", "1", "yes"):
        print(_ok("CHUZOM_CLAUDE_SUBSCRIPTION is already set — Claude models routed via subscription."))
        enable_cc = True
    else:
    ...
    if gemini_mode.lower() in ("true", "1", "yes"):
        print(_ok("CHUZOM_GEMINI_SUBSCRIPTION is already set — Gemini models routed via local CLI."))
        enable_gemini = True
    else:
    ...
    if enable_cc:
        new_keys["CHUZOM_CLAUDE_SUBSCRIPTION"] = "true"
    if enable_gemini:
        new_keys["CHUZOM_GEMINI_SUBSCRIPTION"] = "true"
        new_keys["CHUZOM_PROFILE"] = "quota_balanced"
    ...
    if new_keys:
        ...
        lines = ["# Chuzom provider keys — auto-generated by `chuzom setup`", ""]
    ...
    print()
    ans = input("Run `chuzom install` now? [Y/n]: ").strip().lower()
    if ans in ("", "y", "yes"):
        from chuzom.commands.install import cmd_install
        cmd_install([])
    else:
        print(f"\n  Run {_bold('chuzom install')} when ready to activate routing.\n")
