"""``chuzom okf`` — inspect and garbage-collect the OKF knowledge store.
...
def _print_help() -> None:
    print(f"""
{_b('chuzom okf')} — inspect the knowledge store that feeds routed prompts

  {_b('chuzom okf status')}        what is stored, per project, and what gets injected
  {_b('chuzom okf gc')}            report docs that are model prose rather than verified structure
  {_b('chuzom okf gc --apply')}    move those to knowledge/quarantine/ (recoverable, never deleted)
  {_b('chuzom okf adopt')}         move VERIFIED legacy docs into this project's store
  {_b('chuzom okf restore')}       move everything in quarantine/ back into retrieval
...
def cmd_okf(args: list[str]) -> None:
    ...
    if sub == "status":
        ...
        print(f"  enabled        : {'yes' if okf._okf_enabled() else 'no (CHUZOM_OKF=off)'}")
        ...
        if legacy:
            ...
            print(_dim("    projects. Run `chuzom okf gc` to see which are model prose."))
            for p in legacy:
                print(f"    {p.name}")

        q = sorted(okf.QUARANTINE_DIR.rglob("*.md")) if okf.QUARANTINE_DIR.exists() else []
        if q:
            print(f"\n  {_dim('quarantined')}: {len(q)} doc(s) — `chuzom okf restore` to undo")
    ...
    if sub == "gc":
        ...
        if apply:
            ...
            print(_dim("\n  Nothing was deleted. `chuzom okf restore` puts them back.\n"))
        else:
            print(f"\n  Run {_b('chuzom okf gc --apply')} to move them to quarantine/.")
