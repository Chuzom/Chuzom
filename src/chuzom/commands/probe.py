"""chuzom probe — verify which local Ollama models can drive the agentic loop.

Runs a live best-of-N ground-truth probe of every installed model, caches the
verdicts to ~/.chuzom/agentic_models.json, and reports the dynamically-selected
...
def cmd_probe(args: list[str]) -> int:
    """Execute: chuzom probe [--cached]"""
    from chuzom.agentic_registry import best_agentic_model, get_registry
    ...
    if not verdicts:
        if use_cache:
            print("No cached verdicts yet. Run `chuzom probe` (no --cached) to build the registry.")
        else:
