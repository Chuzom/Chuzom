"""chuzom invoice — finance-facing provider invoice reconciliation (#58).

    chuzom invoice report [--month YYYY-MM] [--format text|csv] [--provider P]

For each supported provider, pulls that month's billed total, tallies
chuzom's own recorded cost for the same period, and renders a
...
def _usage_db_path() -> str:
    return os.environ.get("CHUZOM_USAGE_PATH") or os.path.expanduser("~/.chuzom/usage.db")


def _chuzom_tally(provider: str, month: str) -> tuple[float, int]:
    """Return (total_usd, call_count) chuzom recorded for provider+month.
...
def _pull_invoice(provider: str, month: str):
    ...
    """
    try:
        mod = __import__(f"chuzom.invoice_reconciliation.{provider}", fromlist=["pull_monthly_invoice"])
        return mod.pull_monthly_invoice(period=month)
    except Exception as exc:  # noqa: BLE001 — best-effort, degrade gracefully
        print(f"[chuzom] skipping {provider}: {exc}", file=sys.stderr)
        return None


def cmd_invoice(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="chuzom invoice", description="Provider invoice reconciliation report",
    )
    ...
    for provider in providers:
        ...
        if invoice is None:
            continue
        total_usd, call_count = _chuzom_tally(provider, month)
        diffs.append(
            compute_diff(
                invoice=invoice, chuzom_total_usd=total_usd, chuzom_call_count=call_count,
            )
        )
