"""``chuzom policy …`` — Plan 07 Cat G policy-side CLI.
...
from chuzom.policy_diff import Sample, diff_policies, format_diff_report
...
def _cmd_diff(argv: list[str]) -> int:
    parser = argparse.ArgumentParser(prog="chuzom policy diff")
...
def _print_help(exit_code: int = 0) -> int:
    print(
        "Usage: chuzom policy <subcommand> [options]\n"
        "\n"
        "Subcommands:\n"
