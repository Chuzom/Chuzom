...
def _run_team(subcmd: str, flags: list[str]) -> None:
    """chuzom team report|push|setup [period]."""
    import asyncio
    from chuzom.team import (
    ...
    if calls == 0:
        ...
        print(f"  Try: {_bold('chuzom team report all')}\n")
        return
    ...
    if subcmd == "push":
        if not endpoint:
            ...
            print(f"  Run {_bold('chuzom team setup')} to configure Slack/Discord/Telegram.\n")
            return
        channel = detect_channel(endpoint)
        print(f"\n  Pushing to {_bold(channel)}...")
        success, msg = asyncio.run(push_report(
            report, endpoint, telegram_chat_id=config.chuzom_team_chat_id,
        ))
        if success:
            print(_green(f"  {msg}"))
        else:
        ...
        channel = detect_channel(endpoint)
        print(f"\n  {_dim(f'Endpoint: {channel} configured — run chuzom team push to send.')}")
    else:
        print(f"\n  {_dim('Tip: run chuzom team setup to configure Slack/Discord/Telegram push.')}")
    print()


def _run_team_setup(config) -> None:
    """Interactive wizard to configure the team notification endpoint."""
    print(f"\n{_bold('[chuzom] Team Notification Setup')}\n")
    ...
    env_path = Path.home() / ".chuzom" / ".env"
    env_content = env_path.read_text() if env_path.exists() else ""
    for key, val in [("CHUZOM_TEAM_ENDPOINT", url), ("CHUZOM_TEAM_CHAT_ID", chat_id)]:
        if not val:
            continue
    ...
    from chuzom.team import detect_channel
    channel = detect_channel(url)
    print(f"\n{_green('✓')} Team endpoint configured: {_bold(channel)}")
    print(f"  Run {_bold('chuzom team push')} to send your first report.\n")


# ── Entry point ─────────────────────────────────────────────────────────────

def cmd_team(args: list[str]) -> int:
    """Execute: chuzom team [report|push|setup] [period]
