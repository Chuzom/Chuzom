...
def _run_budget(subcmd: str, flags: list[str]) -> None:
    ...
    """
    import asyncio
    from chuzom.budget_store import list_caps, remove_cap, set_cap
    from chuzom.budget import get_all_budget_states, invalidate_cache
    from chuzom.types import LOCAL_PROVIDERS

    if subcmd == "set":
        if len(flags) < 2:
            print(_red("Usage: chuzom budget set <provider> <amount>"))
            sys.exit(1)
        ...
        print(f"  {_dim('Saved to ~/.chuzom/budgets.json')}")
        return

    if subcmd == "remove":
        if not flags:
            print(_red("Usage: chuzom budget remove <provider>"))
            sys.exit(1)
        provider = flags[0]
    ...
    if uncapped:
        ...
        print(_dim("     Set one: chuzom budget set <provider> <amount>"))

    stored = caps
    if stored:
        print()
        print(_dim("  Caps stored in ~/.chuzom/budgets.json"))
    print()
...
def cmd_budget(args: list[str]) -> int:
    """Execute: chuzom budget [list|set|remove]
