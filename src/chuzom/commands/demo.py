...
def _run_demo() -> None:
    ...
    cc_mode = os.getenv("CHUZOM_CLAUDE_SUBSCRIPTION", "").lower() in ("true", "1", "yes")
    gemini_mode = os.getenv("CHUZOM_GEMINI_SUBSCRIPTION", "").lower() in ("true", "1", "yes")
    ...
    if not using_real:
        ...
        print(f"  {_dim('Source: ~/.chuzom/usage.db  (your actual routing decisions)')}\n")
    ...
    if not using_real:
        print(f"\n  {_yellow('Next steps:')}")
        print(f"    {_yellow('→')} Run {_bold('chuzom install')} to enable automatic routing")
        if not cc_mode:
            print(f"    {_yellow('→')} Set {_bold('CHUZOM_CLAUDE_SUBSCRIPTION=true')} to use subscription models")
    else:
        print(f"\n  {_yellow('Your routing history:')}")
    print(f"  {_yellow('→')} Check savings: {_bold('chuzom gain')}")
    print(f"  {_yellow('→')} View dashboard: {_bold('chuzom dashboard')}\n")
