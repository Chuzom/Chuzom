...
def _run_set_enforce(mode: str) -> None:
    ...
    if env_path.exists():
        ...
        if "CHUZOM_ENFORCE=" in env_content:
            env_content = re.sub(
                r"CHUZOM_ENFORCE=\S*", f"CHUZOM_ENFORCE={mode}", env_content
            )
        else:
            env_content += f"\nCHUZOM_ENFORCE={mode}\n"
        env_path.write_text(env_content)
    else:
        env_path.write_text(f"CHUZOM_ENFORCE={mode}\n")
...
def cmd_set_enforce(args: list[str]) -> int:
    """Execute: chuzom set-enforce <mode>
