#!/usr/bin/env python3
"""chuzom gc — TTL-sweep stale per-session shards from ~/.chuzom.

Command: uv run chuzom gc [--ttl-days N] [--apply] [--root PATH]

Chuzom's hooks write small per-session shard files (`last_route_<id>.json`,
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="chuzom gc",
        description="Sweep stale per-session shard files from ~/.chuzom (dry-run by default).",
    )
    parser.add_argument(
        "--ttl-days",
        type=float,
    ...
    parser.add_argument(
        ...
        help="directory to sweep (default: ~/.chuzom)",
    )
    ...
    kb = total_bytes / 1024
    print(
        f"chuzom gc: {verb} {len(stale)} shard(s) older than "
