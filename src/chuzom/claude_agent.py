...
Code subscription (subscription auth, no API key). This lets Chuzom's offload tools
...
def offload_available(config: object) -> bool:
    ...
    5h+weekly Claude pressure is below ``chuzom_claude_offload_max_pressure`` — so
    offload never starves the user's primary Claude Code work.
    """
    if not getattr(config, "chuzom_claude_subscription", False):
        return False
    if not is_claude_available():
        return False
    from chuzom.claude_usage import get_claude_pressure
    cap = getattr(config, "chuzom_claude_offload_max_pressure", 0.80)
...
async def run_claude(
    ...
    if timeout is None:
        timeout = int(os.environ.get("CHUZOM_CLAUDE_TIMEOUT", "300"))
