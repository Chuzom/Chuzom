...
* ``load_dataset`` reads JSONL from ``~/.chuzom/data/routerarena/<split>.jsonl``
...
def _default_dataset_root() -> Path:
    ...
    Per-user (not per-project) because the same dataset serves every chuzom
    workspace on this machine — no need to re-download or duplicate.
    """
    return Path.home() / ".chuzom" / "data" / "routerarena"
...
# via ``chuzom.benchmark.get_runner("routerarena")``.
