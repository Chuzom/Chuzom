...
log = logging.getLogger("chuzom.benchmark_fetcher")
...
def generate_benchmarks_md(data: dict[str, Any], output_path: Path) -> None:
    ...
    lines: list[str] = [
        ...
        "<!-- Source: src/chuzom/data/benchmarks.json · Generator: scripts/update_benchmarks.py -->",
        "",
        "# Model Benchmarks",
        "",
    ...
    lines += [
        ...
        "        ├── Writes src/chuzom/data/benchmarks.json  ← used by router",
        ...
        "with `~/.chuzom/benchmarks.json` and overwrites if the bundled copy is newer.",
    ]
