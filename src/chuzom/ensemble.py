...
from chuzom.classify import _COMPLEXITY_RANK, apply_complexity_floor, classify_signals
from chuzom.logging import get_logger
from chuzom.sanitization import sanitize_prompt
from chuzom.types import ClassificationResult, Complexity, Subject, TaskType

log = get_logger("chuzom.ensemble")
...
# Complexity floor policy lives in chuzom.classify (single source of truth,
...
def _ensemble_enabled() -> bool:
    ...
    Disable with CHUZOM_ENSEMBLE=off to fall back to the cloud classifier."""
    return os.environ.get("CHUZOM_ENSEMBLE", "on").strip().lower() in ("1", "true", "on", "yes")
...
def _primary_model() -> str:
    return os.environ.get("CHUZOM_ENSEMBLE_PRIMARY", DEFAULT_PRIMARY)
...
def warm_primary(model: str | None = None) -> None:
    ...
    threading.Thread(target=_run, name="chuzom-ollama-warmup", daemon=True).start()
...
async def classify_for_routing(prompt: str, **_legacy_kwargs) -> "ClassificationResult":
    ...
    LLM-first: when CHUZOM_ENSEMBLE is on (default), the local ensemble classifies
    ...
    if _ensemble_enabled():
        timeout = float(os.environ.get("CHUZOM_ENSEMBLE_TIMEOUT", "120"))
