...
_SCHEMA = """
CREATE TABLE IF NOT EXISTS chuzom_cp_tenants (
    tenant_id TEXT PRIMARY KEY,
    org_id TEXT,
    created_at DOUBLE PRECISION
);

CREATE TABLE IF NOT EXISTS chuzom_cp_policy_versions (
    tenant_id TEXT,
    version INTEGER,
    yaml_text TEXT,
...
CREATE TABLE IF NOT EXISTS chuzom_cp_tenant_active_policy (
    tenant_id TEXT PRIMARY KEY,
    version INTEGER
);

CREATE TABLE IF NOT EXISTS chuzom_cp_instances (
...
CREATE TABLE IF NOT EXISTS chuzom_cp_heartbeats (
...
CREATE TABLE IF NOT EXISTS chuzom_cp_policy_notifications (
    id BIGSERIAL PRIMARY KEY,
    tenant_id TEXT,
    version INTEGER,
    digest TEXT,
...
class PostgresControlPlaneStore:
    def __init__(self, dsn: str | None = None) -> None:
        ...
        self._dsn = dsn or os.environ.get("CHUZOM_CP_POSTGRES_DSN", "")
        if not self._dsn:
            raise RuntimeError(
                "PostgresControlPlaneStore requires a DSN; set "
                "CHUZOM_CP_POSTGRES_DSN or pass dsn=..."
            )
        ...
        with self._conn.transaction():
            with self._conn.cursor() as cur:
                ...
                cur.execute(
                    ...
                    "FROM chuzom_cp_policy_versions "
                    "WHERE tenant_id = %s",
                    (tenant_id,),
                )
                row = cur.fetchone()
                ...
                cur.execute(
                    "INSERT INTO chuzom_cp_policy_versions ("
    ...
    def set_active_policy(self, tenant_id: str, version: int) -> None:
        ...
        with self._conn.transaction():
            with self._conn.cursor() as cur:
                cur.execute(
                    "INSERT INTO chuzom_cp_tenant_active_policy "
    ...
    def get_current_policy(
        ...
        with self._conn.cursor() as cur:
            cur.execute(
                ...
                "FROM chuzom_cp_tenant_active_policy AS ap "
                "JOIN chuzom_cp_policy_versions AS pv "
        ...
        *,
        ...
        with self._conn.transaction():
            with self._conn.cursor() as cur:
                ...
                cur.execute(
                    "INSERT INTO chuzom_cp_instances ("
                ...
                cur.execute(
                    "INSERT INTO chuzom_cp_heartbeats ("
                    "instance_id, tenant_id, effective_version, effective_digest, "
    ...
    def list_instances(self, tenant_id: str) -> list[InstanceHeartbeatRecord]:
        with self._conn.cursor() as cur:
            cur.execute(
                ...
                "source, sidecar_version, last_apply_latency_ms, last_seen_at "
                "FROM chuzom_cp_instances WHERE tenant_id = %s "
    ...
    def _ensure_tenant(
        ...
        cur.execute(
            "INSERT INTO chuzom_cp_tenants (tenant_id, org_id, created_at) "
    ...
    def _fetch_tenant(
        ...
        cur.execute(
            "SELECT tenant_id, org_id, created_at FROM chuzom_cp_tenants "
            "WHERE tenant_id = %s",
            (tenant_id,),
        )
        return cur.fetchone()
    ...
    def _fetch_policy_version(
        ...
        cur.execute(
            ...
            "created_at FROM chuzom_cp_policy_versions "
    ...
    def _fetch_instance(
        ...
        cur.execute(
            "SELECT instance_id, tenant_id, effective_version, effective_digest, "
            "source, sidecar_version, last_apply_latency_ms, last_seen_at "
            "FROM chuzom_cp_instances WHERE instance_id = %s",
            (instance_id,),
        )
        return cur.fetchone()
