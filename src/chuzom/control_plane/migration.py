...
(``CHUZOM_CP_SIDECAR_ENABLED`` etc.), at which point it fetches+verifies the same
...
from chuzom.control_plane.policy_bundle import make_payload, normalize_org_policy_yaml, policy_digest
from chuzom.control_plane.store import ControlPlaneStore
...
def _local_policy_yaml(policy_path: str | os.PathLike | None) -> str:
    ...
    path = policy_path or os.environ.get("CHUZOM_POLICY_PATH")
    p = Path(path) if path else Path.home() / ".chuzom" / "org-policy.yaml"
    if p.is_file():
