...
SEPARATE hash-chained SQLite DB (``CHUZOM_CP_AUDIT_PATH``), distinct from any
chuzom instance's local runtime audit. Reuses the tamper-evident
``chuzom.enterprise.audit`` machinery. Writes are best-effort (never break a
...
def _cp_audit_path() -> Path:
    return Path(os.environ.get("CHUZOM_CP_AUDIT_PATH", Path.home() / ".chuzom" / "cp_audit.db"))
...
def append_cp_event(
    *,
    ...
    try:
        ...
        chuzom.logging.get_logger("chuzom.control_plane.audit").warning(
