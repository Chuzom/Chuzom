...
def get_cp_store() -> ControlPlaneStore:
    ...
    with _store_lock:
        if _store_singleton is None:
            db_path = Path(
                os.environ.get(
                    "CHUZOM_CP_STORE_PATH",
                    str(Path.home() / ".chuzom" / "cp_store.db"),
                )
            )
...
def authenticate_sidecar(
    ...
    expected_token = os.environ.get("CHUZOM_CP_SIDECAR_TOKEN")
    presented_token = ""
    if authorization:
...
def _require_manage_policy() -> Callable[..., Any]:
    from chuzom.admin_api import require_perm
...
def get_signing_key():
    ...
    Loaded from ``CHUZOM_CP_ED25519_PRIVATE_KEY`` via signing.load_signing_key.
