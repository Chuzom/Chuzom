...
ALLOW_PUBLIC_ENV = "CHUZOM_ALLOW_PUBLIC_BIND"
...
LEGACY_SSE_ALLOW_PUBLIC_ENV = "CHUZOM_SSE_ALLOW_PUBLIC"
...
def refuse_public_bind_or_exit(host: str, *, component: str) -> None:
    ...
    Chuzom servers needs to know WHICH one refused; "refusing to bind" alone
    ...
    if not is_public_host(host) or allow_public_bind():
        return
    sys.stderr.write(
        f"[chuzom {component}] refusing to bind {host!r}: this exposes "
