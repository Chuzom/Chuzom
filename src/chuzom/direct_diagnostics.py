...
def diagnose(
    ...
    if observed >= _VIABLE_CEILING_S:
        return Advice(
            ...
            message=(
                ...
                f"Consider a smaller model, or leave CHUZOM_DIRECT_EXECUTION=false "
    ...
    return Advice(
        ...
        message=(
            ...
            f"Set CHUZOM_OLLAMA_TIMEOUT={int(suggested)} to match this machine."
        ),
...
def _samples_path():
    """Resolve via chuzom.paths, the ONE module that honours CHUZOM_HOME.

    CHZ-SR-01: 182 sites resolve ~/.chuzom themselves and ignore it, which is how
    a test that believed itself sandboxed reached the operator's real state and
    destroyed live data. A diagnostic that writes on every failed DIRECT attempt
    must not add a 183rd — and deliberately takes NO `home=` override, so a test
    isolates it the same way everything else does: by setting CHUZOM_HOME.
    """
    from chuzom.paths import state_path

    return state_path("direct_samples.jsonl")


def record_sample(elapsed_s: float, timed_out: bool) -> None:
...
def current_advice(timeout_s: float = 4.0) -> Optional[Advice]:
    """Advice from the persisted samples, or None."""
    return diagnose(load_samples(), timeout_s=timeout_s)
