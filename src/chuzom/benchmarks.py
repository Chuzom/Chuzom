...
copied to ``~/.chuzom/benchmarks.json`` on startup) and uses it to
...
# WP-03: derived from chuzom.pricing rather than hand-listed.
...
# telling chuzom.pricing that gpt-5.4 is free. The distinction matters: the same
...
_INSTALLED = Path.home() / ".chuzom" / "benchmarks.json"
...
def check_and_update_benchmarks() -> str | None:
    """Copy the bundled benchmarks file to ``~/.chuzom/`` if it is newer.
...
def get_benchmark_data() -> dict[str, Any] | None:
    ...
    Load order: ``~/.chuzom/benchmarks.json`` (user-local, may be newer
...
def get_model_failure_penalty(
    ...
    try:
        if failure_rates is not None:
            ...
            import asyncio
            from chuzom.cost import get_model_failure_rates
...
def get_quality_score(model_id: str, task_type: str) -> float:
    ...
    Lookup order:
      1. **Benchmark data** (``~/.chuzom/benchmarks.json``) — API models
...
def maybe_refresh_benchmarks_background(ttl_days: int = 7) -> bool:
    ...
    Checks whether ``~/.chuzom/benchmarks.json`` was last fetched more
    ...
    Args:
        ttl_days: Days before the benchmark file is considered stale.
            Reads ``chuzom_benchmark_ttl_days`` from config when available.
    ...
    try:
        ...
        ttl_days = get_config().chuzom_benchmark_ttl_days
    except Exception:
        pass
