"""Async sidecar routing service for chuzom.
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(
            Path.home() / ".chuzom" / "service.log", mode="a"
        ),
    ],
)
logger = logging.getLogger("chuzom.service")

app = FastAPI(title="chuzom-service", version="5.3.0")
...
def start_service(host: str = "127.0.0.1", port: int = 7337, log_level: str = "warning"):
    ...
    logger.info(f"Starting chuzom service on {host}:{port}")
...
if __name__ == "__main__":
    port = int(os.environ.get("CHUZOM_SERVICE_PORT", "7337"))
