...
from chuzom.types import PRO_FEATURES, Tier
...
def get_active_profile():
    ...
    return get_config().chuzom_profile
...
def _check_tier(feature: str) -> str | None:
    ...
    """
    config = get_config()
    if config.chuzom_tier == Tier.FREE and feature in PRO_FEATURES:
        return (
            f"'{feature}' requires Pro tier ($12/mo). "
            f"Current tier: free. Upgrade at https://chuzom.dev/pricing"
        )
    return None
