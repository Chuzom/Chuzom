...
GH#51/#52. Only `chuzom` and `chuzom-install-hooks` handled `--help`.
`chuzom-onboard` and `chuzom-quickstart` ignored it, ran their
interactive flows and died on `EOFError` the moment stdin was not a TTY;
`chuzom-route` ignored it and ran a full health check;
`chuzom-sse` tried `int("--help")` as a port — after having already made a
