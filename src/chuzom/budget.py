...
Pressure sources by provider type:
  ...
  Claude subscription (anthropic)
      Reads ``~/.chuzom/usage.json`` (written by the session-start hook).
...
_USAGE_JSON = Path.home() / ".chuzom" / "usage.json"
...
# Env var override: CHUZOM_STALE_PRESSURE_FLOOR (float 0.0–1.0, default 0.5)
...
async def get_budget_state(provider: str) -> BudgetState:
    ...
    Returns:
        :class:`~chuzom.types.BudgetState` for the provider.
    """
    # Cache hit
...
async def get_all_budget_states() -> dict[str, BudgetState]:
    ...
    Returns:
        Dict mapping provider name → :class:`~chuzom.types.BudgetState`.
    """
    cfg = get_config()
...
async def _claude_subscription_state() -> BudgetState:
    ...
    import os
    stale_floor = float(os.environ.get("CHUZOM_STALE_PRESSURE_FLOOR", "0.5"))
    try:
...
async def _api_provider_state(provider: str) -> BudgetState:
    ...
    Spend sources:
      ...
      2. Helicone pull (when CHUZOM_HELICONE_PULL=true)
      3. LiteLLM Proxy DB (when CHUZOM_LITELLM_BUDGET_DB is set)

    Aggregation strategy is controlled by ``chuzom_spend_aggregation``:
    ...
    try:
        from chuzom.integrations.litellm_budget import (
    ...
    if cfg.chuzom_spend_aggregation == "sum":
...
def _get_cap(provider: str, cfg) -> float:
    ...
    """
    from chuzom.budget_store import get_cap as _store_cap
    stored = _store_cap(provider)
    if stored > 0.0:
        return stored

    cap_map = {
        "openai": cfg.chuzom_budget_openai,
        "gemini": cfg.chuzom_budget_gemini,
        "groq": cfg.chuzom_budget_groq,
        "deepseek": cfg.chuzom_budget_deepseek,
        "together": cfg.chuzom_budget_together,
        "perplexity": cfg.chuzom_budget_perplexity,
        "mistral": cfg.chuzom_budget_mistral,
    }
