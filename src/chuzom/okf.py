...
# `git add -A` and committed. Nothing chuzom infers should land in someone's
# history by accident.
...
# cross-contamination fix. `chuzom okf gc` reports and relocates them.
...
def project_slug(root: Path | None = None) -> str:
    ...
    The hash disambiguates same-named checkouts (two clones both called `chuzom`)
...
def gc_store(base: Path = KNOWLEDGE_DIR, apply: bool = False) -> dict[str, Any]:
    ...
    for explicitly, so `apply` has to be set — `chuzom okf gc` reports, and
    `chuzom okf gc --apply` acts.
    """
