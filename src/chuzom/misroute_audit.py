...
``chuzom.audit_routing`` already exists and is a different feature: a *live*,
...
break live routing. ``CHUZOM_AUDIT_DISABLED`` turns it off entirely, and it is
...
log = get_logger("chuzom.misroute_audit")
...
def audit_disabled() -> bool:
    """True when ``CHUZOM_AUDIT_DISABLED`` is set to an affirmative value."""
...
async def run_audit(limit: int = 100) -> dict[str, Any]:
    ...
    try:
        from chuzom.routing_quality import summarize as _summarize_quality
