"""Best-effort operational alert sink for critical chuzom events.

chuzom already *detects* and *acts on* critical conditions (audit-chain
...
standard structlog logger; if ``CHUZOM_ALERT_WEBHOOK`` is set, it also
POSTs a small JSON envelope there. Disable entirely with
``CHUZOM_ALERTS_DISABLED=1``.
"""
...
def emit_alert(
    ...
    *,
    ...
    if _env_on("CHUZOM_ALERTS_DISABLED"):
        return
    ...
    webhook = (os.environ.get("CHUZOM_ALERT_WEBHOOK") or "").strip()
    if not webhook:
        return
