...
class QuotaTracker:
    ...
    - USAGE_JSON: Path to cached usage.json (default ~/.chuzom/usage.json)
    ...
    TTL_SECONDS: int = int(os.environ.get("CHUZOM_QUOTA_TTL", "300"))
    MAX_RETRY: int = int(os.environ.get("CHUZOM_QUOTA_RETRY", "3"))
    RETRY_DELAY_SEC: float = float(os.environ.get("CHUZOM_QUOTA_DELAY", "2.0"))
    USAGE_JSON: Path = Path.home() / ".chuzom" / "usage.json"
    ...
    async def _fetch_claude_oauth_data(self) -> tuple[float, float, float]:
        ...
        from chuzom.claude_usage import refresh_claude_usage
