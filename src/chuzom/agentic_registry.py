...
Draft generated via chuzom routing (codex/gpt-5.5), then verified against the
...
from chuzom.hooks.agent_loop import _get_ollama_url, run_agent_loop


CACHE_PATH = Path.home() / ".chuzom" / "agentic_models.json"
PROBE_PROMPT = (
...
def probe_model(model: str, timeout: int = 90) -> bool:
    ...
    temp_dir = tempfile.mkdtemp(prefix="chuzom-agentic-probe-")
...
def populate_in_background() -> bool:
    ...
    try:
        ...
        subprocess.Popen(
            [_sys.executable, "-m", "chuzom.agentic_registry"],
