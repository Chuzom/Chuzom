...
reasoning. Chuzom already has a ``DEEP_REASONING`` tier, but today it fires from
...
log = get_logger("chuzom.reason_gate")
...
def _classify_regexes() -> tuple[re.Pattern, re.Pattern]:
    ...
    from chuzom.classify import _COMPLEXITY_COMPLEX, _COMPLEXITY_DEEP
...
def _params() -> tuple[dict[str, float], float]:
    ...
    from chuzom.semantic_classify import _centroids_path
