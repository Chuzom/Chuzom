...
This module is the central dispatch layer of chuzom. It receives a
...
async def _build_and_filter_chain(
    ...
    if model_override:
        ...
        if (
            ...
            or (config.chuzom_gemini_subscription and model_override.startswith("gemini/"))
        ):
            log.warning(
    ...
    try:
        from chuzom.dynamic_routing import get_dynamic_model_chain
    ...
    if task_type not in MEDIA_TASK_TYPES:
        ...
        if active_agent is None:
            ...
            elif config.chuzom_gemini_subscription:
...
def _resolve_profile(
    ...
    Args:
        ...
        config: Application config (provides chuzom_profile default).
    ...
    if profile is None and not model_override:
        ...
        resolved = profile or config.chuzom_profile
...
def _auth_error_hint(provider: str) -> str:
    ...
    if env_var:
        return (
            ...
            f"    Fix: run `chuzom setup` to configure it, or set {env_var} in "
            f"~/.chuzom/.env\n"
    ...
    return (
        ...
        f"    Fix: run `chuzom setup` to configure your providers.\n"
        f"    Note: Claude Code subscription covers Haiku/Sonnet/Opus — no API key needed "
...
async def route_and_call(
    ...
    *,
    ...
    with traced_span(
        ...
        #
        # v10.0.0 migration knob: ``CHUZOM_BANDIT=off`` skips the reorder
        ...
        _bandit_off = os.environ.get("CHUZOM_BANDIT", "on").lower() in {"off", "0", "false", "no"}
        if models_to_try and not model_override and not _bandit_off:
            try:
                from chuzom.bandit import EpsilonGreedyBandit
        ...
        _escalate_above = getattr(config_for_escalation, "chuzom_escalate_above", 0.0)
        if isinstance(_escalate_above, (int, float)) and _escalate_above > 0 and task_type not in MEDIA_TASK_TYPES:
            try:
                ...
                if _estimated > config_for_escalation.chuzom_escalate_above:
                    from chuzom.tools.admin import _set_pending_approval
                    _set_pending_approval({"model": _top_model, "estimated_cost": _estimated})
                    raise BudgetExceededError(
                        f"Call to {_top_model} (estimated ${_estimated:.4f}) exceeds "
                        f"CHUZOM_ESCALATE_ABOVE=${config_for_escalation.chuzom_escalate_above:.2f}. "
                ...
                if (config_for_escalation.chuzom_hard_stop_above > 0
                        and _session_total >= config_for_escalation.chuzom_hard_stop_above):
                    raise BudgetExceededError(
                        f"Session spend ${_session_total:.4f} has reached the hard stop limit "
                        f"(CHUZOM_HARD_STOP_ABOVE=${config_for_escalation.chuzom_hard_stop_above:.2f}). "
        ...
        if system_prompt is None and task_type not in MEDIA_TASK_TYPES and models_to_try:
            try:
                from chuzom.context_prep import prepare_prompt as _prepare
                _prepared = _prepare(
                    user_prompt=prompt,
                    task_type=task_type,
                    complexity=c,
