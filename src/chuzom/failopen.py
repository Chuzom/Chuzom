...
* **Append-only, one small write per event.** Same reasoning as
  :mod:`chuzom.coverage`: these are separate short-lived processes and this sits
...
def store_path():
    """Path to the fail-open counter store, inside CHUZOM_HOME when isolated."""
...
def record(code: str, exc: BaseException | None = None, *, detail: str = "") -> None:
    ...
    try:
        ...
        structlog.get_logger("chuzom.failopen").debug(
