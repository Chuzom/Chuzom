...
from chuzom.policy import RoutingPolicy, PolicyManager, get_policy_manager
...
def _select_preset_policy(pm: PolicyManager) -> None:
    ...
    os.environ["CHUZOM_POLICY"] = choice
    
    print(f"\n✅ Selected policy: {choice}")
    print(f"   Set CHUZOM_POLICY={choice} in your environment")
...
def _create_custom_policy(pm: PolicyManager) -> None:
    ...
    print("\n💡 To use this policy:")
    print(f"   export CHUZOM_POLICY={name}")
    print("   Or set in ~/.env or ~/.chuzom/config.yaml")
