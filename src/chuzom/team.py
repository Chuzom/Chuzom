...
def get_user_id(override: str = "") -> str:
    ...
    Priority:
      1. ``override`` arg (from ``CHUZOM_USER_ID`` env var)
...
def _slack_payload(report: dict[str, Any]) -> dict:
    ...
    return {
        "blocks": [
            ...
            {
                "type": "context",
                "elements": [{"type": "mrkdwn", "text": "Powered by <https://github.com/ypollak2/chuzom|chuzom>"}],
            },
        ]
    }
...
def _discord_payload(report: dict[str, Any]) -> dict:
    ...
    return {
        "embeds": [{
            ...
            "footer": {"text": "chuzom · github.com/ypollak2/chuzom"},
        }]
    }
...
def _generic_payload(report: dict[str, Any]) -> dict:
    return {"source": "chuzom", "version": "3.0", "report": report}
...
async def push_report(
    ...
    if channel == "slack":
        ...
        if not telegram_chat_id:
            return False, "Telegram requires CHUZOM_TEAM_CHAT_ID to be set."
        post_url = url.rstrip("/")
...
async def build_team_report(
    ...
    """
    from chuzom.cost import get_team_savings
