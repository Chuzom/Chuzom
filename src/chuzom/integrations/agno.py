...
def _llm_response_to_model_response(resp: LLMResponse) -> ModelResponse:
    """Convert chuzom LLMResponse to an Agno ModelResponse."""
...
class RouteredModel(Model):
    """Drop-in Agno Model that routes each call through chuzom.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(id="chuzom", provider="chuzom", name="RouteredModel", **kwargs)
    ...
    def invoke_stream(
        ...
        # chuzom doesn't stream natively — return full response as one chunk
        assistant_message.metrics.start_timer()
        llm_resp = _run_coroutine_sync(self._aroute(messages))
        assistant_message.metrics.stop_timer()
...
class RouteredTeam(Team):
    ...
    async def _apply_budget_pressure(self) -> bool:
        ...
        try:
            from chuzom.cost import get_quality_report
