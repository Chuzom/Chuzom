"""LiteLLM BudgetManager integration for chuzom.

When teams run a LiteLLM Proxy with budget management enabled, chuzom
...
This allows enterprises to:
  ...
  - Have chuzom automatically route away from expensive providers
...
Configuration::
    ...
    export CHUZOM_LITELLM_BUDGET_DB=/path/to/litellm.db

    # Optional: which user/team key to track (defaults to aggregate)
    export CHUZOM_LITELLM_USER=my-team
...
_DB_ENV = "CHUZOM_LITELLM_BUDGET_DB"
_USER_ENV = "CHUZOM_LITELLM_USER"
...
async def get_litellm_budget_cap(provider: str, user: str | None = None) -> float:
    ...
    Args:
        ...
        user:     LiteLLM user/team key. Defaults to ``CHUZOM_LITELLM_USER``.
