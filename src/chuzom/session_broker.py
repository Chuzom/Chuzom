...
Problem (P1): Chuzom's always-on gateway daemon runs headless (launchd), so it
...
(`chuzom broker run`). It listens on a per-user Unix socket. The gateway keeps
...
Security model (local-only, defence in depth):
  * Unix domain socket at ~/.chuzom/broker.sock, mode 0600, owner-only.
  * Every message is HMAC-signed with a per-user secret (~/.chuzom/broker.secret,
...
log = get_logger("chuzom.session_broker")
...
def broker_socket_path() -> Path:
    """Per-user Unix socket path. Override with CHUZOM_BROKER_SOCK (tests)."""
    override = os.environ.get("CHUZOM_BROKER_SOCK")
    return Path(override) if override else _chuzom_dir() / "broker.sock"


def _secret_path() -> Path:
    override = os.environ.get("CHUZOM_BROKER_SECRET_FILE")
    return Path(override) if override else _chuzom_dir() / "broker.secret"
...
async def run_broker_server() -> None:
    """Entry point for `chuzom broker run` — serve until interrupted."""
