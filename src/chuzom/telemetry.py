...
``routing_decisions`` table in :mod:`chuzom.cost` already captures every
...
(:mod:`chuzom.bandit`) consumes when reordering the candidate chain.

Design principles mirror :mod:`chuzom.calibration`:
...
log = logging.getLogger("chuzom.telemetry")
