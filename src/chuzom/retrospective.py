"""IAF-style session retrospective engine for chuzom.
...
RETROSPECT_DIR = Path.home() / ".chuzom" / "retrospectives"
DIRECTIVES_FILE = Path.home() / ".chuzom" / "directives.md"
SESSION_START_FILE = Path.home() / ".chuzom" / "session_start.txt"
...
def write_retrospective_file(retro: dict, session_id: str = "") -> Path:
    ...
    File location: ~/.chuzom/retrospectives/YYYY-MM-DD-HHMMSS.md
...
def write_directives(actions: list[dict]) -> None:
    ...
    File: ~/.chuzom/directives.md (append-only)
