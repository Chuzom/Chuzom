...
DEFAULT_PORT = 7337
_TOKEN_FILE = Path.home() / ".chuzom" / "dashboard.token"
...
async def _get_stats() -> dict:
    ...
    try:
        ...
        _pkg_ver = "?"
    stats["config"] = {
        "profile": config.chuzom_profile.value,
        "monthly_budget": config.chuzom_monthly_budget,
        "daily_limit": config.chuzom_daily_spend_limit,
        "subscription_mode": config.chuzom_claude_subscription,
        "version": _pkg_ver,
    }
...
<div id="tab-budget" class="p-8 max-w-[1600px] mx-auto space-y-8 hidden">
  ...
  <p class="text-xs text-outline mt-2">Caps are saved to <code class="font-mono">~/.chuzom/budgets.json</code> and take effect immediately on the next routing call.</p>
</div>
...
async def run(port: int = DEFAULT_PORT) -> None:
    ...
    async def handle_budget_get(request: "web.Request") -> "web.Response":
        from chuzom.budget import get_all_budget_states
        from chuzom.budget_store import list_caps
    ...
    async def handle_budget_set(request: "web.Request") -> "web.Response":
        ...
        from chuzom.budget import invalidate_cache
        if cap <= 0:
            remove_cap(provider)
        else:
    ...
    async def handle_metrics(request: "web.Request") -> "web.Response":
        ...
        lines = [
            "# HELP chuzom_spend_usd Monthly spend per provider",
            "# TYPE chuzom_spend_usd gauge",
        ]
        for p, s in sorted(states.items()):
            lines.append(f'chuzom_spend_usd{{provider="{p}"}} {s.spend_usd:.4f}')

        lines += [
            "# HELP chuzom_budget_pressure Budget pressure per provider (0=free 1=exhausted)",
            "# TYPE chuzom_budget_pressure gauge",
        ]
        for p, s in sorted(states.items()):
            lines.append(f'chuzom_budget_pressure{{provider="{p}"}} {s.pressure:.4f}')

        lines += [
            "# HELP chuzom_cap_usd Configured monthly cap per provider (0=unlimited)",
            "# TYPE chuzom_cap_usd gauge",
        ]
        for p, s in sorted(states.items()):
            lines.append(f'chuzom_cap_usd{{provider="{p}"}} {s.cap_usd:.2f}')

        lines += [
            "# HELP chuzom_savings_usd_total Lifetime savings vs Sonnet baseline",
            "# TYPE chuzom_savings_usd_total counter",
            f"chuzom_savings_usd_total {total_saved:.4f}",
            "# HELP chuzom_external_usd_total Lifetime external API spend",
            "# TYPE chuzom_external_usd_total counter",
            f"chuzom_external_usd_total {total_external:.4f}",
            "",
        ]
    ...
    # The tokenized URL must NOT go through structlog: configure_logging()
    # installs the scrubber as the first processor process-wide, and its "token"
    # pattern matches both the token= field and the url= field (the URL carries
    # ?token=<value> as a substring). Both arrived as [REDACTED-TOKEN], making
    # the only documented way to obtain a working URL unusable — while the
    # credential still travelled through the log pipeline for no benefit.
