...
from chuzom.types import ClassificationResult
...
def _resolve_alias(model: str) -> str:
    ...
    """
    try:
        from chuzom.model_aliases import resolve_model_alias
...
class PolicyManager:
    ...
    DEFAULT_POLICY_DIR = Path.home() / ".chuzom" / "policies"
    ...
    def load_policy(self, name: str) -> RoutingPolicy:
        ...
        Searches in order:
        1. User custom policies (~/.chuzom/policies/)
        2. Preset policies (bundled with chuzom)
    ...
    def get_active_policy(self) -> RoutingPolicy:
        ...
        Loads from CHUZOM_POLICY env var, or returns default (balanced).
        ...
        policy_name = os.environ.get("CHUZOM_POLICY", "balanced")
        try:
...
def load_org_policy(path: Path | None = None) -> OrgPolicy | None:
    ...
    Args:
        path: Path to org policy YAML file. Defaults to ~/.chuzom/org-policy.yaml

    Returns:
        OrgPolicy if file exists, else default permissive policy
    """
    if path is None:
        path = Path.home() / ".chuzom" / "org-policy.yaml"
