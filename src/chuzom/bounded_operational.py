...
Ships behind ``CHUZOM_BOUNDED_OPERATIONAL`` (default OFF for the first release, §8.2
...
# Representative tool-capable model per tier for pricing (cheapest first). Local
# tiers price at ~$0, so budget derivation uses a cheap EXTERNAL model to size a cap
# that can absorb one escalation to a paid tier.
_TIER_PRICING_MODEL = {
...
def bounded_operational_enabled() -> bool:
    ...
    return os.environ.get("CHUZOM_BOUNDED_OPERATIONAL", "0").strip().lower() in (
        "1", "true", "yes", "on")
...
def get_model_prices(model_tier: int) -> tuple[float, float]:
    ...
    model = _TIER_PRICING_MODEL.get(int(model_tier), "gpt-4o-mini")
    try:
        from chuzom.calibration import _lookup_pricing
        ...
        return 0.15, 0.60  # gpt-4o-mini defaults
