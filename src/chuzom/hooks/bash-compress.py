...
Env vars:
  CHUZOM_BASH_COMPRESS       Set to "off" to disable this hook
  CHUZOM_DB_PATH             SQLite path for compression stats (default: ~/.chuzom/usage.db)
"""
...
try:
    from chuzom.compression.rtk_adapter import RTKAdapter
except ImportError:
    RTKAdapter = None
...
def main() -> None:
    ...
    if os.environ.get("CHUZOM_BASH_COMPRESS", "").lower() == "off":
        sys.exit(0)
