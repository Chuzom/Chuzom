...
from chuzom.library.store import LibraryStore, now_utc, scrub_secrets  # noqa: E402
...
def harvest(hook_input: dict) -> None:
    ...
    try:
        from chuzom.library.sealer import maybe_seal_on_event
...
def main() -> int:
    ...
    try:
        if "--session-start" in sys.argv:
            ...
            store = LibraryStore.for_repo(Path(cwd))
            if store is not None:
                from chuzom.library.pack import pack_for
            ...
            if store is not None:
                ...
                if book:
                    from chuzom.library.book_closer import close_book
