...
which chuzom MCP tool to call.

Usage: Installed at ~/.chuzom/hooks/gemini-cli-auto-route.py by `chuzom install`.
...
async def classify_complexity(prompt: str) -> tuple[str, float]:
    ...
    try:
        from chuzom.classifier import classify_prompt_heuristic
