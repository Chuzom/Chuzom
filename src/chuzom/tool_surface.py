...
``CHUZOM_SLIM`` defaulted to ``consolidated``, a tier under which *none of them
...
not guaranteed to run under an interpreter that has ``chuzom`` importable (the
...
``chuzom.tool_tiers`` re-exports the tier sets from here, and
``chuzom.tools.consolidated`` re-exports :data:`DEPRECATED_TOOLS` from here.
Adding a ``chuzom.*`` import to this file re-breaks the standalone path.
"""
...
# Single source of truth for the 1.0 cutover. ``chuzom.tools.consolidated`` and
...
DEPRECATED_TOOLS: dict[str, str] = {
    ...
    "llm_check_usage": "chuzom_status",  # emitted by agent-route's stale-quota note
    # config → chuzom_admin
...
def active_slim() -> str:
    ...
    Reads ``CHUZOM_SLIM`` directly rather than importing ``chuzom.config`` so
    hooks stay light. The default MUST track ``Config.chuzom_slim`` — an unset
    env var means ``consolidated``, not ``off``. Getting this default wrong is
    the original bug (an emitter assuming the legacy surface).
    """
    return (os.environ.get("CHUZOM_SLIM") or "consolidated").strip().lower()


def registered_tools(slim: str | None = None) -> frozenset[str] | None:
    """Tools registered under ``slim``; ``None`` means "all of them" (tier off).

    Unknown tier names mirror :func:`chuzom.tool_tiers.make_should_register` and
...
# `chuzom.tools.consolidated._TIER_TO_COMPLEXITY` — renaming the TOOL without
