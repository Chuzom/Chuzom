...
correct probe would have to point CHUZOM_DB_PATH at a tmpdir, run the coroutine,
...
BASELINE_ERA_MUTATIONS: list[Mutation] = [
    # ── money ────────────────────────────────────────────────────────────────
    Mutation(
        "B1", "money", "src/chuzom/execution_ledger.py",
        ...
        "survive review.",
        probe="__import__('chuzom.execution_ledger', fromlist=['x'])._host_opus_rates()",
    ),
    Mutation(
        "B2", "money", "src/chuzom/cost.py",
        ...
        probe=(
            "[__import__('chuzom.cost', fromlist=['x'])._claude_cost('claude-opus-5', 1000, 1000)]"
        ),
    ),
    Mutation(
        "B3", "money", "src/chuzom/cost.py",
        ...
        probe="__import__('chuzom.cost', fromlist=['x'])._claude_cost('claude-sonnet-5', 500, 500)",
    ),
    # ── routing ──────────────────────────────────────────────────────────────
    Mutation(
        "B4", "routing", "src/chuzom/tool_surface.py",
        ...
        probe="__import__('chuzom.tool_surface', fromlist=['x']).is_registered('llm_bogus_zzz', 'core')",
    ),
    Mutation(
        "B5", "routing", "src/chuzom/tool_surface.py",
        ...
        probe="sorted(__import__('chuzom.tool_surface', fromlist=['x']).registered_tools('core') or [])",
    ),
    Mutation(
        "B6", "routing", "src/chuzom/tool_surface.py",
        ...
        probe=(
            "__import__('chuzom.tool_surface', fromlist=['x']).resolve('llm_generate', 'core').degraded"
        ),
    ),
    # ── verification / ledger ────────────────────────────────────────────────
    Mutation(
        "B7", "verification", "src/chuzom/execution_ledger.py",
        'return Path.home() / ".chuzom" / "usage.db"',
        'return Path.home() / ".chuzom" / "usage_alt.db"',
        ...
        probe="str(__import__('chuzom.execution_ledger', fromlist=['x'])._db_path())",
    ),
    Mutation(
        ...
        probe=(
            ...
            "(__import__('chuzom.budget', fromlist=['x']))"
        ),
    ),
    Mutation(
        "B9", "verification", "src/chuzom/budget.py",
        ...
        probe=(
            "(lambda b: (b._pending_tokens.update({'openai': 10**6}), b._get_pending_pressure_offset('openai'))[1])(__import__('chuzom.budget', fromlist=['x']))"
        ),
    ),
    Mutation(
        "B10", "verification", "src/chuzom/budget.py",
        ...
        probe=(
            "__import__('chuzom.budget', fromlist=['x'])._neutral('openai').pressure"
        ),
    ),
]
