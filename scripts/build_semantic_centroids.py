...
Usage:
    python scripts/build_semantic_centroids.py \
        ...
        --out src/chuzom/data/semantic_centroids.json \
...
CHUZOM_SEMANTIC_CLASSIFIER_BACKEND (ollama default | st). The artifact records
...
# Make `chuzom` importable when run from the repo root without install.
sys.path.insert(0, str(Path(__file__).resolve().parent.parent / "src"))

from chuzom.contamination_audit import audit  # noqa: E402
from chuzom.semantic_centroids import LabeledPrompt, build_artifact  # noqa: E402
...
def _load_ra_hashes(args) -> set[str] | None:
    ...
    if args.ra_prompts:
        from chuzom.contamination_audit import hash_set
...
def _embed_fn():
    ...
    from chuzom import semantic_classify as sc

    backend = os.getenv("CHUZOM_SEMANTIC_CLASSIFIER_BACKEND", "ollama").strip().lower()
    model = (
        os.getenv("CHUZOM_SEMANTIC_ST_MODEL", "all-MiniLM-L6-v2")
