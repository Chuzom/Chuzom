...
Output: src/chuzom/data/domain_centroids.json — consumed by chuzom_domain_router.
"""
...
from chuzom.semantic_centroids import (  # noqa: E402
    _kmeans, build_prototype, calibrate_floor, calibrate_temperature,
)
from chuzom.semantic_classify import _embed, _norm  # noqa: E402
...
def main() -> int:
    ...
    ap.add_argument("--data-dir", type=Path, default=Path("src/chuzom/data"))
    ap.add_argument("--k", type=int, default=2, help="prototypes per domain")
    ap.add_argument("--out", type=Path, default=Path("src/chuzom/data/domain_centroids.json"))
    args = ap.parse_args()
