...
`chuzom.<module>.<def name>`, e.g. `chuzom.execution_ledger.x__db_path__mutmut_1`.

THE SPLIT
---------
    seed = sha256("chuzom-gf-v1" + baseline_sha + universe_sha)
...
The holdout list is written to `.chuzom/gf/` (gitignored) so a run can execute
...
MUTANTS = REPO / "mutants" / "src" / "chuzom"
SEALED = REPO / ".chuzom" / "zero-tolerance-audit" / "gf"
WORKING = REPO / ".chuzom" / "gf"
...
def enumerate_universe() -> list[tuple[str, str, str]]:
    ...
    for mod in MODULES:
        ...
        for defname in _DEF.findall(path.read_text()):
            ...
            out.append((f"chuzom.{mod}.{defname}", mod, base))
    if not out:
...
def main() -> int:
    ...
    seed_material = f"chuzom-gf-v1{baseline_sha}{universe_sha}"
    ...
    meta = {
        "protocol": ".chuzom/zero-tolerance-audit/20_GF_QUALIFICATION_PROTOCOL.md",
        "baseline_sha": baseline_sha,
        "git_dirty": bool(_git("status", "--porcelain")),
        "universe_sha256": universe_sha,
        "universe_size": len(names),
        "seed_material": "sha256('chuzom-gf-v1' + baseline_sha + universe_sha256)",
        ...
        "holdout_list_location": "UNVERSIONED (.chuzom/gf/holdout.txt)",
