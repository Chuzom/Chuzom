...
`chuzom.tool_surface`) and checks the hook's emitted name against it.
...
HOOK = REPO / "src" / "chuzom" / "hooks" / "auto-route.py"
...
def server_registered_tools() -> set[str]:
    ...
    Deliberately not `chuzom.tool_surface.registered_tools()` — using the module
...
def run_hook(prompt: str, tier: str, session: str) -> str:
    ...
    env = {
        **os.environ,
        "CHUZOM_SLIM": tier,
        "CHUZOM_DIRECT_EXECUTION": "false",  # want the directive, not a draft
    }
    proc = subprocess.run(
...
_TOOL_RE = re.compile(r"\b(llm(?:_\w+)?|chuzom_\w+)(\(task=\"(\w+)\"\))?")
...
def invoke(bare: str, task: str | None, prompt: str) -> tuple[bool, str]:
    ...
    from chuzom import server as _srv  # noqa: F401  (ensures tools are registered)
    from chuzom.tools.consolidated import llm
    from chuzom.tools.text import llm_code, llm_query
...
def main() -> int:
    ...
    for tier in tiers:
        ...
        proc = subprocess.run(
            [sys.executable, "-c",
             "import asyncio,json;from chuzom import server as s;"
             "print(json.dumps(sorted(t.name for t in asyncio.run(s.mcp.list_tools()))))"],
            capture_output=True, text=True,
            env={**os.environ, "CHUZOM_SLIM": tier}, timeout=300,
        )
        try:
