...
#: of "src"/"chuzom" in the file. Two looser rules were tried first and both were WRONG,
#: in the same both-directions way as the CHZ-SS-01 lint earlier in this audit:
#:
#:   "any traversal + the word chuzom"  -> 20 files, sweeping in sandbox tests that walk
#:                                        tmp_path.rglob("*") and settings.json*.bak
#:   "constructs a src/chuzom path"     -> 110+ files, because nearly every test builds
...
#: "chuzom" -- and swept in whole files like test_welcome_cli.py, because nearly every
...
#: mutmut rewrites only the files in `only_mutate`; every other file under src/chuzom is
#: copied verbatim, so reading it yields identical text in both trees and can never
#: produce a spurious kill.
#:
#: Requiring only "reads something under src/chuzom" produced two false positives that
#: made this obvious: test_cluster5 reads `src/chuzom/rules/chuzom.md` (markdown, never
#: mutated) and test_welcome_cli reads `src/chuzom/cli.py` (a .py file, but not in scope).
