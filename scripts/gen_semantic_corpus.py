...
Usage:
    python scripts/gen_semantic_corpus.py --out-dir src/chuzom/data \
...
def _unique_by_task(rng: random.Random, seen: set[str]) -> dict[str, list[dict]]:
    ...
    """
    from chuzom.contamination_audit import normalize
...
def main() -> int:
    ...
    ap.add_argument("--out-dir", type=Path, default=Path("src/chuzom/data"))
