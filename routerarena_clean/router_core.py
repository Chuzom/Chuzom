...
_MATH_TeX = re.compile(r"\\(?:frac|sqrt|sum|int|binom|cdot|theta|alpha|pi)\b|\$[^$\n]{2,}\$")
...
def decide(query: str, call_fn: CallFn, pool: Pool, tau: float = 0.999,
           self_consistency_k: int = 0) -> Decision:
    ...
    if not trust and self_consistency_k and probes:
        cheapest = probes[0]
        samples = [extract_answer(call_fn(cheapest, query)) for _ in range(self_consistency_k)]
    ...
    if trust:
        return Decision(model=probes[0], reason="probes-agree", agreement=frac,
                        escalated=False, probe_answers=answers)
    return Decision(model=pool.strong, reason="disagree-escalate", agreement=frac,
                    escalated=True, probe_answers=answers)
